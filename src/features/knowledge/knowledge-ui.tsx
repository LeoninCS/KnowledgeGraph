...
} from "../../data/knowledge-points/sources";
import type { GraphKnowledgePoint } from "../../data/knowledge-points/types";
import { coreKnowledgePointIdsByCategory } from "../../data/knowledge-points/core";
import { categoryIds } from "../../data/knowledge-points/metadata";
...
export async function findBestSearchMatch(
  t: CategoryCopy,
  query: string,
  preloadedPoints: Partial<Record<CategoryId, GraphKnowledgePoint[]>> = {},
) {
  const normalized = normalizeSearch(query);
  ...
  let bestMatch: KnowledgeSearchMatch | undefined;
  const categoryPoints = await Promise.all(
    categoryIds.map(async (categoryId) => ({
      categoryId,
      points: preloadedPoints[categoryId] ?? await loadKnowledgePoints(categoryId),
    })),
  );

  for (const { categoryId, points } of categoryPoints) {
    ...
    points.forEach((point) => {
      ...
      if (score > (bestMatch?.score ?? 0)) {
        bestMatch = { categoryId, pointId: point.id, score };
      }
    });
  }
...
export async function findFirstSearchMatch(
  t: CategoryCopy,
  query: string,
  preloadedPoints: Partial<Record<CategoryId, GraphKnowledgePoint[]>> = {},
) {
  const match = await findBestSearchMatch(t, query, preloadedPoints);
...
export const essentialKnowledgeIds: Record<CategoryId, string[]> = coreKnowledgePointIdsByCategory;
...
export function getVisualizablePoints(
  categoryId: CategoryId,
