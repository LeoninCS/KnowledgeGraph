...
const mysqlKnowledgePointBase = [
  ...
  /* <!-- KG_REVIEWED: 反范式 | 2026-06-05 | source_count=30 --> */
  ...
  /* <!-- KG_REVIEWED: 行锁 | 2026-06-05 | source_count=13 --> */
  /* <!-- KG_EXPLAINED: 行锁 | 2026-05-23 | source_count=5 --> */
  {
    ...
    area: "lock",
    difficulty: "medium",
    concept: "行锁是 InnoDB 在索引记录粒度串行化当前读和写入冲突的并发控制机制。",
    explanation: [
      "概念定位：行锁（Row Lock）解决的是高并发 OLTP 系统里“同一批行同时被读写或修改”时的一致性和吞吐问题。库存扣减、订单状态流转、账户余额更新、任务队列领取、幂等去重表写入都会触发行锁。\n\nInnoDB 的行锁与索引强绑定：执行器按访问路径扫描索引，存储引擎在遇到的索引记录上放置共享锁或排他锁。索引越精准，锁影响范围越小；事务越短，等待链越短；观测越充分，线上判断越可靠。",
      "准确定义：MySQL 语境里的行锁通常指 InnoDB row-level locking。它保护的是索引记录及其相关范围，而业务上看到的是“某一行正在被事务保护”。\n\n常见触发入口如下：\n\n- `UPDATE`、`DELETE`：对匹配行加排他锁（`X lock`），直到事务结束释放。\n- `SELECT ... FOR UPDATE`：读取最新可锁版本并加排他锁，适合后续要修改的行。\n- `SELECT ... FOR SHARE`：读取最新可锁版本并加共享锁（`S lock`），适合确认引用关系或防止目标行被改动。\n- 普通 `SELECT`：多数场景走 MVCC 一致性读，主要读取快照版本。\n- `NOWAIT`、`SKIP LOCKED`：只改变锁等待语义，常用于低等待的队列领取或抢占式处理。",
      "心智模型：把 InnoDB 表想成多棵 B+ 树。行锁像贴在索引叶子项上的占用标签，事务沿着执行计划走哪棵树、扫过哪些叶子项，就可能把标签贴到哪些位置。\n\n这个模型能解释三个工程现象：\n\n- 主键等值命中通常锁得很窄，冲突集中在目标记录。\n- 二级索引锁定读常同时涉及二级索引项和聚簇索引记录，回表路径也会进入锁分析。\n- 范围条件、低选择性条件和缺少合适索引会扩大扫描面，锁等待、死锁和连接堆积概率随之上升。",
      "主流程机制：一次典型 `UPDATE orders SET status = 'paid' WHERE id = 1001 AND status = 'created'` 会沿着下面路径推进。\n\n1. SQL 层解析语句，优化器根据统计信息选择访问路径，例如主键 `PRIMARY`。\n2. InnoDB 打开事务上下文，读取当前可锁版本，检查记录是否满足 `WHERE` 条件。\n3. 存储引擎在扫描到的索引记录上申请 `X` 记录锁；已有冲突锁时，事务进入等待队列。\n4. 获得锁后生成 Undo 旧版本，修改聚簇索引记录和相关二级索引，写入 Redo 记录。\n5. 事务提交后释放行锁，等待事务被唤醒，复制、Binlog、监控和慢日志记录对应证据。\n\n行锁的结果是：同一行的冲突写入被串行化，其他无关行可以继续并发处理。",
      "实践例子：下面用两个会话观察主键行锁、等待链和诊断表。\n\n```sql\nCREATE TABLE orders (\n  id BIGINT PRIMARY KEY,\n  user_id BIGINT NOT NULL,\n  status VARCHAR(32) NOT NULL,\n  updated_at DATETIME NOT NULL,\n  KEY idx_user_status (user_id, status)\n) ENGINE=InnoDB;\n\nINSERT INTO orders VALUES\n  (1001, 7, 'created', NOW()),\n  (1002, 7, 'created', NOW());\n\n-- 会话 A：持有 id=1001 的排他行锁\nSTART TRANSACTION;\nUPDATE orders\nSET status = 'paid', updated_at = NOW()\nWHERE id = 1001 AND status = 'created';\n\n-- 会话 B：同一记录更新会等待\nSTART TRANSACTION;\nUPDATE orders\nSET status = 'cancelled', updated_at = NOW()\nWHERE id = 1001 AND status = 'created';\n\n-- 排查锁持有和等待\nSELECT trx_id, trx_state, trx_started, trx_mysql_thread_id,\n       trx_query, trx_rows_locked, trx_rows_modified\nFROM information_schema.innodb_trx\\G\n\nSELECT ENGINE_TRANSACTION_ID, OBJECT_SCHEMA, OBJECT_NAME,\n       INDEX_NAME, LOCK_TYPE, LOCK_MODE, LOCK_STATUS, LOCK_DATA\nFROM performance_schema.data_locks\nWHERE OBJECT_NAME = 'orders'\\G\n\nSELECT *\nFROM performance_schema.data_lock_waits\\G\n```\n\n如果把条件换成 `WHERE user_id = 7 AND status = 'created' FOR UPDATE`，锁范围跟随 `idx_user_status` 的扫描范围；候选订单越多，持锁记录越多，任务领取吞吐会更依赖批量大小和提交速度。",
      "深层细节：行锁的真实成本由访问路径、隔离级别和事务边界共同决定。\n\n- 锁对象：InnoDB 行锁实际是索引记录锁；隐藏聚簇索引也可以承载记录锁。\n- 锁模式：`S` 锁允许并发共享读取，`X` 锁串行化修改；表级意向锁帮助表锁和行锁快速判断兼容性。\n- 锁范围：完整唯一索引等值命中通常只锁目标记录；范围查询、非唯一索引和部分联合索引会引入更多记录锁或 Next-Key Lock。\n- 隔离级别：`REPEATABLE READ` 下范围当前读更常见 Next-Key Lock；`READ COMMITTED` 下搜索和索引扫描中的间隙锁范围收缩，外键检查和重复键检查仍会触发必要间隙锁。\n- 二级索引：锁定读命中二级索引后，还可能在聚簇索引记录上加锁；返回列、回表和更新列会影响实际路径。\n- 长事务：持锁时间、Undo 保留、DDL 等待、复制延迟和连接池占用会一起放大。\n- `SKIP LOCKED`：适合任务队列表的多消费者领取，结果集会跳过被锁行，业务需要补偿重扫和公平性控制。\n- 死锁检测：访问顺序交叉、批量更新顺序漂移、二级索引与主键锁顺序交错，都可能形成等待环。",
      "边界与故障模式：线上行锁问题通常表现为延迟尖刺、线程堆积、锁等待超时、死锁回滚和发布卡顿。\n\n- 热点行：同一账户、同一库存、同一订单状态被大量请求更新，接口 p99 跟随锁等待上升。\n- 扫描放大：`WHERE` 条件缺少合适索引，执行计划扫描大量记录，行锁影响面随扫描行数扩大。\n- 批量写：大事务一次更新很多行，持锁时间长，回滚成本和复制延迟也变大。\n- 状态队列：多个消费者用 `FOR UPDATE` 抢同一批 `pending` 行，需要索引覆盖状态和排序字段。\n- 外键与唯一约束：父子表检查和重复键检测会带来额外锁等待证据。\n- 应用超时：数据库仍在事务中等待或执行，应用侧超时后连接归还策略决定后续污染风险。\n- DDL 卡顿：长事务持有表访问相关元数据锁，在线 DDL 等待兼容窗口。",
      "排查实践：处理行锁故障时，目标是把“谁持锁、谁等待、锁在哪个索引、SQL 走什么路径、业务是否允许重试”串起来。\n\n1. 先看等待事务：`information_schema.innodb_trx` 里的 `trx_state`、`trx_started`、`trx_query`、`trx_rows_locked`。\n2. 再看锁对象：`performance_schema.data_locks` 里的 `OBJECT_NAME`、`INDEX_NAME`、`LOCK_MODE`、`LOCK_STATUS`、`LOCK_DATA`。\n3. 连接等待边：`performance_schema.data_lock_waits` 能把阻塞事务和等待事务连成图。\n4. 回到 SQL：用 `EXPLAIN ANALYZE` 或慢查询日志确认访问类型、索引、扫描行数和过滤比例。\n5. 处理业务：缩短事务、补精准索引、统一资源访问顺序、拆小批量、把重试设计成幂等。\n6. 复盘参数：检查 `innodb_lock_wait_timeout`、事务隔离级别、连接池超时、应用重试间隔和 DDL 发布窗口。\n\n常用辅助命令：\n\n```sql\nSHOW VARIABLES LIKE 'transaction_isolation';\nSHOW VARIABLES LIKE 'innodb_lock_wait_timeout';\nSHOW ENGINE INNODB STATUS\\G\nEXPLAIN ANALYZE\nSELECT * FROM orders WHERE user_id = 7 AND status = 'created' FOR UPDATE;\n```",
      "工程取舍：行锁把一致性和并发度放到同一个设计问题里。\n\n- 悲观并发：`SELECT ... FOR UPDATE` 直接保护后续修改，适合库存、余额、任务领取等冲突高且错误代价高的场景。\n- 乐观并发：`UPDATE ... WHERE id = ? AND version = ?` 用受影响行数判断竞争，适合冲突较低和重试成本可控的场景。\n- 索引设计：为高频锁定读建立“业务过滤列 + 排序列 + 主键”的稳定路径，可以减少扫描和等待。\n- 事务粒度：把外部 RPC、缓存刷新、消息发送放到事务边界外，通过 Outbox、幂等键和补偿任务连接副作用。\n- 批量策略：大批量更新按主键有序分片提交，降低单次持锁窗口和死锁概率。\n- 队列消费：`FOR UPDATE SKIP LOCKED` 提升多消费者并发，监控跳过行重扫、任务饥饿和重复领取保护。",
      "常见误区：行锁的正确判断来自执行计划和事务语义。\n\n- 行锁关注索引记录，业务行只是最终表现。\n- 锁等待代表并发控制正在生效，故障判断要结合等待时长、连接池容量和业务超时。\n- 精准主键更新和低选择性范围更新的锁影响差异很大。\n- `READ COMMITTED`、`REPEATABLE READ` 会改变范围锁行为，业务语义和排查证据要一起看。\n- 死锁是并发访问顺序形成等待环，应用侧幂等重试是常规工程处理。\n- `SKIP LOCKED` 是队列消费工具，结果跳过已锁行，配套重扫和监控很关键。",
      "面试追问：行锁题适合按“定义 -> 触发语句 -> 索引路径 -> 隔离级别 -> 排查证据 -> 设计取舍”回答。\n\n- InnoDB 行锁为什么说加在索引记录上？\n- `UPDATE`、`DELETE`、`SELECT ... FOR UPDATE`、`SELECT ... FOR SHARE` 分别触发什么锁语义？\n- 主键等值、非唯一索引等值、范围查询的锁范围有什么差异？\n- 二级索引锁定读为什么可能同时涉及二级索引和聚簇索引？\n- `READ COMMITTED` 和 `REPEATABLE READ` 对行锁、间隙锁、Next-Key Lock 有什么影响？\n- 缺少索引的更新为什么会放大锁等待？\n- 线上出现 `Lock wait timeout exceeded` 时如何定位持锁事务和索引位置？\n- 死锁和锁等待超时在处理策略上有什么差异？\n- 库存扣减、账户余额、任务队列分别适合悲观锁还是乐观锁？\n- `NOWAIT` 和 `SKIP LOCKED` 的业务边界是什么？",
      "参考来源：本文主要参考 MySQL 8.4 Reference Manual 的 InnoDB Locking、Locking Reads、Locks Set by Different SQL Statements、Transaction Isolation Levels、Performance Schema Lock Tables，以及小林 coding 的 MySQL 加锁实验、JavaGuide 的 MVCC 讲解和 OneUptime 的 MySQL Locks 实践文章。官方资料用于校准锁类型、语句语义和观测表字段，中文资料用于补充案例、面试问法和工程表达。",
    ],
    typicalProblems: [
      "InnoDB 行锁是什么，为什么它实际锁定索引记录？",
      "哪些 SQL 会触发行锁，`FOR UPDATE` 和 `FOR SHARE` 的差异是什么？",
      "主键等值、二级索引等值、范围查询分别会产生怎样的锁范围？",
      "为什么缺少合适索引会放大行锁等待和死锁概率？",
      "如何用 `innodb_trx`、`data_locks`、`data_lock_waits` 定位持锁事务、等待事务和锁所在索引？",
      "`READ COMMITTED` 与 `REPEATABLE READ` 对行锁、间隙锁、Next-Key Lock 有哪些影响？",
      "库存扣减、订单状态流转、任务队列领取如何选择悲观锁、乐观锁、`NOWAIT` 或 `SKIP LOCKED`？",
      "线上出现锁等待超时、死锁回滚和长事务时，应用重试、索引优化和事务拆分怎样配合？",
    ],
    useCases: ["高并发更新", "库存扣减", "订单状态流转", "任务队列领取", "锁等待排查", "死锁治理"],
    prerequisites: ["lock", "mysql-index", "transaction", "isolation-level"],
    related: ["record-lock", "gap-lock", "next-key-lock", "deadlock", "mvcc", "repeatable-read", "sql-optimization"],
    order: 51,
  },
  /* <!-- KG_REVIEWED: 记录锁 | 2026-06-05 | source_count=11 --> */
  {
    sourceRefs: [
      "mysql-innodb-locking",
      "mysql-innodb-locks-set",
    ...
    area: "lock",
    difficulty: "medium",
    concept: "记录锁是 InnoDB 加在单个索引记录上的行级锁，用于串行化同一记录的锁定读、更新和删除。",
    explanation: [
      "概念定位：记录锁（Record Lock）解决的是“多个事务同时读写同一条业务记录时，谁先修改、谁等待、谁能继续读”的问题。它常出现在订单状态流转、库存扣减、账户余额更新、任务领取、唯一键保护、锁等待排查和 MySQL 锁面试题中。\n\n新手可以先把记录锁理解成“给 B+ 树叶子节点里的一个索引条目上锁”；有经验的工程师继续关注访问路径、二级索引回表、隔离级别、长事务、死锁和 `performance_schema.data_locks` 中的证据。",
      "准确定义：InnoDB 的记录锁是加在索引记录上的锁。表没有显式索引时，InnoDB 会创建隐藏聚簇索引用于组织数据和承载记录锁。\n\n关键术语如下：\n\n- `Record Lock`：锁住某个索引记录，例如主键 `id=1001` 对应的叶子条目。\n- `S lock`：共享记录锁，服务 `SELECT ... FOR SHARE` 这类共享锁定读。\n- `X lock`：排他记录锁，服务 `UPDATE`、`DELETE`、`SELECT ... FOR UPDATE` 这类修改或修改前保留。\n- `REC_NOT_GAP`：Performance Schema 和 InnoDB 状态里常见的锁模式标记，表示锁作用在记录本身。\n- `Gap Lock`：锁住两个索引记录之间的间隙，目标是控制插入位置。\n- `Next-Key Lock`：记录锁加前置间隙锁，常用于范围当前读。\n\n记录锁的核心边界是“索引记录本身”。唯一索引等值命中已存在记录时，InnoDB 通常只需要记录锁；范围条件、普通二级索引和缺索引扫描会把问题带到间隙锁或 Next-Key Lock。",
      "心智模型：把 InnoDB 表想成一组按索引排序的 B+ 树。\n\n- 主键聚簇索引叶子节点保存整行数据。\n- 二级索引叶子节点保存二级索引列和主键值。\n- 记录锁锁住的是某棵索引树上的某个叶子条目。\n- 通过二级索引锁定并修改行时，事务可能先锁二级索引记录，再锁对应的聚簇索引记录。\n- 锁等待的真实范围由优化器选中的索引扫描路径决定。\n\n这个模型能解释线上常见现象：SQL 看起来只改一行，执行计划扫过更多索引记录时，等待链和死锁概率会随扫描路径一起扩大。",
      "主流程机制：一次典型 `UPDATE accounts SET balance = balance - 100 WHERE id = 42 AND balance >= 100` 可以按下面步骤理解。\n\n1. 应用开启事务，连接继承当前隔离级别和 `autocommit` 设置。\n2. SQL 优化器选择访问路径，例如主键 `PRIMARY` 上的等值查找。\n3. InnoDB 进入当前读路径，读取最新可锁版本，并检查 `WHERE` 条件。\n4. 存储引擎在表上设置意向排他锁 `IX`，在主键索引记录 `id=42` 上申请 `X, REC_NOT_GAP` 记录锁。\n5. 锁兼容时语句继续执行；存在冲突 `S` 或 `X` 锁时，事务进入锁等待。\n6. 获得锁后，InnoDB 生成 Undo 旧版本，修改 Buffer Pool 中的数据页，并写入 Redo。\n7. 事务提交或回滚后释放记录锁，等待事务被唤醒并重新检查可执行条件。\n8. 慢日志、应用 Trace、`innodb_trx`、`data_locks`、`data_lock_waits` 和 InnoDB 状态共同留下排查证据。\n\n记录锁保证同一索引记录上的冲突修改按事务顺序串行化，同时让无关索引记录继续并发处理。",
      "场景示例：下面的最小实验可以观察主键等值命中已存在记录时的记录锁。\n\n```sql\nCREATE TABLE account (\n  id BIGINT PRIMARY KEY,\n  balance DECIMAL(10,2) NOT NULL,\n  status VARCHAR(16) NOT NULL,\n  KEY idx_status (status)\n) ENGINE=InnoDB;\n\nINSERT INTO account VALUES (42, 500.00, 'normal');\n\n-- 会话 A\nSTART TRANSACTION;\nSELECT * FROM account WHERE id = 42 FOR UPDATE;\n\n-- 会话 B：等待会话 A 提交或回滚\nUPDATE account SET balance = balance - 100 WHERE id = 42;\n```\n\n观测锁类型时重点看 `LOCK_TYPE`、`LOCK_MODE`、`INDEX_NAME`、`LOCK_STATUS` 和 `LOCK_DATA`：\n\n```sql\nSELECT\n  ENGINE_TRANSACTION_ID,\n  OBJECT_SCHEMA,\n  OBJECT_NAME,\n  INDEX_NAME,\n  LOCK_TYPE,\n  LOCK_MODE,\n  LOCK_STATUS,\n  LOCK_DATA\nFROM performance_schema.data_locks\nWHERE OBJECT_NAME = 'account'\\G\n```\n\n主键等值命中时，常见证据是 `INDEX_NAME='PRIMARY'`、`LOCK_TYPE='RECORD'`、`LOCK_MODE` 包含 `X,REC_NOT_GAP` 或同等记录锁语义。",
      "深层细节：记录锁的难点集中在“锁哪棵索引树、锁几个条目、锁多久”。\n\n- 唯一索引等值命中：完整唯一键命中已存在记录时，记录锁就能保护这条记录的更新、删除和锁定读。\n- 唯一索引等值缺失：查找的唯一值尚未存在时，InnoDB 需要保护插入位置，问题会进入间隙锁语义。\n- 普通二级索引：`status='pending'` 可能命中多条二级索引记录；修改行时还会触达对应主键聚簇索引记录。\n- 二级唯一索引：二级索引记录和聚簇索引记录都可能留下锁证据，排查时要同时看 `INDEX_NAME` 和 `LOCK_DATA`。\n- 范围访问：`>`, `<`, `BETWEEN`, 前缀范围和非唯一条件会把记录锁扩展到范围锁、间隙锁或 Next-Key Lock。\n- 隔离级别：`READ COMMITTED` 下搜索和索引扫描中的间隙锁范围通常收缩；外键检查和重复键检查仍会触发必要锁。\n- 复杂执行：`UNION`、临时表、中间结果和回表会影响锁释放时机，排查时以实际执行计划和锁表为准。\n- 长事务：记录锁本身很细，持锁时间被业务代码、RPC、批处理、慢 SQL、连接池占用和人工事务窗口放大。",
      "边界与故障模式：线上记录锁问题通常表现为等待、超时、死锁和吞吐下降。\n\n- 热点行争用：库存、余额、计数器和全局配置行被大量事务更新，等待集中在同一个 `LOCK_DATA`。\n- 扫描放大：缺少合适索引时，`UPDATE` 或 `DELETE` 扫过大量记录，锁对象数量和等待方一起增加。\n- 二级索引死锁：两个事务以不同顺序访问二级索引和主键记录，容易形成等待环。\n- 条件漂移：执行计划从主键等值变成普通索引或全表扫描后，锁影响面突然扩大。\n- 长事务持锁：应用在事务内做外部调用、分页批处理或人工确认，锁等待扩散到连接池和上游超时。\n- 重试风暴：锁等待超时或死锁回滚后，应用无退避重试会继续冲击同一热点记录。\n- 读写口径混用：普通一致性读看到快照版本，锁定读读取最新可锁版本；同一事务内混用时要明确业务语义。",
      "排查实践：定位记录锁等待时，按“谁在等、谁持锁、锁在哪个索引记录、SQL 为什么扫到这里”推进。\n\n1. 找到当前活跃事务和运行时长。\n2. 查看数据锁和等待关系。\n3. 结合执行计划确认索引路径。\n4. 从业务链路收敛事务边界和重试策略。\n5. 用短事务、精准索引、固定访问顺序和幂等重试修复。\n\n```sql\nSELECT\n  trx_id,\n  trx_state,\n  trx_started,\n  trx_mysql_thread_id,\n  trx_query\nFROM information_schema.innodb_trx\nORDER BY trx_started;\n\nSELECT\n  r.ENGINE_TRANSACTION_ID AS waiting_trx,\n  b.ENGINE_TRANSACTION_ID AS blocking_trx,\n  r.OBJECT_SCHEMA,\n  r.OBJECT_NAME,\n  r.INDEX_NAME,\n  r.LOCK_MODE AS waiting_mode,\n  b.LOCK_MODE AS blocking_mode,\n  r.LOCK_DATA\nFROM performance_schema.data_lock_waits w\nJOIN performance_schema.data_locks r\n  ON w.REQUESTING_ENGINE_LOCK_ID = r.ENGINE_LOCK_ID\nJOIN performance_schema.data_locks b\n  ON w.BLOCKING_ENGINE_LOCK_ID = b.ENGINE_LOCK_ID;\n\nEXPLAIN ANALYZE\nUPDATE account SET balance = balance - 100 WHERE id = 42 AND balance >= 100;\n\nSHOW ENGINE INNODB STATUS\\G\n```\n\n证据判断要点：`INDEX_NAME` 说明锁落在哪棵索引树，`LOCK_MODE` 说明记录、间隙或 Next-Key 语义，`LOCK_STATUS='WAITING'` 指出等待方，`trx_started` 和应用 Trace 说明持锁时间来源。",
      "工程取舍：记录锁是悲观并发控制工具，适合“先锁定再修改”的强一致写场景。\n\n- 账户扣款、库存扣减、订单状态机适合用短事务加记录锁保护状态迁移。\n- 高吞吐计数器适合拆分热点、批量汇总或改用原子条件更新降低单行争用。\n- 任务队列可用 `FOR UPDATE SKIP LOCKED` 提升多消费者并发，业务要设计补扫和幂等。\n- 用户编辑资料这类低冲突场景可以用版本号乐观锁，把冲突处理放到应用层。\n- 大批量修复建议按主键有序分批提交，让锁集合和 Undo 压力保持可控。\n\n记录锁的目标是保护确定的索引记录；工程目标是让事务短、索引准、访问顺序稳定、失败可重试。",
      "常见误区：判断记录锁要从 SQL 文本走向实际执行证据。\n\n- 行级锁的实体位置在索引记录上，业务表的一行可能对应主键记录和多个二级索引记录。\n- `LOCK_TYPE='RECORD'` 表示行级数据锁类别，具体语义继续看 `LOCK_MODE` 是否带 `REC_NOT_GAP`、`GAP` 或普通 `X`。\n- 主键等值命中已存在记录时，锁范围通常最精准；普通索引和范围条件带来更宽的锁集合。\n- 普通 `SELECT` 主要走一致性读；`FOR UPDATE`、`FOR SHARE`、`UPDATE`、`DELETE` 进入锁定读或当前读路径。\n- 锁等待本身是并发控制成本；慢事务、差索引和无退避重试会把成本放大成故障。",
      "面试追问：记录锁题适合按“定义 -> 索引位置 -> SQL 触发 -> 访问路径 -> 排查证据 -> 工程取舍”组织答案。\n\n- 记录锁为什么说锁在索引记录上？\n- `S` 记录锁和 `X` 记录锁的兼容关系是什么？\n- 主键等值命中、唯一键缺失、普通索引范围扫描分别会产生哪些锁语义？\n- `SELECT ... FOR UPDATE` 和 `UPDATE` 在加锁路径上有什么共同点？\n- 二级索引锁定读为什么可能同时出现二级索引和主键索引锁？\n- `LOCK_MODE='X,REC_NOT_GAP'`、`X,GAP`、`X` 分别说明什么？\n- 线上出现 `Lock wait timeout exceeded` 时如何定位阻塞事务和锁所在索引？\n- 记录锁等待和死锁如何通过索引、访问顺序、事务边界和重试策略治理？",
      "参考来源：本文主要参考 MySQL 8.4 Reference Manual 的 InnoDB Locking、Locks Set by Different SQL Statements、Locking Reads、Performance Schema Lock Tables、INNODB_TRX、Transaction Isolation Levels、Deadlocks 文档，并结合小林 coding 的 MySQL 加锁实验和 JavaGuide 的 MVCC 讲解校准中文表达、锁表字段和面试语境。",
    ],
    typicalProblems: [
      "InnoDB 记录锁是什么，为什么它锁住的是索引记录？",
      "记录锁、间隙锁和 Next-Key Lock 的边界分别是什么？",
      "`SELECT ... FOR UPDATE`、`SELECT ... FOR SHARE`、`UPDATE`、`DELETE` 分别会触发怎样的记录锁语义？",
      "主键等值命中、唯一索引缺失、普通二级索引等值和范围扫描的锁范围如何变化？",
      "二级索引锁定读为什么可能同时锁二级索引记录和聚簇索引记录？",
      "如何用 `information_schema.innodb_trx`、`performance_schema.data_locks`、`data_lock_waits` 和 `SHOW ENGINE INNODB STATUS` 排查记录锁等待？",
      "热点行、长事务、缺索引和访问顺序交叉分别怎样导致锁等待或死锁？",
      "库存扣减、账户余额、订单状态流转、任务队列领取如何选择记录锁、乐观锁、`NOWAIT` 或 `SKIP LOCKED`？",
      "`READ COMMITTED` 和 `REPEATABLE READ` 对记录锁、间隙锁、范围锁的影响如何回答？",
      "线上修复记录锁争用时，索引优化、事务拆分、固定访问顺序和退避重试如何配合？",
    ],
    useCases: ["精确条件更新", "唯一键更新", "库存扣减", "账户余额更新", "订单状态流转", "任务队列领取", "锁等待排查", "死锁治理"],
    prerequisites: ["row-lock", "mysql-index", "transaction", "isolation-level"],
    related: ["row-lock", "gap-lock", "next-key-lock", "deadlock", "repeatable-read", "read-committed", "mvcc", "sql-optimization", "performance-schema"],
    order: 52,
  },
  /* <!-- KG_REVIEWED: 间隙锁 | 2026-06-05 | source_count=11 --> */
  {
    sourceRefs: [
      "mysql-innodb-locking",
      "mysql-innodb-locks-set",
    ...
    area: "lock",
    difficulty: "hard",
    concept: "间隙锁是 InnoDB 加在索引记录之间空隙上的锁，用于控制其他事务向指定索引范围插入新记录。",
    explanation: [
      "概念定位：间隙锁（Gap Lock）解决的是“事务正在检查或锁定一个索引范围时，其他事务能否向这个范围插入新记录”的问题。它常出现在范围更新、唯一键插入冲突、订单号区间保护、任务队列抢占、幻读分析、死锁日志和 MySQL 锁面试中。\n\n新手可以把间隙锁理解成“给 B+ 树叶子节点之间的空位上锁”；有经验的工程师继续关注访问路径、隔离级别、`LOCK_MODE` 证据、插入意向锁、范围扫描放大、长事务和业务吞吐取舍。",
      "准确定义：InnoDB 的间隙锁锁住索引记录之间的 gap，目标是控制其他事务向该 gap 插入索引记录。它是纯粹的范围位置锁，通常服务范围当前读、Next-Key Lock、唯一性检查、外键检查和幻读防护。\n\n关键术语如下：\n\n- `Gap Lock`：锁住两个索引记录之间的空隙，也包括第一条记录之前和最后一条记录之后的范围。\n- `Record Lock`：锁住一个已存在索引记录本身。\n- `Next-Key Lock`：记录锁加前置间隙锁，覆盖“某条记录以及它前面的 gap”。\n- `Insert Intention Lock`：事务准备向某个 gap 插入时设置的插入意向锁。\n- `LOCK_MODE` 包含 `GAP`：Performance Schema 中识别 gap 语义的重要证据。\n- `supremum pseudo-record`：InnoDB 用于表示索引末尾上界的伪记录，尾部 gap 锁常与它一起出现。\n\n间隙锁的判断边界是“插入位置”。已有记录的冲突修改由记录锁处理；新记录插入某个索引区间时，间隙锁决定它是否需要等待。",
      "心智模型：把一棵索引 B+ 树叶子层想成有序刻度尺。\n\n```text\n索引值:       10        20        30\n范围:    (-inf,10) (10,20) (20,30) (30,+inf)\n锁对象:     gap      gap      gap      gap\n```\n\n- 记录锁锁住 `10`、`20`、`30` 这些刻度点。\n- 间隙锁锁住刻度点之间的空白区间。\n- `Next-Key Lock` 常表现为“前一个 gap + 当前记录”。\n- 插入 `25` 时，事务要进入 `(20,30)` 这个 gap。\n- 执行计划选择哪棵索引，间隙锁就落在哪棵索引的有序空间。\n\n这个模型能解释线上现象：范围条件、普通二级索引、唯一键缺失和执行计划扫描会把锁落到索引空隙上，锁范围以实际索引路径为准。",
      "主流程机制：一次 `REPEATABLE READ` 下的范围锁定读可以按下面步骤理解。\n\n1. 应用开启事务，当前隔离级别为 `REPEATABLE READ`。\n2. SQL 使用锁定读或写语句，例如 `SELECT ... FOR UPDATE`、`UPDATE` 或 `DELETE`。\n3. 优化器选择访问路径，例如二级索引 `idx_amount` 上的 `amount BETWEEN 100 AND 200`。\n4. InnoDB 沿索引范围扫描，读取最新可锁版本并检查条件。\n5. 对命中的索引记录申请记录锁，同时对相关索引间隙申请 gap 语义锁，组合场景表现为 Next-Key Lock。\n6. 其他事务更新已有记录时按记录锁兼容性判断；向受保护 gap 插入新索引值时需要等待。\n7. 当前事务提交或回滚后释放锁，等待插入方继续执行并重新检查唯一性、外键或业务条件。\n8. `data_locks`、`data_lock_waits`、`innodb_trx`、死锁日志和执行计划提供可验证证据。\n\n间隙锁把“这个范围此刻没有满足条件的新行”变成可保护事实，帮助当前读和范围写避免新插入破坏判断。",
      "场景示例：下面实验展示范围锁定读如何阻塞插入位置。\n\n```sql\nCREATE TABLE ticket (\n  id BIGINT PRIMARY KEY,\n  seat_no INT NOT NULL,\n  status VARCHAR(16) NOT NULL,\n  KEY idx_seat_no (seat_no)\n) ENGINE=InnoDB;\n\nINSERT INTO ticket VALUES\n  (1, 10, 'free'),\n  (2, 20, 'free'),\n  (3, 30, 'free');\n\n-- 会话 A\nSET TRANSACTION ISOLATION LEVEL REPEATABLE READ;\nSTART TRANSACTION;\nSELECT * FROM ticket WHERE seat_no BETWEEN 10 AND 20 FOR UPDATE;\n\n-- 会话 B：插入 seat_no=15 进入 (10,20) gap，通常会等待会话 A\nINSERT INTO ticket VALUES (4, 15, 'free');\n```\n\n排查等待时看锁对象和等待关系：\n\n```sql\nSELECT\n  ENGINE_TRANSACTION_ID,\n  OBJECT_SCHEMA,\n  OBJECT_NAME,\n  INDEX_NAME,\n  LOCK_TYPE,\n  LOCK_MODE,\n  LOCK_STATUS,\n  LOCK_DATA\nFROM performance_schema.data_locks\nWHERE OBJECT_NAME = 'ticket'\nORDER BY ENGINE_TRANSACTION_ID, INDEX_NAME, LOCK_DATA;\n\nSELECT\n  r.ENGINE_TRANSACTION_ID AS waiting_trx,\n  b.ENGINE_TRANSACTION_ID AS blocking_trx,\n  r.INDEX_NAME,\n  r.LOCK_MODE AS waiting_mode,\n  b.LOCK_MODE AS blocking_mode,\n  r.LOCK_DATA\nFROM performance_schema.data_lock_waits w\nJOIN performance_schema.data_locks r\n  ON w.REQUESTING_ENGINE_LOCK_ID = r.ENGINE_LOCK_ID\nJOIN performance_schema.data_locks b\n  ON w.BLOCKING_ENGINE_LOCK_ID = b.ENGINE_LOCK_ID;\n```\n\n常见证据是 `INDEX_NAME='idx_seat_no'`，`LOCK_MODE` 包含 `GAP`、`INSERT_INTENTION` 或普通 `X` 的 Next-Key 语义，`LOCK_DATA` 指向相邻索引记录或 `supremum pseudo-record`。",
      "深层细节：间隙锁的复杂度来自“范围怎么被优化器解释”。\n\n- 唯一索引完整等值命中已有记录时，InnoDB 通常只需要记录锁。\n- 唯一索引等值查找缺失值时，InnoDB 需要保护目标插入位置，可能出现 gap 语义。\n- 普通二级索引等值条件会形成一段相同键值范围，还要保护相邻 gap。\n- 范围查询会沿索引扫描多个区间，命中的记录和相邻空隙都可能被锁住。\n- 尾部范围会触达 `supremum pseudo-record`，表现为索引末尾之后的插入被等待。\n- 缺少合适索引时，扫描路径变宽，锁住的索引位置和等待影响面一起扩大。\n- 外键检查和重复键检查需要保护参照或唯一性判断，即使在较低隔离级别也会保留必要锁。\n- `READ COMMITTED` 下 InnoDB 对搜索和索引扫描中的 gap 锁使用更少，但约束检查相关锁仍然存在。\n\n因此，回答“会锁多大范围”时要同时给出隔离级别、SQL 类型、执行计划、索引唯一性和命中情况。",
      "边界与故障模式：生产中的间隙锁问题通常表现为插入卡住、死锁、批量任务变慢和连接池耗尽。\n\n- 范围持锁过久：报表修复、批量更新或人工事务窗口持有范围锁，插入请求排队。\n- 普通索引锁扩大：`WHERE status='pending'` 这类低选择性条件命中大量相同索引值，插入和更新互相影响。\n- 尾部插入阻塞：范围条件扫到索引末尾，后续递增值插入集中等待 `supremum` 附近。\n- 插入意向等待：多个插入方彼此可以并行进入不同位置，但遇到范围 gap 锁会一起等待。\n- 死锁回滚：两个事务以不同顺序锁范围和插入位置，等待环触发 InnoDB 死锁检测。\n- 计划漂移：统计信息变化后执行计划换索引，原本精准的锁范围变成宽范围。\n- 重试放大：应用对锁等待超时做立即重试，会继续冲击同一 gap。",
      "排查实践：定位间隙锁问题时，按“等待插入点 -> 持锁范围 -> 执行计划 -> 事务来源 -> 修复策略”推进。\n\n1. 确认等待现象：应用错误通常是 `Lock wait timeout exceeded`、死锁回滚、接口 p99 突增或连接池耗尽。\n2. 找等待关系：查询 `performance_schema.data_lock_waits`，拿到等待事务和阻塞事务。\n3. 看锁模式：在 `data_locks` 中确认 `LOCK_MODE` 是否包含 `GAP`、`INSERT_INTENTION`，并记录 `INDEX_NAME`、`LOCK_DATA`。\n4. 查活跃事务：用 `information_schema.innodb_trx` 找到事务开始时间、线程 ID 和当前 SQL。\n5. 复盘访问路径：对阻塞 SQL 与等待 SQL 分别执行 `EXPLAIN ANALYZE` 或 `EXPLAIN FORMAT=TREE`，确认索引、范围和扫描行数。\n6. 收敛业务边界：检查事务内外部调用、批处理大小、人工操作、重试策略和隔离级别配置。\n7. 落地修复：补更精准的联合索引、改主键有序小批量提交、缩短事务、固定访问顺序，必要时评估 `READ COMMITTED`。\n\n```sql\nSELECT\n  trx_id,\n  trx_state,\n  trx_started,\n  trx_mysql_thread_id,\n  trx_query\nFROM information_schema.innodb_trx\nORDER BY trx_started;\n\nSHOW ENGINE INNODB STATUS\\G\n\nEXPLAIN ANALYZE\nSELECT * FROM ticket WHERE seat_no BETWEEN 10 AND 20 FOR UPDATE;\n```\n\n有效结论应包含：阻塞事务 ID、等待事务 ID、锁所在索引、相邻索引值、SQL 扫描范围、持锁时长来源和修复后验证指标。",
      "工程取舍：间隙锁用并发吞吐换范围一致性，适合需要保护“范围内当前没有新记录插入”的业务判断。\n\n- 防止重复预订、区间资源分配、唯一候选号抢占和范围状态迁移，适合用锁定读配合短事务。\n- 高并发追加写、消息落库和日志流水对插入吞吐敏感，应让写入路径避开被长范围事务覆盖的索引区间。\n- 批量修复适合按主键或高选择性索引分段，每段快速提交，降低 gap 覆盖时间。\n- 低选择性普通索引适合补充更具体的联合索引，让范围和 gap 更贴近业务对象。\n- `READ COMMITTED` 能减少搜索扫描中的 gap 锁影响，业务要同时确认幻读容忍度、复制语义、约束检查和历史行为变化。\n- `NOWAIT`、`SKIP LOCKED` 适合任务领取等场景，业务要处理跳过、补扫和幂等。",
      "常见误区：间隙锁要从索引空间和插入位置理解。\n\n- 间隙锁保护的是索引空隙，核心影响是插入新索引记录。\n- `LOCK_TYPE='RECORD'` 仍可能带 gap 语义，判断时继续看 `LOCK_MODE`。\n- 锁范围由实际执行计划决定，SQL 条件、索引设计和统计信息共同影响它。\n- `REPEATABLE READ` 下范围当前读常见 Next-Key Lock，间隙锁是其中的范围部分。\n- 已有记录的修改冲突主要看记录锁，新记录插入等待主要看 gap 和插入意向锁。\n- 隔离级别调整属于业务一致性决策，需要配合测试、回归和故障预案。",
      "面试追问：间隙锁题适合按“定义 -> 索引范围 -> 触发 SQL -> 与幻读关系 -> 排查证据 -> 工程取舍”组织答案。\n\n- InnoDB 间隙锁是什么，为什么它锁的是索引记录之间的范围？\n- 间隙锁、记录锁、Next-Key Lock 和插入意向锁分别解决什么问题？\n- 主键等值命中、唯一键缺失、普通索引等值和范围查询分别会产生怎样的 gap 语义？\n- `REPEATABLE READ` 与 `READ COMMITTED` 对 gap 锁行为有什么影响？\n- 间隙锁如何帮助当前读处理幻读？\n- 为什么缺少合适索引会放大间隙锁等待？\n- `LOCK_MODE` 中的 `GAP`、`INSERT_INTENTION`、`REC_NOT_GAP` 分别说明什么？\n- 线上插入语句卡住时，如何用 `data_locks`、`data_lock_waits`、`innodb_trx` 和执行计划定位原因？\n- 如何通过索引优化、事务拆分、访问顺序、隔离级别和重试退避治理间隙锁问题？",
      "参考来源：本文主要参考 MySQL 8.4 Reference Manual 的 InnoDB Locking、Locks Set by Different SQL Statements、Locking Reads、Phantom Rows、Performance Schema Lock Tables、INNODB_TRX、Transaction Isolation Levels、Deadlocks 文档，并结合小林 coding 的 MySQL 加锁实验校准中文示例、锁范围判断、排查字段和面试表达。官方资料用于定义、隔离级别、锁模式和命令语义，实践文章用于补足执行计划、范围案例和工程排障路径。",
    ],
    typicalProblems: [
      "InnoDB 间隙锁是什么，它为什么锁住索引记录之间的范围？",
      "间隙锁、记录锁、Next-Key Lock 和插入意向锁的边界分别是什么？",
      "`SELECT ... FOR UPDATE`、`UPDATE`、`DELETE`、唯一键缺失查找分别会怎样触发 gap 语义？",
      "`REPEATABLE READ` 和 `READ COMMITTED` 下间隙锁行为有哪些差异？",
      "间隙锁和幻读治理之间的关系如何结合当前读解释？",
      "普通二级索引、低选择性索引、范围查询和缺索引为什么会放大间隙锁影响面？",
      "如何用 `performance_schema.data_locks`、`data_lock_waits`、`information_schema.innodb_trx` 和 `SHOW ENGINE INNODB STATUS` 排查插入等待？",
      "`LOCK_MODE` 里的 `GAP`、`INSERT_INTENTION`、`REC_NOT_GAP` 和 `supremum pseudo-record` 分别代表什么证据？",
      "线上出现间隙锁等待或死锁时，索引优化、事务拆分、固定访问顺序和退避重试如何配合？",
      "什么时候可以评估切换到 `READ COMMITTED`，需要验证哪些一致性、复制和业务语义风险？",
    ],
    useCases: ["范围并发控制", "幻读治理", "唯一键缺失保护", "插入等待排查", "死锁治理", "区间资源分配", "批量更新治理", "MySQL 锁面试"],
    prerequisites: ["row-lock", "repeatable-read", "mysql-index", "transaction", "isolation-level"],
    related: ["record-lock", "next-key-lock", "phantom-read", "deadlock", "read-committed", "sql-optimization", "performance-schema"],
    order: 53,
  },
  /* <!-- KG_REVIEWED: Next-Key Lock | 2026-06-05 | source_count=11 --> */
  {
    sourceRefs: [
      "mysql-innodb-locking",
      "mysql-innodb-locks-set",
    ...
    area: "lock",
    difficulty: "hard",
    concept: "Next-Key Lock 是 InnoDB 把索引记录锁与该记录前方间隙锁组合起来的范围锁，用于保护当前读扫描范围并控制并发插入造成的幻读。",
    explanation: [
      "概念定位：Next-Key Lock 解决的是“事务按索引范围做当前读、锁定读或范围写入时，已有记录和相邻插入位置怎样一起受控”的问题。它常出现在 `REPEATABLE READ` 下的 `SELECT ... FOR UPDATE`、范围 `UPDATE`、范围 `DELETE`、幻读治理、插入等待、死锁日志、任务队列领取、区间资源分配和 MySQL 锁面试里。\n\n新手先抓住一个主线：Next-Key Lock 同时保护“记录点”和“记录前的空位”。经验工程师继续看实际执行计划、索引顺序、隔离级别、锁模式、插入意向锁、尾部 `supremum`、低选择性索引、长事务和重试策略。",
      "准确定义：Next-Key Lock 是 index-record lock 加 gap lock 的组合。对某个索引记录 `R` 来说，它锁住 `R` 这条索引记录，同时锁住索引顺序里紧挨着 `R` 前面的 gap。常见表达是 `(prev_key, current_key]`，其中右端点是当前索引记录，左侧开区间是前一条记录之后到当前记录之前的插入范围。\n\n关键术语如下：\n\n- `Record Lock`：锁住已存在的索引记录本身，控制已有行的更新、删除或锁定读冲突。\n- `Gap Lock`：锁住两个索引记录之间的空位，控制其他事务向该位置插入新索引记录。\n- `Next-Key Lock`：`Record Lock + 前置 Gap Lock`，覆盖一段前开后闭的索引区间。\n- `Insert Intention Lock`：插入方进入某个 gap 前设置的意向锁，遇到覆盖该 gap 的范围锁时会等待。\n- `supremum pseudo-record`：索引末尾上界伪记录，相关 Next-Key Lock 保护最大键之后的尾部范围。\n- `current read`：`UPDATE`、`DELETE`、`SELECT ... FOR UPDATE`、`SELECT ... FOR SHARE` 这类读取最新可锁版本并加锁的读写路径。\n\nNext-Key Lock 的判断单位是“实际扫描的索引区间”。SQL 文本表达业务范围，执行计划决定 InnoDB 沿哪棵索引扫描并设置锁。",
      "心智模型：把 InnoDB 索引叶子层看成有序路段，记录是路标，gap 是路标之间的车道。Next-Key Lock 像把“某个路标”和“它前面那段车道”一起封住。\n\n```text\n索引值:        10          20          30        supremum\n区间:     (-inf,10]   (10,20]    (20,30]    (30,+inf)\n锁形态:    next-key   next-key   next-key   tail next-key\n```\n\n- 锁住 `(10,20]` 时，其他事务修改 `20` 这条记录会冲突。\n- 插入 `15` 需要进入 `(10,20)`，会遇到同一把锁保护的 gap。\n- 插入 `25` 命中 `(20,30)`，它由另一段 Next-Key 范围控制。\n- 范围扫描经过多少索引记录，锁影响面就随扫描路径扩展。\n- 查询使用二级索引时，锁首先落在二级索引空间，必要时还会涉及聚簇索引记录。",
      "主流程机制：一次 `REPEATABLE READ` 下的范围当前读通常可以按下面步骤理解。\n\n1. 应用开启事务，会话隔离级别为 `REPEATABLE READ`，业务执行锁定读或写语句。\n2. 优化器选择访问路径，例如主键范围、普通二级索引范围、联合索引前缀或全索引扫描。\n3. InnoDB 沿选定索引读取最新可锁版本；普通一致性读走 ReadView，当前读走加锁路径。\n4. 对扫描遇到的索引记录设置共享或排他记录锁，并把记录前方 gap 一起纳入锁范围，形成 Next-Key Lock。\n5. 其他事务更新已锁记录时按记录锁兼容性等待；向受保护 gap 插入新记录时，插入意向锁进入等待。\n6. 二级索引上的排他锁定读可能继续回到聚簇索引记录设置锁，实际等待链会同时包含二级索引和主键记录。\n7. 事务提交或回滚后释放锁，等待方继续执行并重新检查唯一性、外键、条件过滤和业务约束。\n8. `performance_schema.data_locks`、`data_lock_waits`、`information_schema.innodb_trx`、执行计划和死锁日志共同提供证据。",
      "场景示例：下面用座位预订表复现范围锁定读保护“可预约区间”的过程。\n\n```sql\nCREATE TABLE seat_booking (\n  id BIGINT PRIMARY KEY,\n  room_id BIGINT NOT NULL,\n  seat_no INT NOT NULL,\n  status VARCHAR(16) NOT NULL,\n  KEY idx_room_seat (room_id, seat_no)\n) ENGINE=InnoDB;\n\nINSERT INTO seat_booking VALUES\n  (1, 101, 10, 'free'),\n  (2, 101, 20, 'free'),\n  (3, 101, 30, 'free');\n\n-- 会话 A：锁定 101 房间 10 到 20 号座位范围\nSET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ;\nSTART TRANSACTION;\nSELECT * FROM seat_booking\nWHERE room_id = 101 AND seat_no BETWEEN 10 AND 20\nFOR UPDATE;\n\n-- 会话 B：seat_no=15 落入 idx_room_seat 的 (10,20) gap，通常等待会话 A\nINSERT INTO seat_booking VALUES (4, 101, 15, 'free');\n```\n\n排查时先看锁表，再看执行计划：\n\n```sql\nSELECT\n  ENGINE_TRANSACTION_ID,\n  OBJECT_SCHEMA,\n  OBJECT_NAME,\n  INDEX_NAME,\n  LOCK_TYPE,\n  LOCK_MODE,\n  LOCK_STATUS,\n  LOCK_DATA\nFROM performance_schema.data_locks\nWHERE OBJECT_NAME = 'seat_booking'\nORDER BY ENGINE_TRANSACTION_ID, INDEX_NAME, LOCK_DATA;\n\nSELECT\n  w.REQUESTING_ENGINE_TRANSACTION_ID AS waiting_trx,\n  w.BLOCKING_ENGINE_TRANSACTION_ID AS blocking_trx,\n  r.INDEX_NAME,\n  r.LOCK_MODE AS waiting_mode,\n  b.LOCK_MODE AS blocking_mode,\n  r.LOCK_DATA\nFROM performance_schema.data_lock_waits w\nJOIN performance_schema.data_locks r\n  ON w.REQUESTING_ENGINE_LOCK_ID = r.ENGINE_LOCK_ID\nJOIN performance_schema.data_locks b\n  ON w.BLOCKING_ENGINE_LOCK_ID = b.ENGINE_LOCK_ID;\n\nEXPLAIN ANALYZE\nSELECT * FROM seat_booking\nWHERE room_id = 101 AND seat_no BETWEEN 10 AND 20\nFOR UPDATE;\n```\n\n重点证据是 `INDEX_NAME='idx_room_seat'`、`LOCK_DATA` 指向相邻索引值或 `supremum pseudo-record`，`LOCK_MODE` 中出现 `GAP`、`INSERT_INTENTION`、`REC_NOT_GAP` 等组合，以及阻塞事务的真实 SQL 和事务年龄。",
      "深层细节：Next-Key Lock 的复杂度主要来自索引路径和语句类型。\n\n- 完整唯一索引等值命中已有记录时，InnoDB 通常只需要锁定目标索引记录。\n- 范围条件、普通二级索引、部分联合唯一索引、等值命中一组重复键、唯一值缺位查找都会把 gap 纳入判断。\n- `UPDATE ... WHERE ...` 和 `DELETE ... WHERE ...` 对搜索遇到的记录设置排他 Next-Key Lock；完整唯一键命中场景范围更窄。\n- `SELECT ... FOR UPDATE` 和 `SELECT ... FOR SHARE` 属于锁定读，使用索引范围时会按扫描路径保留锁语义。\n- 普通 `INSERT` 插入成功后持有新记录的记录锁；插入前设置插入意向 gap 锁，遇到覆盖该 gap 的 Next-Key Lock 时等待。\n- `INSERT ... ON DUPLICATE KEY UPDATE`、`REPLACE` 和重复键检查可能触发不同的记录锁或 Next-Key 语义。\n- `INSERT ... SELECT`、`CREATE TABLE ... SELECT` 在特定隔离级别和复制语义下可能对来源表设置共享 Next-Key Lock。\n- `READ COMMITTED` 缩小搜索和索引扫描中的 gap 锁使用范围，外键检查与重复键检查仍保留必要锁。\n- 执行计划漂移会改变扫描索引，Next-Key Lock 的实际覆盖范围也随之变化。\n\n因此，判断“会锁住什么”时，要同时给出隔离级别、SQL 类型、索引唯一性、命中情况、执行计划和事务边界。",
      "边界与故障模式：生产里的 Next-Key Lock 问题通常表现为插入等待、死锁增加、批处理拖慢、连接池耗尽和业务重试风暴。\n\n- 范围过宽：条件缺少高选择性联合索引，当前读扫描大量索引记录，Next-Key 覆盖面扩大。\n- 尾部阻塞：范围条件扫到索引末尾，`supremum` 附近的递增值插入集中等待。\n- 低选择性状态列：`status='pending'` 这类索引值集中，任务领取和状态迁移容易锁住一段重复键范围。\n- 二级索引回表：二级索引范围锁和聚簇索引记录锁交织，等待链和死锁图更复杂。\n- 长事务持锁：事务里夹杂远程调用、人工确认或大批量处理，锁释放时间被业务流程拉长。\n- 访问顺序交叉：多个事务按不同顺序扫描重叠范围，记录锁、gap 和插入意向锁形成等待环。\n- 立即重试放大：应用收到锁等待超时或死锁回滚后立刻重放同一范围，加重热点 gap 争用。\n- 隔离级别差异：连接池会话状态、框架事务配置和只读事务设置会改变真实锁行为。",
      "排查实践：定位 Next-Key Lock 建议按“等待事实 -> 锁对象 -> 执行计划 -> 事务来源 -> 修复验证”推进。\n\n1. 固定时间窗：记录接口 p99、`Lock wait timeout exceeded`、`Deadlock found`、连接池等待、主机 I/O 和发布批处理时间。\n2. 找等待链：查询 `performance_schema.data_lock_waits`，拿到等待事务、阻塞事务和相关锁 ID。\n3. 看锁对象：在 `data_locks` 中记录 `OBJECT_NAME`、`INDEX_NAME`、`LOCK_TYPE`、`LOCK_MODE`、`LOCK_DATA`、`LOCK_STATUS`。\n4. 看事务年龄：用 `information_schema.innodb_trx` 找持锁事务开始时间、线程 ID、当前 SQL 和状态。\n5. 看执行计划：对等待 SQL 与阻塞 SQL 执行 `EXPLAIN FORMAT=TREE` 或 `EXPLAIN ANALYZE`，确认扫描索引、估算行数和实际行数。\n6. 查死锁样本：用 `SHOW ENGINE INNODB STATUS\\G` 读取最近死锁图，核对事务顺序、锁模式和索引名。\n7. 收敛修复：补更精准的联合索引，按主键或业务分片小批量提交，缩短事务，统一访问顺序，加入退避重试，必要时评估 `READ COMMITTED`。\n8. 验证结果：观察等待事务数、锁等待时长、死锁次数、扫描行数、提交耗时和业务成功率。",
      "工程取舍：Next-Key Lock 用更强的范围保护换取更高的锁等待成本，适合需要把“范围当前事实”纳入事务保护的场景。\n\n- 区间预约、库存区间扣减、任务领取、唯一候选号生成和范围状态迁移，适合用短事务和精准索引承载。\n- 高并发追加写、日志流水、消息落库和事件表写入，应让插入路径避开长范围当前读覆盖的索引区间。\n- 批量修复适合固定排序、分段扫描、每段快速提交，并把外部调用移出持锁事务。\n- 低选择性索引适合补充租户、分片、状态、时间或主键后缀，让锁范围贴近业务对象。\n- `NOWAIT` 与 `SKIP LOCKED` 适合任务队列和抢占式处理，业务需要接受立即失败、跳过、补扫和幂等补偿。\n- 隔离级别调整要配合回归测试、复制语义、约束检查、历史行为和业务一致性口径一起评审。",
      "常见误区：Next-Key Lock 的正确心智模型是“索引记录 + 前置 gap + 当前读扫描路径”。\n\n- Next-Key Lock 保护索引区间，核心影响由实际执行计划决定。\n- 普通一致性读主要依赖 MVCC 和 ReadView；锁定读与写语句走当前读加锁路径。\n- `LOCK_TYPE='RECORD'` 只是数据锁表里的对象类型，`LOCK_MODE`、`INDEX_NAME` 和 `LOCK_DATA` 才能继续判断 record、gap、insert intention 语义。\n- 完整唯一键等值命中通常是最窄锁范围，普通索引和范围扫描需要按索引顺序分析。\n- 幻读治理需要同时看 MVCC、Next-Key Lock、唯一约束、幂等键和业务状态机。\n- 修复锁争用的主线是缩小扫描范围、缩短持锁时间、统一访问顺序、控制重试节奏。",
      "面试追问：Next-Key Lock 题适合按“定义 -> 索引区间 -> 触发语句 -> 幻读关系 -> 排查证据 -> 工程取舍”组织答案。\n\n- Next-Key Lock 是什么，它和记录锁、间隙锁分别是什么关系？\n- 为什么说 Next-Key Lock 覆盖 `(prev_key, current_key]` 这样的索引区间？\n- `REPEATABLE READ` 下范围 `SELECT ... FOR UPDATE` 如何通过 Next-Key Lock 控制并发插入？\n- 完整唯一索引等值命中、普通二级索引等值、范围查询和缺少合适索引的锁范围有什么差异？\n- 插入意向锁和 Next-Key Lock 冲突时，线上会看到哪些等待现象？\n- `READ COMMITTED` 与 `REPEATABLE READ` 对 Next-Key Lock 使用范围有什么影响？\n- 二级索引锁定读为什么可能同时涉及二级索引和聚簇索引记录？\n- `supremum pseudo-record` 在尾部范围锁里说明什么？\n- 如何用 `data_locks`、`data_lock_waits`、`innodb_trx`、执行计划和死锁日志定位 Next-Key Lock 问题？\n- 生产中如何通过索引、事务拆分、访问顺序、退避重试和隔离级别治理 Next-Key Lock 争用？",
      "参考来源：本文主要参考 MySQL 8.4 Reference Manual 的 InnoDB Locking、Locks Set by Different SQL Statements、Locking Reads、Phantom Rows、Performance Schema Lock Tables、INNODB_TRX、Transaction Isolation Levels、Deadlocks 文档，并结合小林 coding 的 MySQL 加锁实验校准中文示例、索引范围判断、锁表字段、死锁排查和面试表达。官方资料用于确定定义、触发语句、隔离级别和观测字段，实践文章用于补足执行计划、锁范围实验和工程治理路径。",
    ],
    typicalProblems: [
      "Next-Key Lock 是什么，它如何组合记录锁和间隙锁？",
      "为什么 Next-Key Lock 常被描述为锁住 `(prev_key, current_key]` 这种索引区间？",
      "`SELECT ... FOR UPDATE`、`UPDATE`、`DELETE` 在范围扫描时怎样触发 Next-Key Lock？",
      "Next-Key Lock 如何帮助 InnoDB 在当前读路径控制幻读和并发插入？",
      "完整唯一索引等值命中、唯一键缺位、普通二级索引范围和缺索引扫描的锁范围如何变化？",
      "插入意向锁、gap 语义和 Next-Key Lock 在锁等待中分别提供什么证据？",
      "`READ COMMITTED` 和 `REPEATABLE READ` 对 Next-Key Lock 与 gap 锁的使用范围有什么影响？",
      "二级索引锁定读为什么可能同时锁二级索引记录和聚簇索引记录？",
      "线上出现插入等待、死锁或 `supremum pseudo-record` 相关锁时如何建立排查证据链？",
      "如何通过联合索引、短事务、固定访问顺序、退避重试、`NOWAIT`/`SKIP LOCKED` 和隔离级别评估治理 Next-Key Lock 争用？",
    ],
    commonCommands: [
      "SELECT * FROM performance_schema.data_locks WHERE OBJECT_NAME = '<table>'",
      "SELECT * FROM performance_schema.data_lock_waits",
      "SELECT trx_id, trx_state, trx_started, trx_mysql_thread_id, trx_query FROM information_schema.innodb_trx",
      "SHOW ENGINE INNODB STATUS\\G",
      "EXPLAIN FORMAT=TREE SELECT ... FOR UPDATE",
      "EXPLAIN ANALYZE SELECT ... FOR UPDATE",
      "SHOW VARIABLES LIKE 'transaction_isolation'",
      "SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ",
    ],
    useCases: ["范围当前读", "幻读治理", "插入等待排查", "死锁治理", "任务队列领取", "区间资源分配", "批量状态迁移", "唯一候选号保护", "MySQL 锁面试"],
    prerequisites: ["gap-lock", "record-lock", "row-lock", "repeatable-read", "mysql-index", "transaction", "isolation-level"],
    related: ["record-lock", "gap-lock", "phantom-read", "deadlock", "read-committed", "mvcc", "sql-optimization", "performance-schema"],
    order: 54,
  },
  /* <!-- KG_REVIEWED: 死锁 | 2026-06-05 | source_count=16 --> */
  /* <!-- KG_EXPLAINED: 死锁 | 2026-05-23 | source_count=5 --> */
  {
    ...
    area: "lock",
    difficulty: "hard",
    concept: "死锁是多个事务在等待图中形成环，彼此等待对方释放锁；InnoDB 会检测等待环，选择一个事务回滚，让系统继续前进。",
    explanation: [
      "概念定位：死锁（Deadlock）解决的是“并发事务互相等待时，数据库如何发现无法自然推进的等待环，并把系统带回可继续执行状态”的问题。它常出现在库存扣减、账户转账、订单状态流转、任务领取、唯一键冲突、批量更新、范围锁竞争和高并发写故障排查中。\n\n死锁本身是并发控制的正常分支，代表多个事务以交叉顺序持有并请求锁。工程上关注三件事：业务能否安全重试，SQL 与索引是否放大锁范围，事务访问顺序是否稳定。",
      "准确定义：在数据库里，死锁是等待关系形成闭环：事务 T1 持有资源 A 并等待资源 B，事务 T2 持有资源 B 并等待资源 A。用等待图（wait-for graph）表示就是 `T1 -> T2 -> T1` 的环。\n\nInnoDB 默认开启死锁检测。检测到环后，它会选择一个事务作为 victim 回滚，释放该事务持有的锁，并向客户端返回 deadlock 错误；另一个事务获得继续执行机会。应用层需要把这个错误当作可预期的并发冲突分支，结合幂等键、状态机和有限重试处理。",
      "心智模型：把每个事务想成拿着几把钥匙排队进门的人。\n\n- 锁是钥匙，索引记录、索引间隙、表意向层和唯一检查都可能成为资源。\n- 等待边从“等待者”指向“持有者”。\n- 等待边形成环时，环里的任何事务都需要别人先释放资源，系统进入僵持。\n- 死锁检测器像调度员，找到环后让一个事务退出队伍，释放它拿着的钥匙。\n\n这个模型能帮助新手理解“为什么两个单独看都合理的事务会互相卡住”，也能帮助老手把排查重点放到锁对象、访问顺序和回滚策略上。",
      "主流程机制：一次 InnoDB 死锁通常按 8 步形成并被打破。\n\n1. 事务 A 开始，按执行计划访问索引并锁住记录或范围，例如锁住 `accounts.id = 1`。\n2. 事务 B 开始，锁住另一条记录或范围，例如锁住 `accounts.id = 2`。\n3. 事务 A 继续请求 B 已持有的锁，进入锁等待。\n4. 事务 B 继续请求 A 已持有的锁，等待图形成 `A -> B -> A`。\n5. InnoDB 在事务等待时运行死锁检测，遍历等待关系并识别等待环。\n6. InnoDB 根据事务代价选择 victim，常见判断会考虑事务已修改或持有的行数等成本信号。\n7. victim 被回滚并释放锁，客户端收到 `ERROR 1213 (40001): Deadlock found when trying to get lock; try restarting transaction`。\n8. 另一个事务继续执行；应用层对被回滚事务执行有限次数重试，并保证重复提交语义安全。\n\n```text\nT1 holds row(id=1) -> waits row(id=2)\nT2 holds row(id=2) -> waits row(id=1)\n\nwait-for graph:\nT1 -> T2\nT2 -> T1\ncycle => deadlock detector chooses a victim\n```",
      "复现实验：下面的转账顺序交叉会构造最典型的行锁死锁。两个会话分别执行，差异只在更新顺序。\n\n```sql\nCREATE TABLE accounts (\n  id BIGINT PRIMARY KEY,\n  balance DECIMAL(12,2) NOT NULL\n) ENGINE=InnoDB;\n\nINSERT INTO accounts VALUES (1, 1000.00), (2, 1000.00);\n\n-- Session A\nSTART TRANSACTION;\nUPDATE accounts SET balance = balance - 100 WHERE id = 1;\n-- 等 Session B 锁住 id=2 后继续\nUPDATE accounts SET balance = balance + 100 WHERE id = 2;\nCOMMIT;\n\n-- Session B\nSTART TRANSACTION;\nUPDATE accounts SET balance = balance - 50 WHERE id = 2;\n-- 等 Session A 锁住 id=1 后继续\nUPDATE accounts SET balance = balance + 50 WHERE id = 1;\nCOMMIT;\n```\n\n同一张表、同一组行、相反访问顺序，是数据库死锁面试和生产复盘中最常见的最小模型。治理动作通常是统一顺序，例如所有转账都按账户 id 从小到大锁定。",
      "深层细节：MySQL 死锁的难点集中在“实际锁住了什么”和“检测成本如何放大”。\n\n- 行锁依赖索引：InnoDB 锁住的是索引记录；二级索引命中后还可能涉及聚簇索引记录。\n- 范围锁参与等待环：`REPEATABLE READ` 下范围当前读可能设置 Next-Key Lock，插入意向锁与范围锁交汇时会形成等待。\n- 唯一检查参与等待：`INSERT`、唯一索引冲突检查、外键检查也会加锁，死锁日志里的 SQL 可能看起来只是在插入。\n- 批量更新顺序漂移：`UPDATE ... WHERE status = 'NEW' LIMIT 100` 这类语句如果缺少稳定排序和合适索引，不同事务可能以不同顺序访问相同记录。\n- 检测有成本：大量事务等待同一批热点行时，死锁检测会频繁遍历等待图；高热点场景需要通过降热点、拆分资源、限流和缩短事务降低检测压力。\n- victim 回滚是语句失败分支：业务事务被回滚后，应用需要重新开启事务并重放业务逻辑，重试次数、退避和幂等保护要一起设计。",
      "工程场景：死锁高发通常意味着资源访问协议需要收敛。\n\n- 转账、换座、库存扣减：多个资源要按稳定顺序锁定，例如按主键升序获取账户、座位或库存行。\n- 订单状态机：用 `WHERE id = ? AND status = ?` 做原子条件更新，减少“先查再改”的窗口。\n- 任务队列领取：使用覆盖状态和排序的联合索引，配合短事务；可跳过场景评估 `SKIP LOCKED`。\n- 批量修复：按主键分批、固定顺序、控制批量大小和事务时长，避免大范围锁和 Undo 膨胀。\n- 唯一性保护：让唯一索引承担最终约束，应用层捕获重复键与 deadlock 后用幂等逻辑收敛。\n- 热点计数器：把单行热点拆分成分片计数或异步聚合，降低所有事务竞争同一把锁的概率。",
      "边界与故障模式：线上死锁常伴随延迟尖刺、事务回滚、锁等待、连接池堆积和业务重试风暴。\n\n- 相反锁顺序：两个接口分别按 `A -> B` 与 `B -> A` 更新同类资源，日志里常能看到相反的主键或索引键。\n- 索引缺失：`EXPLAIN` 显示扫描行数大，`data_locks` 中出现大量记录锁或范围锁，等待环覆盖比业务预期更宽的区间。\n- 长事务：持锁时间变长，等待图更容易形成环，DDL、备份和复制也会受到影响。\n- 间隙锁冲突：范围更新、锁定读和插入在相邻索引区间竞争，`LOCK_MODE` 中可能出现 `GAP` 或 `INSERT_INTENTION`。\n- 自动重试风暴：客户端立即重试且没有退避，高峰期会把同一批热点行再次推入等待环。\n- 连接池状态污染：事务异常后没有明确回滚，连接归还池中，后续请求继承事务或锁相关状态。\n- 可观测性缺口：只记录错误码，没有记录事务 SQL、请求 ID、隔离级别、执行计划和锁表快照，复盘只能猜测。",
      "排查实践：死锁排查要把“谁持有、谁等待、等哪条索引、为什么会以这个顺序访问”写清楚。\n\n1. 固化错误：记录 `ERROR 1213`、SQLSTATE、请求 ID、事务入口、重试次数、发生时间、实例版本和隔离级别。\n2. 读取最近死锁：执行 `SHOW ENGINE INNODB STATUS\\G`，关注 `LATEST DETECTED DEADLOCK`、两个事务、SQL、索引名、锁模式和被回滚事务。\n3. 捕捉当前等待：用 `performance_schema.data_locks` 看锁对象，用 `data_lock_waits` 建立阻塞关系。\n4. 查事务年龄：用 `information_schema.innodb_trx` 找长事务、当前 SQL、锁行数、修改行数和事务开始时间。\n5. 查访问路径：对相关 SQL 执行 `EXPLAIN FORMAT=TREE` 或 `EXPLAIN ANALYZE`，确认是否命中预期索引。\n6. 还原等待图：把 `waiting transaction -> blocking transaction` 画成图，标出表、索引、锁数据和业务资源 id。\n7. 修复协议：统一锁顺序、补索引、缩短事务、拆批、减少交互式事务、加入幂等重试和指数退避。\n8. 验证结果：观察 deadlock 错误率、锁等待数量、事务 p99、重试成功率、连接池等待和扫描行数是否下降。\n\n```sql\nSHOW VARIABLES LIKE 'transaction_isolation';\nSHOW VARIABLES LIKE 'innodb_deadlock_detect';\nSHOW ENGINE INNODB STATUS\\G\n\nSELECT trx_id, trx_state, trx_started, trx_mysql_thread_id,\n       trx_query, trx_rows_locked, trx_rows_modified\nFROM information_schema.innodb_trx\nORDER BY trx_started\\G\n\nSELECT ENGINE_TRANSACTION_ID, THREAD_ID, OBJECT_SCHEMA, OBJECT_NAME,\n       INDEX_NAME, LOCK_TYPE, LOCK_MODE, LOCK_STATUS, LOCK_DATA\nFROM performance_schema.data_locks\\G\n\nSELECT REQUESTING_ENGINE_TRANSACTION_ID, BLOCKING_ENGINE_TRANSACTION_ID,\n       REQUESTING_THREAD_ID, BLOCKING_THREAD_ID\nFROM performance_schema.data_lock_waits\\G\n\nEXPLAIN FORMAT=TREE\nUPDATE accounts SET balance = balance - 100 WHERE id = 1;\n```\n\n一份合格的死锁复盘至少包含：等待图、两侧 SQL、访问索引、锁模式、业务资源顺序、victim、重试结果和长期修复动作。",
      "治理与取舍：降低死锁影响要同时处理数据库、SQL 和应用三层。\n\n- 数据库层：保留死锁检测，开启必要的死锁日志采集；热点极高且检测成本明显时，结合压测评估 `innodb_deadlock_detect` 策略和锁等待超时配置。\n- SQL 层：给锁定读和写语句建立精准索引，批量更新按稳定键排序，范围写入控制批次和事务长度。\n- 应用层：事务入口尽量短，所有路径遵守同一资源顺序；捕获 deadlock 后重开事务，使用有限重试、退避、幂等键和状态机条件。\n- 架构层：热点账户、库存、队列和计数器可以分片、预扣、异步化或引入队列串行化，把数据库锁冲突转成可控的流量整形。\n\n死锁治理的目标是降低频率、缩小影响面、保证自动恢复路径清楚；零死锁目标在高并发 OLTP 中通常会把实现复杂度和吞吐成本推得很高。",
      "常见误区：死锁的正确理解来自等待图、锁对象、访问顺序和重试语义的组合。\n\n- 死锁错误代表数据库已经打破等待环，应用需要把被回滚事务作为失败分支处理。\n- 两条 SQL 文本相同也可能死锁，真实差异常在执行计划、索引扫描顺序、数据分布和事务上下文。\n- `Lock wait timeout exceeded` 和 deadlock 的恢复语义不同，前者来自等待超时，后者来自等待环检测。\n- 缺索引会扩大扫描和锁范围，死锁概率随等待边数量上升。\n- 短事务、稳定锁顺序、精准索引、幂等重试是生产治理的主线。\n- 死锁日志记录的是最近一次或采样证据，线上复盘需要结合应用日志、Performance Schema 和慢 SQL 时间线。",
      "面试追问：死锁题适合按“定义 -> 等待图 -> InnoDB 锁 -> 检测回滚 -> 复现 -> 排查 -> 治理”组织答案。\n\n- 什么是数据库死锁，如何用等待图描述？\n- InnoDB 检测到死锁后会怎样处理，客户端会收到什么错误？\n- 锁等待超时和死锁回滚在触发条件和应用处理上有什么差异？\n- 为什么两个转账事务按相反顺序更新账户会死锁？\n- 行锁、间隙锁、Next-Key Lock、插入意向锁怎样参与等待环？\n- 缺少索引为什么会放大死锁概率？\n- `SHOW ENGINE INNODB STATUS\\G` 的死锁日志要重点看哪些字段？\n- 如何用 `data_locks`、`data_lock_waits` 和 `innodb_trx` 还原谁阻塞谁？\n- 高并发热点行死锁频繁时，如何在 SQL、事务、应用重试和架构拆分之间取舍？\n- 生产系统如何设计 deadlock 重试，才能兼顾幂等、延迟和流量峰值？",
      "参考来源：本讲解主要参考 MySQL 8.4 Reference Manual 的 InnoDB Deadlocks、Deadlock Detection、Deadlock Handling、InnoDB Locking、Locks Set by Different SQL Statements、Performance Schema Lock Tables、INNODB_TRX 和 SHOW ENGINE 文档，并结合 Microsoft/Azure SQL deadlock guide、wait-for graph 资料、SoByte 对 InnoDB 死锁检测实现的分析、小林 coding 的 MySQL 加锁实验和 OneUptime 的锁排查文章校准复现、诊断和工程治理路径。官方资料用于确定 MySQL 行为边界，工程文章用于补充等待图、日志解读、索引路径和生产治理经验。"
    ],
    typicalProblems: [
      "死锁解决什么并发问题，为什么可以用等待图描述？",
      "InnoDB 发现死锁后如何选择 victim，事务和客户端会发生什么？",
      "如何用两个转账事务构造行锁死锁，统一资源顺序为什么有效？",
      "行锁、间隙锁、Next-Key Lock、插入意向锁和唯一检查如何参与死锁？",
      "锁等待超时与死锁检测在触发条件、错误处理和重试策略上有什么差异？",
      "为什么缺少索引、批量更新顺序漂移和长事务会提高死锁概率？",
      "如何解读 `SHOW ENGINE INNODB STATUS\\G` 的 `LATEST DETECTED DEADLOCK`？",
      "线上如何用 `data_locks`、`data_lock_waits`、`innodb_trx` 和执行计划还原等待图？",
      "应用层 deadlock 重试如何设计幂等、退避、最大次数和状态机条件？",
      "热点账户、库存、任务队列死锁频繁时，SQL、事务和架构层各有哪些治理手段？"
    ],
    commonCommands: [
      "SHOW ENGINE INNODB STATUS\\G",
      "SHOW VARIABLES LIKE 'innodb_deadlock_detect'",
      ...
      "SELECT * FROM performance_schema.data_locks\\G",
      "SELECT * FROM performance_schema.data_lock_waits\\G",
      "SELECT trx_id, trx_state, trx_started, trx_query FROM information_schema.innodb_trx ORDER BY trx_started\\G",
      "EXPLAIN FORMAT=TREE UPDATE ...",
    ],
    practiceAdvice: [
      "多资源更新按稳定业务键或主键顺序加锁。",
      "锁定读和写语句优先建立能覆盖过滤条件与排序顺序的索引。",
      "事务保持短小，避免事务内远程调用、人工等待和大批量扫描。",
      "deadlock 捕获后重开事务，使用有限重试、退避和幂等保护。",
      "死锁复盘固定输出等待图、索引、锁模式、业务资源顺序和修复验证指标。",
    ],
    ...
    commonIssues: [
      ...
      "长事务持锁时间过长并放大等待图。",
      "客户端立即重试造成热点资源二次拥塞。",
      "死锁日志采集不足导致复盘缺少锁对象和 SQL 时间线。",
    ],
    useCases: ["并发写故障排查", "事务顺序优化", "锁等待治理", "热点资源拆分", "幂等重试设计", "批量更新规范", "数据库面试"],
    prerequisites: ["lock", "row-lock", "record-lock", "gap-lock", "next-key-lock", "transaction", "mysql-index"],
    related: ["deadlock-log", "show-engine-innodb-status", "lock", "row-lock", "gap-lock", "next-key-lock", "isolation-level", "sql-optimization"],
    order: 55,
  },
  ...
  /* <!-- KG_REVIEWED: SQL 优化 | 2026-06-05 | source_count=25 --> */
  /* <!-- KG_EXPLAINED: SQL 优化 | 2026-05-23 | source_count=5 --> */
  {
    ...
    difficulty: "hard",
    concept:
      "SQL 优化是用执行计划、索引设计、SQL 改写、统计信息和运行指标降低扫描、排序、锁等待、I/O 与返回成本的工程过程。",
    explanation: [
      "概念定位：SQL 优化（SQL Optimization）解决的是“同一份数据需求如何用更低成本、更稳定延迟、更小并发影响完成”的问题。它出现在接口 p95/p99 抖动、慢查询治理、CPU/I/O 飙升、连接池排队、复制延迟、报表跑批、线上发布评审和数据库面试中。\n\n在 MySQL 中，SQL 优化围绕 `SELECT`、`UPDATE`、`DELETE`、`INSERT ... SELECT` 等语句展开。核心工作是让优化器选到合适访问路径，让 InnoDB 少读页、少回表、少排序、少持锁，让应用少传输字段、少做深分页、少放大事务。新手先掌握 `EXPLAIN`、索引和慢查询日志；有经验的工程师还要看统计信息、参数分布、锁范围、Buffer Pool、临时表、复制延迟、发布回滚和优化收益证据。",
      "准确定义：SQL 优化是一套证据驱动的查询成本治理方法，目标是降低总成本并保持语义正确。\n\n关键对象包括：\n\n- 执行计划：优化器选择的表访问顺序、访问类型、索引、JOIN 算法、排序、分组和临时结果处理方式。\n- 索引结构：B+ 树主键、二级索引、联合索引、覆盖索引、选择性、最左前缀和索引维护成本。\n- 谓词与返回列：`WHERE`、`JOIN`、`ORDER BY`、`GROUP BY`、`LIMIT`、投影列共同决定扫描范围和结果宽度。\n- 统计信息：表和索引统计、直方图、数据分布、热点参数会影响优化器估算。\n- 运行证据：慢查询日志、`EXPLAIN ANALYZE`、Performance Schema、状态变量、锁等待、CPU、I/O 和复制延迟展示真实影响。\n- 变更约束：新增索引、改 SQL、改模型、拆批、加缓存和归档都要评估写入成本、空间成本、发布风险和回滚路径。\n\n优化结论要同时回答三个问题：SQL 语义是否正确，访问路径是否高效，线上指标是否改善。",
      "心智模型：把 SQL 优化看成一条从“业务问题”到“数据库执行证据”的成本压缩链。\n\n- 业务层：确认接口需要哪些数据、实时性、分页方式、排序稳定性和一致性要求。\n- SQL 层：让过滤条件、关联条件、排序分组和返回列表达清楚，减少函数包裹列、隐式转换和宽返回。\n- 优化器层：通过统计信息、索引和代价模型选择访问路径，必要时用 `ANALYZE TABLE`、直方图、不可见索引或 hint 辅助验证。\n- InnoDB 层：减少页读取、回表、锁范围、Undo/Redo、临时表和刷盘压力。\n- 观测层：用慢日志、执行计划、语句摘要和系统指标证明扫描行数、耗时、等待和资源占用下降。\n\n这个模型能解释很多线上现象：同样返回 20 行，索引范围扫描和全表扫描的成本差异巨大；同样命中索引，回表多、排序大、参数偏斜和统计陈旧也会让延迟变差。",
      "主流程机制：一次严谨的 SQL 优化按“定位 -> 解释 -> 改动 -> 验证 -> 发布”推进。\n\n1. 定位目标：从慢查询日志、APM、接口超时、Performance Schema 语句摘要中找出高总耗时、高频、高扫描行数或高锁等待的 SQL 模板。\n2. 固化现场：记录 SQL 文本、绑定参数、表结构、索引、数据量、数据分布、隔离级别、事务边界、调用入口和实例角色。\n3. 分析计划：用 `EXPLAIN FORMAT=TREE` 看优化器估算路径，用 `EXPLAIN ANALYZE` 看真实行数、循环次数和各迭代器耗时。\n4. 判断成本来源：区分全表扫描、低选择性索引、回表过多、filesort、临时表、JOIN 中间行膨胀、深分页、锁等待、统计信息偏差和返回列过宽。\n5. 选择修复策略：调整联合索引、改写谓词、裁剪列、改游标分页、拆批、拆报表、归档冷热数据、更新统计信息，必要时用 hint 做受控验证。\n6. 验证收益：对比优化前后的访问类型、扫描行数、返回行数、实际耗时、锁等待、CPU、I/O、临时表、复制延迟和写入开销。\n7. 安全发布：对新增索引用灰度、不可见索引验证、低峰变更、回滚脚本和监控阈值保护；对 SQL 改写用回归用例保证结果集语义一致。\n\nSQL 优化的产出是一组可复盘证据，而优秀优化会同时降低单次成本和系统波动。",
      "实践例子：下面用订单列表慢查询展示从问题到优化的路径。\n\n```sql\nCREATE TABLE orders (\n  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,\n  user_id BIGINT UNSIGNED NOT NULL,\n  status VARCHAR(32) NOT NULL,\n  amount DECIMAL(12,2) NOT NULL,\n  created_at DATETIME NOT NULL,\n  updated_at DATETIME NOT NULL,\n  PRIMARY KEY (id),\n  KEY idx_user_created (user_id, created_at)\n) ENGINE=InnoDB;\n\n-- 问题 SQL：返回列宽、排序字段和索引顺序匹配度低，深分页扫描成本高\nEXPLAIN ANALYZE\nSELECT *\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC, id DESC\nLIMIT 100000, 20;\n\n-- 优化方向：联合索引覆盖过滤、排序和稳定游标，列表页只取必要列\nALTER TABLE orders\n  ADD INDEX idx_user_status_created_id (user_id, status, created_at, id);\n\nEXPLAIN ANALYZE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001\n  AND status = 'PAID'\n  AND (created_at, id) < ('2026-06-05 10:00:00', 987654321)\nORDER BY created_at DESC, id DESC\nLIMIT 20;\n```\n\n这个例子里，优化收益来自四点：`status` 进入联合索引降低候选行，排序键和索引顺序对齐减少 filesort，列裁剪降低回表和网络成本，游标分页把深 offset 的跳过成本变成连续范围扫描。",
      "深层细节：SQL 优化的难点来自优化器估算与真实数据之间的偏差。\n\n- 访问类型：`const`、`eq_ref`、`ref`、`range`、`index`、`ALL` 表示不同访问方式；判断质量时同时看 `rows`、`filtered`、回表、排序和实际耗时。\n- 联合索引：等值列、范围列、排序列、返回列的顺序服务同一条访问路径；范围条件之后的索引列仍可能通过 ICP 过滤候选记录。\n- WHERE 谓词：字段类型匹配、清晰半开区间、左前缀匹配和可定位条件更利于 B+ 树；函数包裹列、隐式转换和跨列 `OR` 会扩大候选集。\n- JOIN：驱动表、连接列类型、JOIN 顺序和连接列索引决定中间行数；参数热点和统计偏差会让计划漂移。\n- 排序分组：`ORDER BY`、`GROUP BY`、`DISTINCT` 常触发 filesort 或临时表；合适索引能把过滤、排序和分组合并到有序扫描。\n- LIMIT：小 `LIMIT` 可以让优化器偏向快速返回少量行；大 offset 仍会消耗读取和丢弃成本，游标分页更适合连续浏览。\n- 统计信息：`ANALYZE TABLE`、持久化统计、直方图和索引基数影响估算；热点值、倾斜分布和历史归档会改变最佳计划。\n- 控制手段：optimizer hint、`optimizer_switch`、不可见索引和 optimizer trace 适合用于验证假设，长期方案优先回到数据模型、索引和 SQL 语义。\n\n老手优化时会先判断“成本来自候选行、回表、排序、JOIN、锁、I/O、网络中的哪一项”，再选择最小改动。",
      "工程场景：SQL 优化要贴合具体业务路径。\n\n- 接口列表：围绕租户、用户、状态、时间、稳定排序键设计联合索引，优先游标分页和明确列清单。\n- 详情查询：主键或唯一键定位，保证访问类型稳定，控制 N+1 查询和 ORM 生成的宽查询。\n- 报表统计：限制时间范围，拆到只读副本或离线链路，使用预聚合、汇总表或宽表承担重复计算。\n- 批量订正：按主键范围分批，控制每批行数、事务时长、锁范围、Undo/Redo、Binlog 和复制延迟。\n- 多表 JOIN：让驱动表过滤强，连接列类型一致，连接列有索引，中间结果可估算。\n- 高并发写：优化 `UPDATE/DELETE` 的定位条件和访问顺序，降低锁范围、死锁概率和长事务影响。\n- 发布评审：新增索引关注 DDL 算法、元数据锁、磁盘空间、写入成本、备份恢复和回滚窗口。",
      "边界与故障模式：SQL 优化常见失败点来自证据缺口和局部收益。\n\n- 计划偏差：统计信息陈旧、参数分布倾斜、热点租户、历史数据归档后，优化器估算行数与真实行数偏离。\n- 索引副作用：新增索引降低读成本，同时增加写入维护、空间、Buffer Pool 压力、DDL 时间和复制延迟。\n- 锁范围扩大：`UPDATE/DELETE` 缺少合适索引会扫描大量记录，带来锁等待、死锁和回滚成本。\n- 排序落盘：大 `ORDER BY`、`GROUP BY`、宽返回列和临时表会占用内存与磁盘临时空间，导致抖动。\n- 深分页放大：大 offset 会先扫描并丢弃大量候选行，接口返回量小也可能拖慢实例。\n- JOIN 爆炸：低选择性驱动表、多对多关系、过滤后置和类型错配会放大中间行数。\n- 缓存掩盖：Buffer Pool 命中能让测试环境很快，冷数据、峰值并发和副本资源紧张时真实成本显现。\n- 语义风险：SQL 改写、分页方式变化、JOIN 顺序 hint 和缓存引入都需要结果集校验、排序稳定性校验和一致性校验。",
      "排查实践：慢 SQL 排查建议使用固定证据包。\n\n1. 找模板：从慢查询日志和 Performance Schema 聚合出高总耗时、高平均耗时、高扫描行数和高执行次数的 SQL digest。\n2. 还原参数：拿到真实绑定参数、租户、时间范围、分页位置、返回列和调用链。\n3. 查结构：导出 `SHOW CREATE TABLE`、`SHOW INDEX`、字段类型、字符集、索引基数和表行数。\n4. 查计划：对典型参数执行 `EXPLAIN FORMAT=TREE` 和 `EXPLAIN ANALYZE`，对比估算行数与实际行数。\n5. 查运行：观察锁等待、临时表、排序、CPU、I/O、Buffer Pool、复制延迟和连接池等待。\n6. 做改动：优先选择能解释成本来源的最小改动，例如补联合索引、列裁剪、半开区间、游标分页、拆批或更新统计信息。\n7. 验证回归：用同一组 SQL、参数和指标对比优化前后，并检查写入成本、DDL 风险和结果集一致性。\n\n```sql\nSHOW VARIABLES LIKE 'slow_query_log';\nSHOW VARIABLES LIKE 'long_query_time';\n\nSELECT DIGEST_TEXT, COUNT_STAR, SUM_ROWS_EXAMINED, SUM_ROWS_SENT,\n       ROUND(SUM_TIMER_WAIT / 1000000000000, 2) AS total_seconds\nFROM performance_schema.events_statements_summary_by_digest\nORDER BY SUM_TIMER_WAIT DESC\nLIMIT 10;\n\nSHOW CREATE TABLE orders\\G\nSHOW INDEX FROM orders;\n\nEXPLAIN FORMAT=TREE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC, id DESC\nLIMIT 20;\n\nEXPLAIN ANALYZE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC, id DESC\nLIMIT 20;\n\nANALYZE TABLE orders;\n```\n\n排查结论要写成证据链：慢 SQL 样本、原计划、成本来源、改动方案、新计划、指标变化和回滚方案。",
      "常见误区：SQL 优化的核心是整体成本和证据闭环。\n\n- 索引命中只是入口，`rows`、`filtered`、回表、filesort、临时表、实际耗时和等待事件共同决定质量。\n- `EXPLAIN` 展示优化器估算，`EXPLAIN ANALYZE` 展示真实执行行数和耗时，两者结合能发现统计偏差。\n- 联合索引服务一组高频访问模式，字段顺序来自过滤、排序、分页和返回列这条完整路径。\n- SQL 改写要保留业务语义，尤其关注 `NULL`、重复行、排序稳定性、分页边界、事务隔离和读写路由。\n- 优化读路径也会影响写路径，新增索引、统计刷新、DDL 和缓存策略都要放进发布评审。\n- 生产优化需要指标闭环，单次测试耗时、开发库数据和缓存命中情况只提供局部证据。",
      "面试追问：SQL 优化类问题适合按“定位、计划、成本、修复、验证、取舍”回答。\n\n- 慢 SQL 从哪些入口定位，慢查询日志和 Performance Schema 分别提供什么证据？\n- `EXPLAIN` 的 `type`、`key`、`rows`、`filtered`、`Extra` 分别如何判断计划质量？\n- `EXPLAIN ANALYZE` 与普通 `EXPLAIN` 如何配合使用，什么时候关注估算与真实行数偏差？\n- 联合索引如何结合等值条件、范围条件、排序字段、分页游标和覆盖列设计？\n- `WHERE` 中隐式转换、函数包裹列、前置通配符、跨列 `OR` 为什么会放大扫描？\n- `JOIN`、`ORDER BY`、`GROUP BY`、`DISTINCT`、`LIMIT offset` 各自有哪些典型成本？\n- 新增索引会带来哪些写入、空间、DDL、复制和回滚成本？\n- 线上 `UPDATE` 慢、锁等待多、死锁增多时，SQL 优化如何缩小锁范围？\n- 统计信息陈旧或参数倾斜导致计划漂移时，如何用 `ANALYZE TABLE`、直方图、hint 或索引验证？\n- 一个 SQL 优化方案上线前需要哪些验证和监控指标？",
      "参考来源：本讲解主要参考 MySQL 8.4 Reference Manual 的 Optimization、Optimizing SELECT Statements、WHERE、Range、How MySQL Uses Indexes、Multiple-Column Indexes、Verifying Index Usage、Index Condition Pushdown、ORDER BY、GROUP BY、LIMIT、Condition Filtering、Optimizer Statistics、Optimizer Trace、Optimizer Hints、`ANALYZE TABLE`、`EXPLAIN`、EXPLAIN Output、Slow Query Log 和 Performance Schema Statement Tables 文档，并结合小林 coding 的 SELECT 执行流程与索引讲解、JavaGuide 的 EXPLAIN 和索引文章进行中文表达校准。官方资料用于定义、优化器行为、命令语义和边界条件，中文资料用于补足学习路径、常见问法和工程排查表述。"
    ],
    typicalProblems: [
      "SQL 优化解决什么问题，为什么要从慢查询样本和运行证据开始？",
      "MySQL 优化器如何利用统计信息、索引和代价模型生成执行计划？",
      "`EXPLAIN` 中 `type`、`key`、`rows`、`filtered`、`Extra` 如何判断计划质量？",
      "`EXPLAIN ANALYZE` 如何补充真实行数、循环次数和迭代器耗时？",
      "联合索引如何围绕 `WHERE`、`JOIN`、`ORDER BY`、`GROUP BY` 和 `LIMIT` 设计？",
      "隐式转换、函数包裹列、低选择性字段、跨列 `OR` 和前置通配符会造成哪些成本？",
      "深分页、filesort、临时表、回表过多和 JOIN 中间行膨胀分别如何优化？",
      "新增索引、SQL 改写、更新统计信息、hint、拆批和归档分别适合哪些场景？",
      "慢 `UPDATE/DELETE` 如何通过索引和访问顺序缩小锁范围并降低死锁概率？",
      "线上 SQL 优化上线前如何验证收益、语义一致性、写入成本和回滚路径？"
    ],
    commonCommands: [
      "SHOW VARIABLES LIKE 'slow_query_log'",
      "SHOW VARIABLES LIKE 'long_query_time'",
    ...
    practiceAdvice: [
      "先收集真实慢 SQL、参数、执行计划和指标，再选择优化动作。",
      "高频列表优先让联合索引覆盖过滤、排序、分页边界和必要返回列。",
      "批量写入和订正脚本按主键范围分批，控制事务时长、锁范围和复制延迟。",
      "新增索引用低峰 DDL、磁盘空间检查、写入成本评估和回滚脚本保护。",
      "优化后保存前后计划、扫描行数、实际耗时、锁等待和业务指标，形成可复盘记录。",
    ],
    applicationScenarios: [
      "接口慢查询治理",
      "订单列表分页",
    ...
    commonIssues: [
      "统计信息陈旧导致估算行数偏离真实行数。",
      "返回列过宽导致回表、临时表和网络传输成本升高。",
      "联合索引字段顺序与过滤、排序和分页路径脱节。",
      "深分页消耗大量候选行扫描和丢弃成本。",
      "新增索引放大写入维护、空间占用和 DDL 风险。",
      "慢写语句扫描范围过大引发锁等待、死锁和复制延迟。",
    ],
    useCases: ["慢查询治理", "接口响应优化", "数据库成本下降", "索引评审", "分页优化", "JOIN 优化", "批量写保护", "线上性能排查"],
    prerequisites: ["mysql-index","select"],
    related: ["explain","slow-query-log","where","join","order-by","group-by","limit-offset","composite-index","covering-index","index-selectivity"],
    order: 57,
  },
  /* <!-- KG_REVIEWED: EXPLAIN | 2026-06-05 | source_count=17 --> */
  /* <!-- KG_EXPLAINED: EXPLAIN | 2026-05-23 | source_count=5 --> */
  {
    ...
    difficulty: "medium",
    concept:
      "EXPLAIN 是 MySQL 查看执行计划的诊断语句，用访问类型、索引、估算行数、过滤比例和 Extra 信息解释一条 SQL 的成本来源。",
    explanation: [
      "概念定位：`EXPLAIN` 解决的是“这条 SQL 为什么这样执行，以及成本主要花在哪里”的问题。它常出现在慢查询定位、索引评审、JOIN 顺序排查、分页优化、报表治理、生产事故复盘和 MySQL 面试里。\n\nMySQL 优化器会把 SQL 转换为执行计划，计划中包含表访问顺序、访问方式、可选索引、最终索引、估算扫描行数、过滤比例和额外操作。`EXPLAIN` 把这些信息展示出来，帮助工程师把“感觉慢”变成可讨论、可验证、可回滚的证据。",
      "准确定义：`EXPLAIN` 是 MySQL 的执行计划查看语句，可用于 `SELECT`，也可用于部分 `INSERT`、`UPDATE`、`DELETE`、`REPLACE` 等语句；它展示优化器选择的估算计划。MySQL 还支持 `EXPLAIN FORMAT=JSON`、`EXPLAIN FORMAT=TREE` 和 `EXPLAIN ANALYZE` 等形式。\n\n关键区别：\n\n- 普通 `EXPLAIN`：展示优化器估算出的访问路径，适合快速判断索引、访问类型和扫描量。\n- `FORMAT=JSON`：展示更结构化的代价、嵌套查询、条件下推和附加属性，适合脚本化分析与复杂计划阅读。\n- `FORMAT=TREE`：用树形结构展示迭代器执行路径，阅读 JOIN、排序和过滤顺序更直观。\n- `EXPLAIN ANALYZE`：真实执行语句并返回实际耗时、实际行数和循环次数，适合验证估算偏差和定位真实瓶颈。\n\n生产环境使用 `EXPLAIN ANALYZE` 要选择可控 SQL、可控参数和低风险窗口，因为它会执行目标语句。",
      "心智模型：把 `EXPLAIN` 看成数据库给 SQL 的“路线图”。\n\n- `table` 像路线图上的站点，说明每一步访问哪张表或派生表。\n- `type` 像交通方式，`const`、`eq_ref`、`ref`、`range`、`index`、`ALL` 表示从精确定位到大范围扫描的不同路径。\n- `possible_keys` 是候选路线，`key` 是实际选择路线，`key_len` 说明用到了索引前缀长度。\n- `rows` 是预计要看多少行，`filtered` 是预计过滤后保留多少比例。\n- `Extra` 是路况提示，例如 `Using index`、`Using where`、`Using filesort`、`Using temporary`。\n\n新手先看 `type/key/rows/Extra` 四列；有经验的工程师继续追踪估算与真实行数差异、参数偏斜、统计信息、回表、排序、临时表和锁影响。",
      "主流程机制：一次 `EXPLAIN` 分析可以按固定顺序推进。\n\n1. 固化 SQL：记录 SQL 模板、绑定参数、表结构、索引、数据量、过滤字段分布和业务调用入口。\n2. 读取计划：先执行 `EXPLAIN FORMAT=TREE` 或普通 `EXPLAIN`，确认表访问顺序、访问类型、索引选择和额外操作。\n3. 判断入口：看 `type`、`possible_keys`、`key`、`key_len`，确认查询是否从高选择性条件或合适联合索引进入。\n4. 估算规模：用 `rows * filtered / 100` 粗略估算进入下一步的候选行，再结合返回行数判断中间结果是否过大。\n5. 识别额外成本：在 `Extra` 中寻找 `Using filesort`、`Using temporary`、`Using index condition`、`Using where`、`Using index` 等信号。\n6. 对照真实执行：对代表性参数运行 `EXPLAIN ANALYZE`，比较 estimated rows 和 actual rows，定位统计偏差、热点参数或计划漂移。\n7. 形成改动：选择补索引、调整联合索引顺序、裁剪返回列、改写谓词、游标分页、更新统计信息或拆批，并再次验证计划。\n\n这个流程的价值在于把执行计划、数据分布和业务参数放到同一张证据表里。",
      "实践例子：订单列表查询可以展示 `EXPLAIN` 如何指导索引和分页优化。\n\n```sql\n-- 业务 SQL：按用户和状态筛选，按创建时间倒序取最近订单\nEXPLAIN FORMAT=TREE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC, id DESC\nLIMIT 20;\n\n-- 传统表格输出，适合快速看 key、rows、Extra\nEXPLAIN\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC, id DESC\nLIMIT 20;\n\n-- 验证真实执行行数和耗时\nEXPLAIN ANALYZE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC, id DESC\nLIMIT 20;\n\n-- 常见修复：让过滤、排序和稳定分页键进入同一个联合索引\nCREATE INDEX idx_user_status_created_id\nON orders (user_id, status, created_at, id);\n```\n\n优化前如果看到 `type=ALL`、`key=NULL`、`rows` 很大、`Extra=Using where; Using filesort`，说明查询要扫描大量候选行再排序。优化后更理想的信号是 `key=idx_user_status_created_id`、`type=ref` 或 `range`、`rows` 接近业务候选量，`filesort` 消失或明显减少。",
      "深层细节：`EXPLAIN` 的字段需要组合解读。\n\n- `id/select_type`：识别查询块、子查询、派生表、UNION 和物化边界，复杂 SQL 要先看执行树层级。\n- `type`：表示访问方式；`ALL` 常提示全表扫描，`index` 表示全索引扫描，`range/ref/eq_ref/const` 通常表示更精确的入口。\n- `possible_keys/key/key_len/ref`：展示候选索引、实际索引、使用前缀长度和比较对象；联合索引是否按预期使用通常靠这些字段确认。\n- `rows/filtered`：都是估算值，受统计信息、索引基数、直方图和条件相关性影响，适合与 `EXPLAIN ANALYZE` 的实际行数对照。\n- `Extra`：补充执行信号；`Using index` 常代表覆盖索引读取，`Using index condition` 代表 ICP，`Using filesort` 代表额外排序，`Using temporary` 代表中间临时结果。\n- `FORMAT=JSON`：能看到更细的成本与附加信息，适合复杂 JOIN、子查询和优化器判断过程分析。\n\n优秀判断会回答“从哪张表进入、通过哪个索引进入、预计读多少行、真实读多少行、额外做了什么、下一步改哪里”。",
      "工程场景：`EXPLAIN` 在生产优化中通常和其他证据一起使用。\n\n- 慢查询治理：慢日志负责发现样本，`EXPLAIN` 解释计划，Performance Schema 量化总耗时和扫描量。\n- 索引评审：新增索引前看 `possible_keys/key/key_len/rows`，上线后确认计划真正使用目标索引。\n- JOIN 优化：通过执行树确认驱动表、被驱动表、连接列索引和中间行数，避免低选择性表先放大结果。\n- 排序分页：用 `Extra` 识别 filesort 和临时表，用联合索引、稳定排序键和游标分页降低候选行。\n- 批量写排障：对 `UPDATE/DELETE` 读取计划，确认定位条件能缩小扫描和锁范围。\n- 计划漂移：同一 SQL 在热点参数、月末数据、历史归档后计划变化时，对比不同参数的 `EXPLAIN ANALYZE`。",
      "边界与故障模式：`EXPLAIN` 提供诊断入口，最终结论要结合真实执行和业务语义。\n\n- 估算偏差：统计信息陈旧、数据倾斜、字段相关性、热点租户会让 `rows` 与真实行数相差很大。\n- 参数敏感：同一 SQL 模板在小用户和大用户、短时间窗和长时间窗下可能选择不同访问路径。\n- 索引命中误判：`key` 有值只说明使用了索引，低选择性、回表过多、filesort 和临时表仍会让查询变慢。\n- `Extra` 语义细分：`Using where` 是过滤信号，`Using index` 是覆盖读取信号，`Using filesort` 是额外排序信号，需要结合数据量判断影响。\n- 写语句风险：对写 SQL 做分析时关注访问路径和锁范围，生产验证要控制事务、超时、回滚和复制影响。\n- 版本差异：MySQL 版本、优化器开关、统计信息和执行格式会影响展示字段与计划选择，排障记录要写明版本。",
      "排查实践：线上慢 SQL 建议把 `EXPLAIN` 放进标准证据包。\n\n```sql\n-- 1. 找高成本 SQL 模板\nSELECT DIGEST_TEXT, COUNT_STAR, SUM_ROWS_EXAMINED, SUM_ROWS_SENT,\n       ROUND(SUM_TIMER_WAIT / 1000000000000, 2) AS total_seconds\nFROM performance_schema.events_statements_summary_by_digest\nORDER BY SUM_TIMER_WAIT DESC\nLIMIT 10;\n\n-- 2. 固化结构和索引\nSHOW CREATE TABLE orders\\G\nSHOW INDEX FROM orders;\n\n-- 3. 看估算计划\nEXPLAIN FORMAT=TREE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC, id DESC\nLIMIT 20;\n\n-- 4. 看真实计划，选择低风险窗口执行\nEXPLAIN ANALYZE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC, id DESC\nLIMIT 20;\n\n-- 5. 统计信息偏差时刷新后再对比\nANALYZE TABLE orders;\n```\n\n记录模板建议包含：SQL、参数、表行数、索引、普通 `EXPLAIN`、`EXPLAIN ANALYZE`、慢日志指标、锁等待、优化动作、新计划、业务结果校验和回滚动作。",
      "常见误区：`EXPLAIN` 的正确用法是证据组合。\n\n- 执行计划用于解释访问路径，慢日志和 Performance Schema 用于证明线上影响。\n- `type` 的好坏要结合 `rows`、`filtered`、返回行数、回表、排序和实际耗时判断。\n- `key` 命中目标索引后，还要看联合索引使用长度、过滤列、排序列和覆盖列是否服务同一条访问路径。\n- `rows` 是估算值，`EXPLAIN ANALYZE` 的 actual rows 能揭示统计信息和参数分布问题。\n- `Using filesort` 和 `Using temporary` 是优化线索，真正优先级取决于数据量、并发量和响应时间目标。\n- SQL 改写、索引新增和 hint 使用都要用同一批参数验证结果集语义、计划质量和线上指标。",
      "面试追问：回答 `EXPLAIN` 可以按“字段含义、成本判断、验证方法、优化动作”组织。\n\n- `EXPLAIN` 解决什么问题，普通 `EXPLAIN` 与 `EXPLAIN ANALYZE` 的证据差异是什么？\n- `type` 从 `const`、`eq_ref`、`ref`、`range`、`index` 到 `ALL` 分别代表什么访问方式？\n- `possible_keys` 有值但 `key` 为 `NULL` 时，通常从哪些方向分析？\n- `rows` 和 `filtered` 如何估算进入下一步的候选行规模？\n- `Using index`、`Using index condition`、`Using where`、`Using filesort`、`Using temporary` 各自说明什么？\n- 多表 JOIN 的执行计划如何判断驱动表、被驱动表和中间结果是否合理？\n- 统计信息陈旧或参数偏斜导致估算错误时，如何用 `ANALYZE TABLE`、直方图、不同参数和 `EXPLAIN ANALYZE` 验证？\n- 慢 `UPDATE/DELETE` 如何通过 `EXPLAIN` 判断扫描范围和锁风险？\n- 新增联合索引后如何证明查询、排序、分页和回表成本真的下降？\n- 生产环境使用 `EXPLAIN ANALYZE` 要注意哪些执行风险和验证边界？",
      "参考来源：本讲解主要参考 MySQL 8.4 Reference Manual 的 `EXPLAIN` Statement、EXPLAIN Output Format、Optimizing SELECT、WHERE/Range Optimization、How MySQL Uses Indexes、Verifying Index Usage、Index Condition Pushdown、ORDER BY/GROUP BY/LIMIT Optimization、Optimizer Statistics 与 Performance Schema Statement Tables；结合 MySQL Workbench Visual Explain、Use The Index, Luke 的执行计划操作说明和 JavaGuide 的 MySQL 执行计划分析校准中文术语、阅读顺序和常见面试问法。"
    ],
    typicalProblems: [
      "`EXPLAIN` 解决什么问题，为什么它是慢 SQL 优化的核心证据之一？",
      "普通 `EXPLAIN`、`FORMAT=JSON`、`FORMAT=TREE`、`EXPLAIN ANALYZE` 分别适合什么场景？",
      "`type`、`possible_keys`、`key`、`key_len`、`ref` 如何判断索引入口质量？",
      "`rows` 和 `filtered` 为什么是估算值，如何和 `EXPLAIN ANALYZE` 的真实行数对照？",
      "`Using index`、`Using where`、`Using index condition`、`Using filesort`、`Using temporary` 各自代表什么执行信号？",
      "多表 JOIN 的执行计划如何判断驱动表、访问顺序和中间结果规模？",
      "为什么 `key` 命中索引后查询仍然可能很慢，如何继续分析回表、排序和扫描量？",
      "统计信息陈旧、数据倾斜和参数偏斜如何影响执行计划，线上如何验证？",
      "对慢 `UPDATE/DELETE` 使用 `EXPLAIN` 时，如何判断锁范围和批量写风险？",
      "新增索引或改写 SQL 后，如何用 `EXPLAIN`、慢日志和 Performance Schema 证明优化有效？"
    ],
    commonCommands: [
      "EXPLAIN <sql>",
      "EXPLAIN FORMAT=TREE <sql>",
    ...
    practiceAdvice: [
      "先用真实 SQL 和真实参数看计划，再讨论索引和 SQL 改写。",
      "阅读计划时按表访问顺序、访问类型、索引、估算行数、Extra 的顺序记录证据。",
      "对高风险 SQL 使用 `EXPLAIN ANALYZE` 前先评估执行成本、锁影响和业务窗口。",
      "新增联合索引后同时验证过滤、排序、分页和回表成本，保留优化前后计划。",
      "估算与真实行数差异明显时，检查统计信息、直方图、数据倾斜和参数分布。",
    ],
    applicationScenarios: [
      "慢查询定位",
      "索引设计评审",
    ...
    commonIssues: [
      "统计信息陈旧导致 `rows` 估算偏离真实行数。",
      "低选择性索引命中后仍产生大量回表和过滤。",
      "`Using filesort` 与 `Using temporary` 在大结果集下放大内存和磁盘开销。",
      "联合索引使用长度短，过滤列和排序列没有进入同一条访问路径。",
      "热点参数让同一 SQL 模板在部分租户或时间窗口下计划变差。",
      "慢写语句扫描范围过大，引发锁等待、死锁和复制延迟。",
    ],
    useCases: ["索引命中分析", "JOIN 顺序排查", "慢 SQL 定位", "排序分页优化", "锁范围评估", "计划漂移诊断", "面试准备"],
    prerequisites: ["sql-optimization"],
    related: ["access-type", "extra", "mysql-index", "composite-index", "slow-query-log", "performance-schema", "join-order", "filesort", "temporary-table"],
    order: 58,
  },
  /* <!-- KG_REVIEWED: 访问类型 | 2026-06-05 | source_count=16 --> */
  /* <!-- KG_EXPLAINED: 访问类型 | 2026-05-23 | source_count=5 --> */
  {
    ...
    difficulty: "medium",
    concept:
      "访问类型是 MySQL EXPLAIN 中 `type` 字段展示的表访问方法，表示优化器准备用常量、唯一键、普通索引、范围扫描、索引全扫或表扫描读取候选行。",
    explanation: [
      "概念定位：访问类型（Access Type）解决的是“这张表在当前执行计划里将怎样被读取”的问题。它常出现在慢 SQL 排查、索引评审、JOIN 顺序分析、分页优化、批量写锁范围评估和 MySQL 面试中。\n\n在 `EXPLAIN` 输出里，访问类型对应 `type` 字段。它把优化器选择的表访问方法压缩成一个信号，例如 `const`、`eq_ref`、`ref`、`range`、`index`、`ALL`。新手可以先用它判断查询入口是否精准；有经验的工程师会继续结合 `key`、`key_len`、`rows`、`filtered`、`Extra`、`EXPLAIN ANALYZE` 和线上指标判断真实成本。",
      "准确定义：访问类型是 MySQL 执行计划中单张表的访问方法分类，描述执行器为了获取该表候选行而使用的定位方式。\n\n常见类型可以按读取范围理解：\n\n- `system` / `const`：表最多匹配一行，优化器可把结果当常量处理。\n- `eq_ref`：对前表每一行，通过主键或唯一非空索引在本表精确匹配一行，常见于高质量 JOIN。\n- `ref`：通过普通索引或唯一索引前缀匹配多行，适合等值过滤和非唯一连接。\n- `range`：通过索引读取一个或多个范围，常见于 `BETWEEN`、`IN`、`>`, `<`, 前缀 `LIKE` 和时间区间。\n- `index`：扫描整棵索引，通常按索引顺序读取，成本取决于索引宽度、过滤比例和是否覆盖。\n- `ALL`：扫描整张表，候选行来自全表读取，常见于缺少合适索引、低选择性或优化器认为全扫更便宜的场景。\n\n访问类型展示入口路径，完整计划质量还要看扫描量、回表、排序、临时表、锁和实际耗时。",
      "心智模型：把访问类型看成数据库找行的路线图。\n\n- `const` 像直接按身份证找到唯一档案。\n- `eq_ref` 像拿上一张表给出的唯一编号逐条精确查找。\n- `ref` 像按部门编号找到一组员工。\n- `range` 像按日期区间翻一段有序目录。\n- `index` 像把整本目录从头到尾翻完。\n- `ALL` 像把仓库里每个箱子都打开看一遍。\n\n这个模型能帮助新手迅速理解成本差异：定位越具体，候选行越少；扫描越宽，后续过滤、回表、排序和锁影响越大。",
      "主流程机制：MySQL 产生访问类型大致经历下面步骤。\n\n1. 解析 SQL，识别 `SELECT`、`JOIN`、`WHERE`、`ORDER BY`、`GROUP BY`、`LIMIT` 和返回列。\n2. 预处理常量、等值传播、条件化简、外连接简化、子查询改写和可下推谓词。\n3. 为每张表列出可用索引，结合谓词形态判断候选访问方法，例如唯一查找、普通等值查找、范围访问、索引扫描或表扫描。\n4. 使用表统计、索引基数、直方图、条件选择性和成本模型估算每种方法的 `rows`、`filtered`、I/O、CPU、排序和回表成本。\n5. 在 JOIN 查询中，优化器会把访问类型放进不同 JOIN 顺序的成本评估，前表行数会放大后表访问次数。\n6. 最终计划输出 `type`、`possible_keys`、`key`、`key_len`、`ref`、`rows`、`filtered` 和 `Extra`。\n7. 执行阶段按计划读取数据；`EXPLAIN ANALYZE` 可以补充真实行数、循环次数和耗时，用来校准估算。",
      "实践例子：下面用订单表展示访问类型如何随着谓词和索引变化。\n\n```sql\nCREATE TABLE orders (\n  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,\n  user_id BIGINT UNSIGNED NOT NULL,\n  status VARCHAR(16) NOT NULL,\n  created_at DATETIME NOT NULL,\n  amount DECIMAL(12,2) NOT NULL,\n  PRIMARY KEY (id),\n  KEY idx_user_status_created (user_id, status, created_at),\n  KEY idx_status_created (status, created_at)\n) ENGINE=InnoDB;\n\n-- const：主键等值定位单行\nEXPLAIN FORMAT=TREE\nSELECT id, amount FROM orders WHERE id = 10001;\n\n-- ref：普通联合索引前缀等值匹配一组行\nEXPLAIN\nSELECT id, created_at FROM orders\nWHERE user_id = 1001 AND status = 'PAID';\n\n-- range：在联合索引上做时间范围扫描\nEXPLAIN ANALYZE\nSELECT id, amount FROM orders\nWHERE user_id = 1001\n  AND status = 'PAID'\n  AND created_at >= '2026-06-01 00:00:00'\n  AND created_at <  '2026-07-01 00:00:00';\n\n-- ALL 风险：函数包裹列后候选范围扩大，排查时继续看 rows 与真实耗时\nEXPLAIN\nSELECT id FROM orders\nWHERE DATE(created_at) = '2026-06-01';\n```\n\n同一张表可能在不同 SQL 中呈现不同访问类型。排查时先看 `type` 和 `key`，再看 `rows`、`filtered`、`Extra` 和实际耗时。",
      "深层细节：访问类型只是计划入口，真实质量取决于行数估算和后续操作。\n\n- `const` / `eq_ref`：通常代表非常精准的访问路径，JOIN 中的 `eq_ref` 说明被驱动表通过唯一键查找，循环次数仍取决于前表候选行。\n- `ref`：普通索引等值命中后可能返回很多行，低选择性字段、热点租户和状态列会放大回表与过滤成本。\n- `range`：范围扫描适合时间、数值和有序前缀；范围宽度、边界写法、联合索引列顺序和 ICP 会决定扫描量。\n- `index`：索引全扫可能比表全扫轻，因为索引页更窄或可覆盖返回列；它依然要遍历大量索引记录。\n- `ALL`：全表扫描在小表、统计判断或高返回比例场景可能是合理计划；大表高频路径出现 `ALL` 需要严肃复核。\n- `index_merge`：优化器可把多个单列索引结果合并，适合部分 `OR` 或多条件场景；高频核心 SQL 往往更适合清晰联合索引。\n- 统计信息：`rows` 和 `filtered` 来自估算，热点值、数据倾斜、过期统计和直方图缺失会让访问类型选择漂移。\n- 锁与写入：`UPDATE`、`DELETE`、`SELECT ... FOR UPDATE` 的访问类型会影响扫描范围和锁范围，宽范围扫描会放大锁等待、死锁和复制延迟。",
      "工程场景：访问类型常用于快速判断 SQL 风险和优化方向。\n\n- 索引评审：目标是让高频等值查找稳定走 `const`、`eq_ref` 或 `ref`，让时间窗口走可控 `range`。\n- JOIN 排查：被驱动表出现 `eq_ref` 或高选择性 `ref` 通常更可控；`ALL` 和大量 `Using join buffer` 需要看连接列索引。\n- 列表分页：`range` 配合联合索引和稳定排序能控制扫描；深分页继续看 offset、覆盖索引和 filesort。\n- 批量写入：`range` 或 `ALL` 写语句要按主键分批，控制事务长度、锁范围、Undo/Redo 和 Binlog 压力。\n- 报表查询：`index` 或 `ALL` 可能出现在大范围聚合中，需要放到只读副本、离线任务或预聚合链路评估。\n- 发布评审：新索引上线后要保存优化前后 `EXPLAIN`、`EXPLAIN ANALYZE`、慢日志和 Performance Schema 证据。",
      "边界与故障模式：访问类型判断常见问题来自过度简化。\n\n- 索引低选择性：`ref` 命中状态列后仍扫描大量候选行，`Rows_examined` 和 `Rows_sent` 比例会很高。\n- 范围过宽：`range` 看起来优于全扫，跨月、跨年或大租户范围仍可能读取大量页。\n- 索引全扫误判：`index` 可能因为覆盖索引而可接受，也可能因为全索引遍历导致 CPU 和 I/O 升高。\n- 统计漂移：批量导入、归档、热点参数和数据倾斜会让优化器在 `range`、`index`、`ALL` 之间切换。\n- 函数与隐式转换：字段被函数包裹、参数类型错配、字符集或 collation 差异会改变索引可用性。\n- 排序与临时表：访问类型较好时，`Using filesort`、`Using temporary` 和回表过多仍能拖慢查询。\n- 写语句锁范围：慢 `UPDATE/DELETE` 的 `ALL` 或宽 `range` 会锁住大量记录或间隙，线上表现为锁等待、死锁和复制延迟。",
      "排查实践：访问类型排查建议固定成一份证据包。\n\n1. 固化 SQL：记录 SQL 模板、绑定参数、调用入口、执行次数、耗时、返回行数和实例角色。\n2. 查结构：导出 `SHOW CREATE TABLE`、`SHOW INDEX`，确认字段类型、索引顺序、基数、可见性和主键宽度。\n3. 读计划：用 `EXPLAIN` 看 `type`、`possible_keys`、`key`、`key_len`、`rows`、`filtered`、`Extra`。\n4. 验真实：用 `EXPLAIN ANALYZE` 对比估算行数和实际行数，关注每个迭代器的循环次数和耗时。\n5. 查统计：数据变化大时执行 `ANALYZE TABLE`，必要时用直方图或 `optimizer_trace` 判断计划选择依据。\n6. 查运行：从慢查询日志和 Performance Schema 观察 `Rows_examined`、`Rows_sent`、总耗时、临时表、排序行和锁等待。\n7. 小步修复：对齐参数类型、改写谓词、补联合索引、字段裁剪、拆分 `OR`、缩小时间范围或把大范围任务迁移到离线链路。\n\n```sql\nSHOW CREATE TABLE orders\\G\nSHOW INDEX FROM orders;\n\nEXPLAIN\nSELECT id, amount\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC\nLIMIT 20;\n\nEXPLAIN ANALYZE\nSELECT id, amount\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC\nLIMIT 20;\n\nANALYZE TABLE orders;\n\nSET optimizer_trace='enabled=on';\nSELECT id FROM orders WHERE user_id = 1001 AND status = 'PAID' LIMIT 20;\nSELECT TRACE FROM information_schema.OPTIMIZER_TRACE\\G\nSET optimizer_trace='enabled=off';\n\nSELECT DIGEST_TEXT, COUNT_STAR, SUM_ROWS_EXAMINED, SUM_ROWS_SENT, SUM_TIMER_WAIT\nFROM performance_schema.events_statements_summary_by_digest\nORDER BY SUM_TIMER_WAIT DESC\nLIMIT 10;\n```\n\n有效修复会表现为访问类型更精准、扫描行下降、实际耗时下降、排序和临时表减少、锁等待收敛。",
      "指标与字段速查：阅读访问类型时建议同时看这些字段。\n\n- `type`：访问类型，先判断入口宽窄。\n- `possible_keys`：优化器认为可选的索引集合。\n- `key`：最终选择的索引。\n- `key_len`：索引使用长度，用于辅助判断联合索引参与列。\n- `ref`：用于索引查找的常量或上游表列。\n- `rows`：估算要读取的候选行数。\n- `filtered`：估算通过表条件过滤后的比例。\n- `Extra`：`Using where`、`Using index`、`Using index condition`、`Using temporary`、`Using filesort` 等后续操作信号。\n- `EXPLAIN ANALYZE`：真实行数、循环次数和耗时，适合验证统计偏差。\n- 慢查询日志：`Rows_examined`、`Rows_sent`、`Query_time` 支持线上判断。\n- Performance Schema：`SUM_ROWS_EXAMINED`、`SUM_ROWS_SENT`、`SUM_TIMER_WAIT` 支持按 SQL 指纹治理。",
      "常见误区：访问类型需要放在完整执行证据里解读。\n\n- `type` 排名只是经验排序，最终判断来自扫描行、返回行、回表、排序、临时表、锁和实际耗时。\n- `key` 命中索引后，低选择性、宽范围和热点参数仍会产生大量候选行。\n- `index` 表示全索引扫描，覆盖索引场景可能成本较低，大表高频路径仍要关注扫描量。\n- `ALL` 在小表或高返回比例场景可以是优化器选择的低成本路径，大表高频接口要用证据复核。\n- `EXPLAIN` 给出估算计划，`EXPLAIN ANALYZE` 和线上指标给出真实成本。\n- 新增索引会改变访问类型，也会增加写入、空间、DDL、缓存和复制成本。",
      "面试追问：访问类型题适合按“定义 -> 类型 -> 证据 -> 场景 -> 排查 -> 取舍”回答。\n\n- MySQL `EXPLAIN` 中的 `type` 表示什么，为什么它能帮助判断查询入口？\n- `system`、`const`、`eq_ref`、`ref`、`range`、`index`、`ALL` 分别代表什么访问方式？\n- `eq_ref` 和 `ref` 的关键区别是什么，JOIN 中如何判断被驱动表访问质量？\n- `range` 访问适合哪些谓词，联合索引和范围边界如何影响扫描量？\n- `index` 和 `ALL` 的成本差异怎样判断，覆盖索引会怎样改变判断？\n- 为什么 `type` 好看时 SQL 仍可能很慢，应该继续看哪些字段？\n- 统计信息陈旧、数据倾斜和参数热点如何导致访问类型漂移？\n- 慢 `UPDATE/DELETE` 的访问类型如何影响锁范围、死锁和复制延迟？\n- 如何用 `EXPLAIN ANALYZE`、慢查询日志和 Performance Schema 证明优化有效？\n- 新增联合索引、更新统计信息、改写谓词和使用 hint 各自适合什么场景？",
      "参考来源：本讲解主要参考 MySQL 8.4 Reference Manual 的 EXPLAIN Output、EXPLAIN Statement、Optimizing SELECT Statements、WHERE Clause Optimization、Range Optimization、How MySQL Uses Indexes、Verifying Index Usage、Multiple-Column Indexes、Index Condition Pushdown、Optimizer Statistics、ANALYZE TABLE、Optimizer Trace 和 Performance Schema Statement Tables 文档，并结合 Use The Index, Luke 的 MySQL 执行计划操作说明、JavaGuide 的 MySQL 执行计划分析和小林 coding 的 SELECT 执行流程进行中文表达校准。官方资料用于定义、字段语义、访问方法和观测命令，工程资料用于补充排查路径、索引判断和面试表达。"
    ],
    typicalProblems: [
      "MySQL `EXPLAIN` 的访问类型解决什么问题，和 `type` 字段是什么关系？",
      "`system`、`const`、`eq_ref`、`ref`、`range`、`index`、`ALL` 分别表示怎样的表访问方法？",
      "`eq_ref` 和 `ref` 在 JOIN 中如何区分，为什么唯一索引会影响访问质量？",
      "`range` 访问如何由范围谓词、联合索引顺序和 ICP 共同决定扫描量？",
      "`index` 全索引扫描和 `ALL` 全表扫描分别适合哪些场景，如何判断成本？",
      "`type` 看起来较好时，为什么还要继续看 `rows`、`filtered`、`Extra` 和 `EXPLAIN ANALYZE`？",
      "统计信息陈旧、数据倾斜、热点参数和隐式转换会怎样影响访问类型选择？",
      "慢 `UPDATE/DELETE` 的访问类型如何放大锁范围、死锁概率和复制延迟？",
      "线上如何用慢查询日志、Performance Schema、`EXPLAIN ANALYZE` 和 `optimizer_trace` 排查访问类型异常？",
      "新增索引、改写 WHERE、刷新统计信息、直方图和 hint 如何取舍？"
    ],
    commonCommands: [
      "EXPLAIN <sql>",
      "EXPLAIN FORMAT=TREE <sql>",
    ...
    practiceAdvice: [
      "先把 `type` 当作入口信号，再用 `rows`、`filtered`、`Extra` 和真实执行数据确认成本。",
      "高频等值查询优先设计稳定的主键、唯一键或高选择性联合索引路径。",
      "范围查询使用清晰边界和合适联合索引，持续关注范围宽度与热点参数。",
      "JOIN 排查时把前表候选行数和后表访问类型一起看，计算循环次数带来的乘法成本。",
      "写语句出现宽 `range` 或 `ALL` 时，先评估锁范围、批量大小、事务时长和复制延迟。",
      "优化发布后保留优化前后计划、真实耗时、扫描行和回滚动作。",
    ],
    applicationScenarios: [
      "慢查询定位",
      "索引设计评审",
    ...
    commonIssues: [
      "低选择性索引让 `ref` 访问读取大量候选行。",
      "时间范围过宽让 `range` 扫描接近全量读取。",
      "函数包裹列和隐式转换让可定位谓词变成宽扫描。",
      "`Using filesort`、`Using temporary` 和回表成本掩盖了较好的访问类型。",
      "统计信息陈旧导致优化器在索引扫描和表扫描之间选择漂移。",
      "慢写语句扫描范围过大，引发锁等待、死锁和复制延迟。",
    ],
    useCases: ["全表扫描识别", "索引效率判断", "JOIN 访问路径评估", "慢 SQL 排查", "锁范围评估", "计划漂移诊断", "面试准备"],
    prerequisites: ["explain"],
    related: ["explain", "extra", "mysql-index", "composite-index", "index-selectivity", "range-query", "slow-query-log", "join-order", "performance-schema"],
    order: 59,
  },
  ...
  /* <!-- KG_REVIEWED: 主从复制 | 2026-06-05 | source_count=20 --> */
  /* <!-- KG_EXPLAINED: 主从复制 | 2026-05-23 | source_count=5 --> */
  {
    ...
    area: "replication",
    difficulty: "medium",
    concept: "MySQL 主从复制通过 Binary Log、复制通道和 Relay Log 把主库已提交事务同步到从库，是读扩展、容灾、迁移和高可用切换的基础链路。",
    explanation: [
      "概念定位：主从复制（Source-Replica Replication）解决“一个 MySQL 实例承载全部读写和故障风险”的问题。Source 负责接受写入并生成 `binary log`，Replica 通过复制通道拉取这些事务并重放，业务可以把读流量、备份、报表、迁移和故障切换能力分散到多个实例。\n\n它在真实系统里常出现在四类场景：\n\n- 读写分离：写请求进入 Source，部分读请求进入 Replica，提升读吞吐。\n- 容灾与高可用：Source 故障后，从追赶完整且健康的 Replica 中选择新 Source。\n- 数据同步：把 OLTP 变更传给报表库、异地库、CDC 或审计链路。\n- 运维隔离：在 Replica 上跑备份、校验、慢报表和升级演练，降低主业务实例压力。",
      "准确定义：MySQL 复制是一套基于提交日志的异步或半同步数据传播机制。一次事务在 Source 上提交后，MySQL 把提交事件写入 Binary Log；Replica 的 receiver 线程从 Source 读取日志事件写入 Relay Log；applier 线程按顺序或按依赖关系重放事务，使 Replica 逐步追赶 Source。\n\n核心术语如下：\n\n- `Source`：旧文档常称 `master`，负责接受写入并提供 binary log。\n- `Replica`：旧文档常称 `slave`，负责接收和应用来自 Source 的日志事件。\n- `Binary Log`：Source 侧的提交事件日志，也是复制、PITR 和 CDC 的共同基础。\n- `Relay Log`：Replica 本地保存的待应用日志。\n- `Receiver thread`：旧称 I/O thread，负责从 Source 拉取日志。\n- `Applier thread`：旧称 SQL thread，负责执行 Relay Log 中的事务。\n- `GTID`：全局事务标识，用 `server_uuid:transaction_id` 描述事务身份，简化定位、跳过已执行事务和故障切换。",
      "心智模型：把主从复制想成“提交日志快递系统”。Source 每完成一笔交易，就把收据按提交顺序放进出库流水 `binary log`；Replica 的接收员把收据搬到本地中转仓 `relay log`；执行员再按规则把收据逐笔入账。\n\n这个模型帮助判断三个关键问题：\n\n1. Source 已经提交的事务，Replica 可能仍在运输或待应用队列中。\n2. 复制链路的健康由“能否取到日志、能否写入 relay log、能否应用事务”共同决定。\n3. 读写分离的一致性风险来自 Replica 的追赶窗口，故障切换的数据风险来自最后一次成功传输和应用的位置。",
      "主流程机制：一次典型的异步复制链路按下面步骤运行。\n\n1. 业务在 Source 上执行事务，InnoDB 完成事务提交，Server 层把事务事件写入 `binary log`。\n2. Source 的 dump 线程把 binary log 事件发送给 Replica 的 receiver 线程。\n3. Replica receiver 把收到的事件写入本地 `relay log`，并推进已接收位置或 GTID 集合。\n4. Replica applier 读取 relay log，把事务在本地重放，推进已执行位置或 `Executed_Gtid_Set`。\n5. 监控系统通过 `SHOW REPLICA STATUS`、Performance Schema replication 表和业务校验确认接收、应用、延迟与错误状态。\n\n```text\nClient COMMIT\n  -> Source InnoDB + Binary Log\n  -> Source dump thread\n  -> Replica receiver thread\n  -> Relay Log\n  -> Replica applier thread(s)\n  -> Replica data + Executed_Gtid_Set\n```\n\n复制的输出是 Replica 上逐步收敛的数据副本。下游读请求、备份任务、报表任务和切换控制器都依赖这个副本的完整性、延迟和错误状态。",
      "配置示例：生产配置会围绕 server identity、binary log、GTID、复制账号、源端定位和安全连接展开。下面示例表达主路径，真实环境还要接入 TLS、最小权限账号、参数基线和变更审计。\n\n```ini\n# source my.cnf\n[mysqld]\nserver_id=101\nlog_bin=mysql-bin\nbinlog_format=ROW\ngtid_mode=ON\nenforce_gtid_consistency=ON\nsync_binlog=1\n\n# replica my.cnf\n[mysqld]\nserver_id=102\nrelay_log=mysql-relay-bin\ngtid_mode=ON\nenforce_gtid_consistency=ON\nread_only=ON\nsuper_read_only=ON\n```\n\n```sql\n-- Source: 创建复制账号，按环境收敛到固定网段和最小权限\nCREATE USER 'repl'@'10.%' IDENTIFIED BY 'strong-password';\nGRANT REPLICATION SLAVE ON *.* TO 'repl'@'10.%';\n\n-- Replica: 使用 GTID 自动定位连接 Source\nCHANGE REPLICATION SOURCE TO\n  SOURCE_HOST = '10.0.0.11',\n  SOURCE_PORT = 3306,\n  SOURCE_USER = 'repl',\n  SOURCE_PASSWORD = 'strong-password',\n  SOURCE_AUTO_POSITION = 1;\n\nSTART REPLICA;\nSHOW REPLICA STATUS\\G\n```\n\n`binlog_format=ROW` 在复杂 SQL、触发器、函数和数据漂移排查中更可预期；`read_only` 与 `super_read_only` 用来保护 Replica 角色边界；`sync_binlog`、`innodb_flush_log_at_trx_commit` 和半同步参数共同影响提交成本与故障窗口。",
      "深层机制：复制质量取决于日志格式、事务边界、依赖关系和应用并行度。\n\n- 日志格式：Statement 记录 SQL，Row 记录行变化，Mixed 在部分场景切换格式。行格式通常更利于确定性重放和数据一致性排查。\n- 提交顺序：Binary Log 记录 Source 可见提交顺序，GTID 给每个事务稳定身份；故障恢复和新主选择要看已接收与已执行集合。\n- Relay Log：Relay Log 是 Replica 的本地缓冲区，receiver 正常但 applier 慢时会产生积压。\n- 多线程应用：Replica 可以用多 worker 应用事务，吞吐提升依赖事务间依赖关系、库表热点和提交顺序约束。\n- 半同步复制：Source 在提交路径上等待至少一个 Replica 确认收到事务事件，能缩小故障窗口，同时会增加写入尾延迟和跨机房抖动敏感度。\n- 过滤规则：库表过滤会改变 Replica 数据范围，适合专用副本和迁移场景，排查时要把过滤规则纳入证据链。",
      "工程场景：主从复制的架构收益来自把不同职责拆到不同实例，代价也来自这条异步链路。\n\n- 高读流量：热点读、后台列表、搜索辅助查询可以走 Replica，强一致读继续走 Source 或加读后写保护。\n- 报表与审计：Replica 承接长 SQL、抽样校验和导出任务，避免拖慢 Source 的前台事务。\n- 备份恢复：在 Replica 上做逻辑备份、物理备份和恢复演练，关注备份开始时的 GTID 或 binlog position。\n- 灰度迁移：新版本 MySQL 或新机型先作为 Replica 追赶，校验一致后再切换写入口。\n- 故障切换：提升 Replica 前要确认 `Retrieved_Gtid_Set`、`Executed_Gtid_Set`、relay log 积压、复制错误和业务写入入口冻结状态。",
      "边界与故障：主从复制的故障通常能归入“取日志、写中继、应用事务、读一致性、角色切换”五类。\n\n- 取日志异常：网络抖动、DNS、账号权限、TLS、Source 连接数、binlog 被清理都会让 receiver 停止推进。\n- 写中继异常：Replica 磁盘空间、文件权限、I/O 抖动和 relay log 损坏会造成接收侧积压。\n- 应用异常：主键冲突、DDL 顺序、字符集差异、复制过滤、外部写入 Replica 会让 applier 报错。\n- 延迟扩大：大事务、热点行、长事务、DDL、Replica 硬件弱、并行度偏低和跨机房链路会扩大追赶窗口。\n- 读一致性风险：用户刚写完再读 Replica，可能读到旧版本；业务可用写后读 Source、会话粘滞、GTID 等待或版本号校验治理。\n- 切换风险：提升落后的 Replica 会扩大数据缺口；提升后旧 Source 恢复要经过隔离、重建或以新 Source 为准重新加入。",
      "排查实践：线上复制问题按“身份、链路、位置、队列、错误、业务影响”建立证据链。\n\n1. 确认拓扑：记录 Source、Replica、server_id、server_uuid、版本、机房、复制账号和当前写入口。\n2. 看线程状态：用 `SHOW REPLICA STATUS\\G` 读取 receiver、applier、错误码、Source 文件位置、Relay 文件位置和延迟。\n3. 看 GTID 差距：对比 `Retrieved_Gtid_Set`、`Executed_Gtid_Set` 和 Source 的 `gtid_executed`。\n4. 看 Performance Schema：定位连接状态、worker 错误、事务应用耗时和 worker 队列。\n5. 看资源瓶颈：检查 CPU、磁盘 I/O、网络吞吐、fsync 延迟、relay log 大小、锁等待和大事务。\n6. 看业务影响：抽样校验关键表版本号、订单状态、库存流水和读写分离路由。\n7. 小步恢复：先恢复接收和应用链路，再处理数据一致性；切换动作前冻结写入口并保留证据快照。\n\n```sql\nSHOW REPLICA STATUS\\G\n\nSELECT @@server_id, @@server_uuid, @@gtid_executed;\nSHOW BINARY LOG STATUS;\n\nSELECT *\nFROM performance_schema.replication_connection_status\\G\n\nSELECT *\nFROM performance_schema.replication_applier_status_by_worker\\G\n\nSELECT CHANNEL_NAME, SERVICE_STATE, LAST_ERROR_NUMBER, LAST_ERROR_MESSAGE\nFROM performance_schema.replication_applier_status\\G\n\nSHOW VARIABLES LIKE 'binlog_format';\nSHOW VARIABLES LIKE 'gtid_mode';\nSHOW VARIABLES LIKE 'replica_parallel_workers';\n```\n\n常用判断信号包括：`Replica_IO_Running`、`Replica_SQL_Running`、`Seconds_Behind_Source`、`Source_Log_File`、`Read_Source_Log_Pos`、`Relay_Log_File`、`Relay_Log_Pos`、`Retrieved_Gtid_Set`、`Executed_Gtid_Set`、`Last_IO_Error`、`Last_SQL_Error` 和 worker 级错误。",
      "性能与安全取舍：复制把写入后的传播能力交给日志系统，也引入新的成本面。\n\n- 写入延迟：`sync_binlog=1`、半同步等待、跨机房网络和 binlog 压缩都会改变提交路径的尾延迟。\n- 应用吞吐：大事务和热点表会让 applier 串行化，拆分事务、提升并行 worker、优化索引和控制 DDL 窗口更有效。\n- 存储成本：binary log、relay log 和历史保留策略要覆盖恢复窗口，同时控制磁盘水位和清理节奏。\n- 安全边界：复制账号应使用固定来源、最小权限、强密码或证书；跨网络复制应启用 TLS 并审计 `CHANGE REPLICATION SOURCE TO` 变更。\n- 数据治理：Replica 上的本地写入、手工跳过事务和过滤规则会影响一致性审计，操作前要记录 GTID、SQL、审批和回滚方案。",
      "常见误区：主从复制的正确心智模型是“日志驱动的副本追赶系统”。\n\n- 复制承担在线冗余、读扩展和迁移通道；备份承担历史恢复、误删恢复和审计留存。\n- 异步复制提供最终追赶语义，强一致读需要路由策略、GTID 等待或业务版本校验支撑。\n- 半同步复制缩小已提交事务的丢失窗口，提交延迟和故障切换流程仍需要独立评估。\n- `Seconds_Behind_Source` 是延迟线索，完整判断还要结合 GTID 差距、relay backlog、worker 状态和业务版本。\n- 多线程复制提升应用吞吐，热点事务、DDL 和提交顺序约束仍会形成瓶颈。\n- 故障切换的核心证据是候选 Replica 的已接收、已执行、错误状态和业务写入口冻结结果。",
      "面试追问：主从复制题适合按“定义 -> 链路 -> 一致性 -> 排查 -> 切换取舍”回答。\n\n- MySQL 主从复制解决什么问题，Source、Replica、Binary Log 和 Relay Log 分别承担什么职责？\n- 一次事务从 Source 提交到 Replica 应用完整经历哪些步骤？\n- receiver 线程、applier 线程、dump 线程各自负责什么？\n- Statement、Row、Mixed 三种 binlog format 对复制确定性和排查有什么影响？\n- GTID 如何简化复制定位、故障恢复和主从切换？\n- 异步复制、半同步复制在写入延迟和数据风险上如何取舍？\n- 读写分离场景如何处理写后读一致性？\n- 复制延迟的常见原因有哪些，如何用 `SHOW REPLICA STATUS` 和 Performance Schema 定位？\n- Replica applier 报错时怎样建立证据、判断影响范围并恢复？\n- 主库故障后选择候选 Replica 时要看哪些指标和状态？",
      "参考来源：本讲解主要参考 MySQL 8.4 Reference Manual 的 Binary Log、Binary Logging Options、Source to Replica Replication、Replication Implementation Details、Replication Threads、Binary Logging Formats、Setting Up Replicas、Replication Source Options、CHANGE REPLICATION SOURCE TO、START REPLICA、Replica Logs and Status、SHOW REPLICA STATUS、Performance Schema Replication Tables、Semisynchronous Replication 和 GTID Life Cycle 文档，并结合 Percona 的 MySQL replication architecture 图解、小林 coding 图解 MySQL、JavaGuide 与 CS-Notes 校准中文表达、面试问法和工程排查主线。官方资料用于定义、状态字段、命令和机制；技术文章用于补充架构图心智模型与实践表达。"
    ],
    typicalProblems: [
      "MySQL 主从复制解决什么问题，Source 和 Replica 的职责是什么？",
      "Binary Log、Relay Log、receiver thread、applier thread 在复制链路中分别承担什么角色？",
      "一次事务从主库提交到从库应用经历哪些步骤？",
      "Statement、Row、Mixed 三种 binlog format 对复制一致性和排查有什么影响？",
      "GTID 如何简化复制定位、故障恢复和主从切换？",
      "异步复制、半同步复制在写入延迟、故障窗口和可用性上如何取舍？",
      "读写分离下用户写完立刻读取，怎样保证读到最新业务状态？",
      "复制延迟常见原因有哪些，如何区分 receiver 慢、relay log 积压和 applier 慢？",
      "如何用 `SHOW REPLICA STATUS`、Performance Schema replication 表和 GTID 集合排查复制故障？",
      "主库故障切换时，如何选择候选 Replica 并控制数据风险？",
      "复制账号、TLS、只读参数、binlog 保留策略分别有哪些生产注意点？"
    ],
    commonCommands: [
      "SHOW REPLICA STATUS\\G",
      "SHOW BINARY LOG STATUS",
      ...
      "SELECT CHANNEL_NAME, SERVICE_STATE, LAST_ERROR_NUMBER, LAST_ERROR_MESSAGE FROM performance_schema.replication_applier_status\\G",
      "SHOW VARIABLES LIKE 'binlog_format'",
      "SHOW VARIABLES LIKE 'gtid_mode'",
      "SHOW VARIABLES LIKE 'replica_parallel_workers'",
      "CHANGE REPLICATION SOURCE TO SOURCE_HOST='...', SOURCE_USER='...', SOURCE_PASSWORD='...', SOURCE_AUTO_POSITION=1",
      "START REPLICA",
      "STOP REPLICA"
    ],
    applicationScenarios: ["读写分离", "容灾高可用", "报表库同步", "备份隔离", "跨机房迁移", "CDC 数据链路"],
    practiceAdvice: [
      "优先启用 GTID、行格式 binlog、固定 server_id/server_uuid 资产登记和复制状态监控。",
      "为强一致读设计明确路由策略，例如写后读 Source、会话粘滞、GTID 等待或业务版本校验。",
      "把 `SHOW REPLICA STATUS`、Performance Schema replication 表、GTID 差距、relay log 大小和业务抽样校验放进同一张排查表。",
      "故障切换前冻结写入口，记录候选 Replica 的已接收、已执行、错误状态和 relay backlog。",
      "复制账号使用最小权限、固定来源和安全传输，变更复制源时保留审批与审计记录。"
    ],
    commonIssues: ["复制延迟", "Relay Log 积压", "applier 报错", "binlog 保留窗口不足", "读写分离读旧数据", "故障切换数据窗口", "复制账号权限或网络异常"],
    useCases: ["读写分离", "灾备", "数据同步", "报表隔离", "迁移切换", "备份演练"],
    prerequisites: ["binlog"],
    related: ["replication-lag", "read-write-splitting", "gtid", "failover", "backup-restore", "point-in-time-recovery"],
    order: 65,
  },
  /* <!-- KG_REVIEWED: 读写分离 | 2026-06-05 | source_count=20 --> */
  /* <!-- KG_EXPLAINED: 读写分离 | 2026-05-23 | source_count=5 --> */
  {
    ...
    area: "replication",
    difficulty: "medium",
    concept: "读写分离把写请求稳定落到 Source，把可容忍异步可见性的读请求分发到 Replica，用复制体系扩展读吞吐并隔离报表、检索和后台读流量。",
    explanation: [
      "概念定位：读写分离（Read Write Splitting）解决的是 OLTP 系统读流量持续放大后的承载问题。典型订单、内容、用户中心和后台查询链路里，写入仍由 Source 承担，读请求按策略落到一个或多个 Replica，从而把热点详情、列表页、报表和审计查询从写主链路中移开。\n\n它依赖 MySQL 复制：Source 提交事务并写入 binary log，Replica 拉取并应用 relay log；路由层根据 SQL 类型、事务状态、会话标记、复制健康和业务一致性要求选择目标节点。",
      "准确定义：在 MySQL 语境里，读写分离通常由应用侧数据源、代理层或 MySQL Router、ProxySQL、ShardingSphere 等中间件完成。\n\n- `write`：`INSERT`、`UPDATE`、`DELETE`、DDL、显式加锁读、强一致读、事务内读写链路，路由到 Source。\n- `read`：普通 `SELECT`、报表查询、后台列表、只读接口，按负载均衡策略路由到 Replica。\n- `Replica`：通过 receiver/applier 线程追赶 Source 的异步副本，健康标准包括连接正常、应用正常、延迟在阈值内、角色只读、数据校验符合业务要求。\n- `read_only` 与 `super_read_only`：用于保护副本角色，降低误写入副本的风险。",
      "心智模型：把读写分离看成“一个写入口、多条读通道、一个一致性闸门”。\n\n1. 写入口负责产生权威顺序：事务提交、binlog 顺序、GTID 集合和业务版本号都从 Source 出发。\n2. 读通道负责承接吞吐：Replica 可以横向增加，读请求通过轮询、权重、延迟阈值或地域就近策略分摊。\n3. 一致性闸门负责判断读哪里：刚写完订单、余额、库存、权限这种强一致场景走 Source 或等待指定 GTID；搜索、推荐、报表、日志这种异步可见场景走 Replica。\n\n新手先记住：读写分离提升读侧容量，复制延迟决定读到的数据新鲜度，路由策略决定线上体验。",
      "主流程机制：一次请求进入读写分离链路后，系统会经历“分类、路由、执行、观测、降级”五步。\n\n1. SQL 分类：代理或应用识别 `SELECT`、DML、DDL、事务语句、锁定读、存储过程和多语句请求。\n2. 会话判断：显式事务、`LAST_INSERT_ID()`、临时表、用户变量、写后读标记和强一致接口会让请求粘到 Source。\n3. 节点筛选：从 Replica 池里剔除连接异常、复制线程异常、`Seconds_Behind_Source` 超阈值、只读保护异常和权重为 0 的节点。\n4. 执行与返回：读请求在候选 Replica 上执行；写请求在 Source 上提交并产生 binlog、GTID 和业务版本。\n5. 反馈闭环：监控复制延迟、错误码、QPS、连接数、慢查询、业务读旧数据样本，并动态调整权重、摘除节点或切回 Source。\n\n这个流程的关键约束是“路由正确性先于负载均衡”。节点再多，强一致请求也要进入能证明数据新鲜度的路径。",
      "配置示例：生产环境可以从“复制角色保护 + 路由规则 + 延迟阈值”三层落地。下面示例表达常见形态，真实环境还要配合 TLS、最小权限账号、连接池、监控告警和变更审计。\n\n```ini\n# Replica my.cnf\n[mysqld]\nserver_id=102\nrelay_log=mysql-relay-bin\nread_only=ON\nsuper_read_only=ON\ngtid_mode=ON\nenforce_gtid_consistency=ON\n```\n\n```yaml\n# ShardingSphere 读写分离规则示意\nrules:\n- !READWRITE_SPLITTING\n  dataSources:\n    rw_ds:\n      staticStrategy:\n        writeDataSourceName: primary_ds\n        readDataSourceNames:\n          - replica_ds_0\n          - replica_ds_1\n      loadBalancerName: round_robin\n  loadBalancers:\n    round_robin:\n      type: ROUND_ROBIN\n```\n\n```sql\n-- ProxySQL 规则示意：写流量到 hostgroup 10，普通读流量到 hostgroup 20\nINSERT INTO mysql_query_rules(rule_id, active, match_digest, destination_hostgroup, apply)\nVALUES\n  (10, 1, '^SELECT .* FOR UPDATE', 10, 1),\n  (20, 1, '^SELECT', 20, 1),\n  (30, 1, '^(INSERT|UPDATE|DELETE|REPLACE|CREATE|ALTER|DROP)', 10, 1);\n\nLOAD MYSQL QUERY RULES TO RUNTIME;\nSAVE MYSQL QUERY RULES TO DISK;\n```\n\n规则顺序要覆盖锁定读、事务内读、写后读和管理语句；Replica 的 `read_only`、`super_read_only` 与代理健康检查共同构成副本保护线。",
      "一致性设计：读写分离的工程难点集中在“写后读”和“读到旧版本”。常用策略按成本从低到高排列：\n\n- 业务容忍异步：列表页、推荐、报表、搜索索引等读取 Replica，并把可见性延迟作为产品语义的一部分。\n- 会话粘滞：同一用户完成写入后，在短时间窗口内把该用户读请求路由到 Source。\n- 版本校验：写入时返回 `updated_at`、版本号或订单状态版本，读取时发现 Replica 版本落后就重试 Source。\n- GTID 等待：写入后记录 GTID，在读 Replica 前等待该 GTID 被执行，适合关键链路的小范围强一致读。\n- 强一致直连 Source：余额、库存扣减结果、权限变更、支付状态确认等链路直接读 Source。\n\n半同步复制可以缩小提交后事务丢失窗口，读到最新数据仍要结合应用路径、Replica 应用进度和路由策略判断。",
      "性能与容量：读写分离的收益来自读请求横向扩展，成本来自复制、路由和多副本维护。\n\n- 吞吐收益：读多写少系统可以通过增加 Replica 承载详情页、报表和后台查询。\n- 写侧瓶颈：所有写入仍进入 Source，写热点、锁冲突、redo/binlog fsync 和大事务仍会决定主链路上限。\n- 延迟成本：大事务、慢 SQL、DDL、网络抖动、磁盘 I/O 饱和和单线程应用瓶颈会放大复制延迟。\n- 路由成本：代理层增加一次网络跳转和连接复用复杂度，应用侧多数据源增加代码治理成本。\n- 运维成本：副本越多，版本升级、参数一致性、账号权限、备份策略、数据校验和故障演练的治理面越大。",
      "边界与故障：线上读写分离故障通常表现为读旧数据、误写副本、读流量压垮副本、切换后路由错误和连接池缓存旧地址。\n\n- 读旧数据：`Seconds_Behind_Source` 升高、GTID 差距扩大、业务版本回退、用户刚提交后列表缺少新记录。\n- 副本误写：Replica 的 `read_only` 或 `super_read_only` 保护异常，账号权限过大，代理规则把 DML 打到读节点。\n- 副本过载：报表 SQL、缺索引查询、全表扫描和大结果集让 Replica SQL 线程追赶速度下降。\n- 切换窗口：主从切换后代理、DNS、连接池和应用配置需要同时刷新，旧 Source 需要进入只读保护。\n- 事务语义：事务内读写分散到不同节点会破坏会话语义，显式事务应固定到 Source 或同一连接上下文。",
      "排查实践：遇到“读写分离疑似异常”，按请求证据、路由证据、复制证据、业务证据四层收敛。\n\n1. 定位请求：记录用户、接口、SQL digest、trace id、写入时间、读取时间和读到的数据版本。\n2. 确认路由：查看应用数据源日志、代理 query rule 命中、目标 hostgroup、连接池实例和节点地址。\n3. 查看复制：检查 receiver/applier 状态、GTID 集合、relay log 队列、最近错误和延迟指标。\n4. 验证业务：在 Source 与 Replica 分别按主键查询，比较状态、版本号、`updated_at` 和关键流水。\n5. 执行处置：强一致接口切 Source，摘除延迟副本，终止副本大查询，修复缺索引 SQL，恢复后逐步放回流量。\n\n```sql\nSHOW REPLICA STATUS\\G\n\nSELECT @@hostname, @@server_id, @@read_only, @@super_read_only, @@gtid_executed;\n\nSELECT *\nFROM performance_schema.replication_connection_status\\G\n\nSELECT *\nFROM performance_schema.replication_applier_status_by_worker\\G\n\nSELECT event_name, count_star, sum_timer_wait\nFROM performance_schema.events_statements_summary_by_digest\nORDER BY sum_timer_wait DESC\nLIMIT 10;\n\n-- 在 Source 和 Replica 上分别验证同一行版本\nSELECT id, status, version, updated_at\nFROM orders\nWHERE id = 123456;\n```\n\n核心指标包括 `Seconds_Behind_Source`、`Retrieved_Gtid_Set`、`Executed_Gtid_Set`、`Replica_IO_Running`、`Replica_SQL_Running`、`Last_SQL_Error`、副本慢查询、代理后端状态、连接池活跃连接和业务读旧样本数。",
      "常见误区：正确的设计以数据新鲜度分级为中心。读写分离适合读侧扩容、报表隔离和只读查询治理；强一致读、事务内读、锁定读和写后立刻校验的路径需要清晰的 Source 路由或 GTID 等待策略。节点健康应同时看复制线程、延迟阈值、业务版本校验和查询负载，单个时间延迟值只能作为入口信号。",
      "参考来源：本文主要参考 MySQL 8.4 Reference Manual 的复制实现、Scale-Out、SHOW REPLICA STATUS、Performance Schema replication 表、`read_only`/`super_read_only` 变量和半同步复制说明，结合 MySQL Router 8.4 读写分离配置、ShardingSphere 读写分离规则、ProxySQL 读写分离与后端监控文档、Percona 复制架构文章，以及小林 coding、JavaGuide、CS-Notes 的中文 MySQL 学习资料。"
    ],
    typicalProblems: [
      "读写分离解决什么问题，和 MySQL 复制是什么关系？",
      "代理层如何判断一条 SQL 应该走 Source 还是 Replica？",
      "写后立刻读取订单详情时，如何保证读到最新状态？",
      "复制延迟升高后，读写分离链路应该如何降级？",
      "事务内读写、`SELECT ... FOR UPDATE`、临时表和用户变量为什么需要特殊路由？",
      "如何用 `SHOW REPLICA STATUS`、Performance Schema 和业务版本号定位读旧数据？",
      "读写分离能带来哪些性能收益，又会增加哪些一致性和运维成本？",
      "主从切换后，连接池、代理规则和副本只读保护应该如何验证？",
      "半同步复制、GTID 等待和会话粘滞分别适合哪些一致性场景？",
      "面试追问：如果 Replica SQL 线程被一条报表 SQL 拖慢，你会如何证明和恢复？"
    ],
    applicationScenarios: [
      "高读流量接口扩容",
      "订单详情和列表读取",
    ...
    practiceAdvice: [
      "为每个接口标注一致性等级，把强一致读、写后读和异步可见读分开路由。",
      "Replica 启用 `read_only` 与 `super_read_only`，复制账号和业务账号按最小权限配置。",
      "代理规则优先匹配事务、锁定读、写语句和管理语句，再匹配普通 `SELECT`。",
      "把复制延迟阈值、GTID 差距、代理后端状态、连接池目标地址和业务读旧样本放进同一张监控面板。",
      "主从切换演练覆盖旧 Source 只读保护、新 Source 写入验证、Replica 重新挂载、代理规则刷新和连接池重连。"
    ],
    commonIssues: [
      "复制延迟导致用户写后读到旧版本。",
      "代理 SQL 规则过宽导致锁定读或事务内读进入 Replica。",
      "Replica 只读保护缺失导致副本产生本地写入。",
      "报表慢查询压垮 Replica 并拖慢复制应用。",
      "主从切换后连接池继续持有旧 Source 连接。",
      "GTID 集合差距扩大导致候选副本缺少事务。",
      "多机房读路由在网络抖动时放大尾延迟。"
    ],
    commonCommands: [
      "SHOW REPLICA STATUS\\G",
    ...
    useCases: ["高读流量系统", "报表查询隔离", "写后读一致性治理", "复制延迟排查", "数据库高可用演练"],
    prerequisites: ["replication"],
    related: ["replication-lag", "gtid", "failover", "connection-pool", "slow-query-log"],
    order: 66,
  },
  /* <!-- KG_REVIEWED: 复制延迟 | 2026-06-05 | source_count=18 --> */
  /* <!-- KG_EXPLAINED: 复制延迟 | 2026-05-23 | source_count=5 --> */
  {
    ...
    area: "replication",
    difficulty: "medium",
    concept: "复制延迟表示 Replica 已接收或已执行的事务进度落后 Source，影响读写分离新鲜度、故障切换窗口、备份口径和下游数据链路。",
    explanation: [
      "概念定位：复制延迟（Replication Lag）解决的是“副本数据到底追到哪里”的判断问题。MySQL 主从架构把写入提交到 Source，Replica 异步接收 binary log、写入 relay log，再由 applier 线程回放事务；任何一个环节变慢都会让副本落后。\n\n它出现在读写分离读旧数据、报表库数据滞后、主从切换候选选择、备份恢复窗口、CDC 延迟、跨地域灾备和面试题里。新手先理解“Source 已提交、Replica 还在路上”；有经验的工程师还要区分传输延迟、回放延迟、时间指标偏差、GTID 差距和业务可见性。",
      "准确定义：MySQL 复制延迟可以从时间、事务集合和日志位点三个维度描述。\n\n- 时间维度：`SHOW REPLICA STATUS` 中的 `Seconds_Behind_Source` 表达 Replica SQL 线程相对 Source 事件时间戳的落后程度，适合作为告警入口。\n- 事务维度：`Retrieved_Gtid_Set` 表示已接收事务集合，`Executed_Gtid_Set` 表示已执行事务集合，二者差距能表达 relay backlog。\n- 位点维度：Source binary log file/position、Replica relay log file/position 和执行位点说明日志移动到哪里。\n- 业务维度：订单版本、`updated_at`、流水号、binlog 心跳表和 pt-heartbeat 这类外部心跳能直接证明业务读到的新鲜度。\n\n生产判断通常把这些维度放在一起看：时间指标负责发现问题，GTID/位点负责定位链路，业务心跳负责校准用户可见结果。",
      "心智模型：把复制链路看成三段流水线。\n\n1. Source 提交段：事务在 InnoDB 中提交，并按提交顺序写入 binary log，组提交、大事务、DDL 和磁盘 fsync 会影响日志产生节奏。\n2. 传输段：Replica receiver 线程连接 Source，读取 binary log，写入本地 relay log；网络抖动、Source dump 线程压力和 relay log 磁盘写入会造成接收落后。\n3. 回放段：Replica coordinator/worker 读取 relay log，把事务应用到 InnoDB；慢 SQL、缺索引、锁等待、单 worker 瓶颈、并行依赖和磁盘 I/O 会造成执行落后。\n\n排查复制延迟的第一步是判断“日志没有及时拉到”还是“日志拉到了但应用变慢”。",
      "主流程机制：一次写入从 Source 到 Replica 可见，按以下过程推进。\n\n1. 客户端在 Source 提交事务，InnoDB 更新数据页与日志，MySQL 写入 binlog，并生成 file/position 或 GTID。\n2. Source 的 dump 线程把 binlog event 提供给 Replica receiver。\n3. Replica receiver 把 event 写入 relay log，并更新接收状态。\n4. Replica coordinator 读取 relay log，根据事务依赖和 `replica_parallel_workers` 分发给 worker。\n5. Worker 在 Replica 上执行 DML、DDL 或事务提交，更新 `Executed_Gtid_Set`、relay log 执行位点和 Performance Schema 状态。\n6. 读写分离层或业务查询读到 Replica 上的最新已执行版本。\n\n延迟产生在任意步骤：Source 写入太猛、网络传输慢、relay log 积压、worker 回放慢、Replica 上只读查询争抢 CPU/I/O、DDL 或大事务阻塞，都可能让用户读到旧版本。",
      "观测指标：复制延迟排查要把 MySQL 内部状态、系统资源和业务证据合并成一张表。\n\n- 线程状态：`Replica_IO_Running` / `Replica_SQL_Running`、`Last_IO_Error`、`Last_SQL_Error`、receiver 与 applier 是否持续工作。\n- 延迟入口：`Seconds_Behind_Source`、云厂商 `ReplicaLag`、pt-heartbeat 延迟、业务心跳表延迟。\n- 日志差距：`Source_Log_File`、`Read_Source_Log_Pos`、`Relay_Log_File`、`Relay_Log_Pos`、`Exec_Source_Log_Pos`。\n- GTID 差距：`Retrieved_Gtid_Set` 与 `Executed_Gtid_Set` 的差异、候选切换节点缺失事务集合。\n- Worker 证据：`performance_schema.replication_applier_status_by_worker` 里的 `LAST_APPLIED_TRANSACTION`、错误、等待和各 worker 进度。\n- 资源指标：Replica CPU、磁盘 await、IOPS、redo/binlog 写入、Buffer Pool 命中、锁等待、慢查询和长事务。",
      "实践示例：下面的证据包适合复制延迟告警后的第一轮定位。\n\n```sql\nSHOW REPLICA STATUS\\G\n\nSELECT *\nFROM performance_schema.replication_connection_status\\G\n\nSELECT *\nFROM performance_schema.replication_applier_status\\G\n\nSELECT WORKER_ID, THREAD_ID, SERVICE_STATE,\n       LAST_APPLIED_TRANSACTION,\n       LAST_APPLIED_TRANSACTION_END_APPLY_TIMESTAMP,\n       LAST_ERROR_NUMBER, LAST_ERROR_MESSAGE\nFROM performance_schema.replication_applier_status_by_worker\\G\n\nSELECT @@hostname, @@server_id, @@read_only, @@super_read_only,\n       @@gtid_mode, @@replica_parallel_workers;\n\nSELECT @@GLOBAL.gtid_executed;\n\n-- 业务心跳表示例：Source 每秒写一次，Replica 读取差值\nSELECT TIMESTAMPDIFF(SECOND, ts, NOW()) AS heartbeat_lag_seconds\nFROM ops.replication_heartbeat\nWHERE id = 1;\n```\n\n如果 `Retrieved_Gtid_Set` 接近 Source 但 `Executed_Gtid_Set` 明显落后，重点看 applier、worker、锁和 I/O；如果接收位点本身落后，重点看网络、Source dump 线程、复制连接和 relay log 写入。",
      "深层细节与性能取舍：复制延迟的根因通常来自“写入速度、回放速度、依赖关系和副本负载”的差距。\n\n- 大事务：单个事务包含大量行变更时，Replica 需要完整回放后才推进可见进度，回滚和重试成本也高。\n- DDL 与元数据锁：表结构变更、在线 DDL、长事务和只读查询可能形成等待链，复制 SQL 线程停在某个事件上。\n- 并行复制：`replica_parallel_workers` 可以提升独立事务回放能力，但同一热点库表、相同写集合或提交依赖会限制并行度。\n- 无主键表：Row-based 复制回放更新或删除时需要定位行，缺少合适主键或唯一键会显著拖慢副本应用。\n- 副本读负载：报表、全表扫描、临时表和排序会抢占 Buffer Pool、CPU 和磁盘 I/O，让 applier 追赶速度下降。\n- 半同步复制：半同步能让 Source 等待至少一个副本确认接收事务，Replica 应用完成与读可见仍取决于 applier 进度。\n- 跨地域复制：网络 RTT、带宽抖动和链路丢包会放大传输段延迟，业务需要把新鲜度 SLA 写清楚。",
      "工程场景：复制延迟的处理方式取决于业务一致性等级。\n\n- 订单写后读：写入返回后短窗口读 Source，或记录 GTID/业务版本，等待指定副本追上再读。\n- 报表副本：允许分钟级延迟，把慢报表和主链路隔离，并设置延迟阈值避免报表口径失真。\n- 主从切换：选择候选节点时看 `Executed_Gtid_Set`、错误状态、relay backlog 和业务校验，切换前尽量冻结写入口。\n- 备份副本：备份前确认复制稳定、延迟在窗口内，避免拿到过旧快照。\n- CDC 与搜索索引：复制延迟会传递到下游消费者，告警要串联 MySQL、采集器 offset、消息队列积压和业务索引可见时间。\n- 大促削峰：写入峰值前预估 binlog 量、Replica I/O、并行 worker 和读流量隔离能力，延迟阈值触发读路由降级。",
      "边界与故障模式：线上复制延迟常见表现有读旧数据、延迟突然跳高、延迟长期缓慢扩大、复制线程停止、候选副本事务缺口和延迟指标失真。\n\n- 指标失真：服务器时钟漂移、长事务事件时间戳、复制线程停止和心跳缺失会让单一秒级指标误导判断。\n- 线程停止：网络错误、权限变更、relay log 损坏、重复键冲突、DDL 失败和数据漂移会让 receiver 或 applier 停止。\n- 回放卡点：`Last_SQL_Error`、worker 最近事务、锁等待、慢 SQL 和表结构变更通常能定位具体事件。\n- 延迟放大：Source 批量写入速度持续高于 Replica 回放速度，relay log backlog 会持续扩大。\n- 读写分离风险：延迟副本继续承接强一致读，会造成用户刚写入后读到旧状态。\n- 切换风险：提升落后副本可能丢失已提交事务，GTID 集合和业务对账需要成为切换门槛。",
      "排查实践：建议按“确认延迟 -> 分段定位 -> 找阻塞点 -> 降级止血 -> 修复复测”执行。\n\n1. 确认告警真实性：同时看 `Seconds_Behind_Source`、pt-heartbeat 或业务心跳、Source/Replica 时钟、业务读旧样本。\n2. 分清接收与回放：对比 Source binlog 位点、Replica read 位点、relay 执行位点、`Retrieved_Gtid_Set` 与 `Executed_Gtid_Set`。\n3. 查复制错误：读取 `Last_IO_Error`、`Last_SQL_Error`、Performance Schema worker 错误和 MySQL error log。\n4. 查阻塞资源：观察 Replica 慢查询、锁等待、长事务、CPU、磁盘 await、Buffer Pool、临时表和文件排序。\n5. 查写入形态：定位 Source 上大事务、批量导入、DDL、热点更新、无主键表和 binlog 体积突增。\n6. 先止血：把强一致读切回 Source，摘除延迟副本，终止副本重查询，暂停报表，必要时扩容或提升副本规格。\n7. 再治理：拆小事务，补主键/索引，调整并行复制参数，隔离报表副本，优化 DDL 窗口，建立心跳和延迟阈值自动摘除。\n8. 复测闭环：延迟回落、GTID 差距缩小、业务心跳正常、读写分离恢复、切换候选重新达标。",
      "常见误区：正确理解复制延迟要围绕“数据新鲜度证据”展开。`Seconds_Behind_Source` 是入口指标，GTID 差距、relay 位点、worker 状态和业务心跳共同支撑结论。半同步复制提升事务接收确认能力，读到最新数据仍依赖 Replica 是否完成应用。并行复制提升可并行事务吞吐，热点写入、大事务和 DDL 仍会形成串行瓶颈。读写分离系统应按接口一致性等级设置路由、等待、降级和告警阈值。",
      "参考来源：本文主要参考 MySQL 8.4 Reference Manual 的复制实现、复制线程、`SHOW REPLICA STATUS`、Replica logs/status、Performance Schema replication 表、Replica 选项变量、延迟复制、GTID 生命周期和半同步复制文档，结合 Percona 的复制架构与复制延迟排查文章、pt-heartbeat 文档、Google Cloud SQL 复制延迟说明，以及小林 coding、JavaGuide 的中文 MySQL 学习资料。"
    ],
    typicalProblems: [
      "复制延迟是什么，它为什么会影响读写分离和主从切换？",
      "MySQL Source 到 Replica 的复制链路分为哪些阶段？",
      "`Seconds_Behind_Source`、GTID 差距和 relay log 位点各自说明什么？",
      "如何区分传输延迟和回放延迟？",
      "大事务、DDL、无主键表和副本慢查询分别如何造成复制延迟？",
      "并行复制能解决哪些延迟问题，又受哪些依赖关系限制？",
      "半同步复制和复制延迟之间是什么关系？",
      "线上用户写后读到旧数据，应如何收集证据并快速止血？",
      "主从切换前如何判断候选 Replica 是否安全？",
      "面试追问：如果 `Seconds_Behind_Source` 为 0 但业务仍读到旧数据，你会从哪些证据继续排查？"
    ],
    applicationScenarios: [
      "读写分离读旧数据排查",
      "主从健康监控",
    ...
    practiceAdvice: [
      "把 `SHOW REPLICA STATUS`、Performance Schema replication 表、GTID 差距、业务心跳和系统资源指标放进同一张排查面板。",
      "强一致接口配置 Source 路由、会话粘滞、GTID 等待或业务版本校验，延迟副本自动摘除。",
      "批量写入、归档删除和订正脚本按主键范围拆批，控制单事务行数、binlog 体积和回放时间。",
      "副本上隔离报表和临时查询，慢查询、锁等待和磁盘 I/O 与 applier 线程统一监控。",
      "切换演练使用 GTID 集合、relay backlog、复制错误和业务抽样校验作为候选节点门槛。"
    ],
    commonIssues: [
      "读写分离副本延迟导致用户读到旧订单状态。",
      "大事务或批量导入让 relay log 积压持续扩大。",
      "DDL 或长事务阻塞 applier，`Last_SQL_Error` 或 worker 状态暴露卡点。",
      "副本报表查询抢占 CPU、磁盘和 Buffer Pool，复制回放速度下降。",
      "无主键表在 row-based 复制回放时定位成本过高。",
      "半同步确认接收后，Replica 应用进度仍未追上业务读请求。",
      "主从切换提升落后副本导致事务缺口风险。"
    ],
    commonCommands: [
      "SHOW REPLICA STATUS\\G",
      ...
      "SELECT * FROM performance_schema.replication_applier_status_by_worker\\G",
      "SELECT @@hostname, @@server_id, @@read_only, @@super_read_only, @@gtid_mode, @@replica_parallel_workers",
      "SELECT @@GLOBAL.gtid_executed",
    ],
    useCases: ["读到旧数据排查", "主从健康监控", "复制延迟告警", "故障切换评估", "CDC 链路延迟治理"],
    prerequisites: ["replication"],
    related: ["read-write-splitting", "gtid", "failover", "slow-query-log", "connection-pool"],
    order: 67,
  },
  /* <!-- KG_REVIEWED: GTID | 2026-06-05 | source_count=26 --> */
  /* <!-- KG_EXPLAINED: GTID | 2026-05-23 | source_count=5 --> */
  {
    ...
    area: "replication",
    difficulty: "hard",
    concept: "GTID 是 MySQL 为已提交事务分配的全局事务标识，常用于复制自动定位、主从切换候选判断、缺失事务排查和写后读等待。",
    explanation: [
      "概念定位：GTID（Global Transaction Identifier，全局事务标识符）解决的是复制链路里的“事务身份”和“副本进度”问题。传统 file/position 复制依赖某个 binlog 文件和偏移量；GTID 把每个已提交事务变成可比较的全局编号集合，复制、切换和恢复可以围绕“这个节点执行过哪些事务、还缺哪些事务”做判断。\n\n它出现在 MySQL 主从复制、读写分离、故障切换、跨机房灾备、备份恢复、在线迁移和面试追问里。新手先抓住“每个事务一张全局票据”；有经验的工程师还要看 `gtid_executed`、`gtid_purged`、自动定位握手、errant transaction、binlog 清理窗口和业务一致性证据。",
      "准确定义：MySQL GTID 由 `source_uuid:transaction_id` 组成，`source_uuid` 来自提交事务的源实例 UUID，`transaction_id` 是该源实例上单调递增的事务序号。多个 GTID 可以压缩成 GTID set，例如 `3E11FA47-71CA-11E1-9E33-C80AA9429562:1-19:23`。\n\n- `gtid_executed`：当前实例已经执行过的 GTID 集合，包括本地事务和复制回放事务。\n- `gtid_purged`：已经执行过且对应 binlog 已清理的 GTID 集合，是恢复和新副本接入时的重要边界。\n- `Retrieved_Gtid_Set`：Replica 已经从 Source 接收到 relay log 的 GTID 集合。\n- `Executed_Gtid_Set`：Replica 已经完成应用的 GTID 集合。\n- `SOURCE_AUTO_POSITION = 1`：Replica 连接 Source 时使用 GTID 自动定位，由双方集合差异决定要发送的事务。",
      "心智模型：把 GTID 看成数据库事务的连续邮戳。\n\n1. Source 每提交一个事务，就在 binlog 中盖上 `server_uuid:sequence` 邮戳。\n2. Replica 保存自己已经处理过的邮戳集合。\n3. Replica 重新连接 Source 时提交自己的集合，Source 计算缺口并发送缺失事务。\n4. 故障切换时，候选副本的集合越接近旧 Source，数据缺口越小。\n\n这个模型的价值在于：复制定位从“找文件和偏移量”升级为“比较事务集合”。集合比较能回答三类生产问题：缺哪些事务、哪些事务已经被清理、候选副本是否带有额外事务。",
      "主流程机制：一次带 GTID 的事务从提交到副本可见，按以下过程推进。\n\n1. `gtid_mode=ON` 且 `enforce_gtid_consistency=ON` 后，Source 为事务分配 GTID，并把事务事件与 GTID 写入 binary log。\n2. Replica receiver 线程从 Source 读取 binlog event，写入本地 relay log，并维护 `Retrieved_Gtid_Set`。\n3. Replica applier 线程回放 relay log，提交成功后把事务加入 `Executed_Gtid_Set` 与 `gtid_executed`。\n4. Replica 断线重连时，使用 GTID auto-position 把已执行集合发给 Source。\n5. Source 计算 Replica 缺失的 GTID 集合，继续发送缺失事务。\n6. 读写分离、备份恢复和故障切换系统用 GTID 集合判断副本进度、事务缺口和切换安全性。\n\nGTID 的核心机制是“事务集合驱动复制定位”；网络、relay log、applier、锁等待和 binlog 保留策略共同决定这个集合能否持续追上。",
      "配置示例：生产开启 GTID 通常与 row-based binlog、复制账号、只读副本保护和自动定位一起配置。下面示例表达主路径，真实环境还要接入 TLS、最小权限账号、参数基线、审计和变更窗口。\n\n```ini\n# Source my.cnf\n[mysqld]\nserver_id=101\nlog_bin=mysql-bin\nbinlog_format=ROW\ngtid_mode=ON\nenforce_gtid_consistency=ON\nsync_binlog=1\n\n# Replica my.cnf\n[mysqld]\nserver_id=102\nrelay_log=mysql-relay-bin\ngtid_mode=ON\nenforce_gtid_consistency=ON\nread_only=ON\nsuper_read_only=ON\n```\n\n```sql\n-- Source: 创建复制账号，生产环境收敛到固定网段和最小权限\nCREATE USER 'repl'@'10.%' IDENTIFIED BY 'strong-password';\nGRANT REPLICATION SLAVE ON *.* TO 'repl'@'10.%';\n\n-- Replica: 通过 GTID 自动定位接入 Source\nCHANGE REPLICATION SOURCE TO\n  SOURCE_HOST = '10.0.0.11',\n  SOURCE_PORT = 3306,\n  SOURCE_USER = 'repl',\n  SOURCE_PASSWORD = 'strong-password',\n  SOURCE_AUTO_POSITION = 1;\n\nSTART REPLICA;\nSHOW REPLICA STATUS\\G\n```\n\n`binlog_format=ROW` 让复制回放和数据漂移排查更可预期；`read_only` 与 `super_read_only` 保护副本角色；`sync_binlog`、InnoDB flush 策略和半同步参数共同影响提交成本与故障窗口。",
      "关键集合与函数：GTID 排查的基础是集合运算。\n\n- `GTID_SUBTRACT(source_set, replica_set)`：计算 Source 已有、Replica 仍缺失的事务集合。\n- `GTID_SUBTRACT(replica_set, source_set)`：计算 Replica 上额外出现的事务集合，常用于识别 errant transaction。\n- `GTID_SUBSET(candidate_set, source_set)`：判断一个集合是否包含在另一个集合内，适合候选副本比较。\n- `WAIT_FOR_EXECUTED_GTID_SET(gtid_set, timeout)`：等待当前实例执行到指定 GTID 集合，常用于写后读一致性网关或运维脚本。\n- `mysqlbinlog --include-gtids`：按 GTID 范围从 binlog 中抽取事务，用于恢复、审计和差异分析。\n\n这些函数把“复制进度”从感性判断变成可审计证据，尤其适合切换前检查和线上读旧数据排查。",
      "工程场景：GTID 在生产里主要服务于定位、切换、恢复和一致性等待。\n\n- 复制自动定位：新 Replica 接入或断线重连时，集合差异决定从哪里继续拉取事务。\n- 主从切换：候选副本必须接近旧 Source 的 `Executed_Gtid_Set`，切换前还要检查错误状态、延迟和业务抽样。\n- 写后读等待：写请求记录返回事务 GTID，读请求在目标副本上执行 `WAIT_FOR_EXECUTED_GTID_SET` 后再读。\n- 备份恢复：从备份恢复的新实例依赖 `gtid_executed` 和 `gtid_purged` 接回复制链路。\n- 多源复制：不同 `source_uuid` 的 GTID 集合可以共存，路由和冲突治理需要清晰记录每个通道来源。\n- 在线迁移：迁移工具和双写校验可以用 GTID 集合判断源库、目标库和回放进度。",
      "深层细节与取舍：GTID 让复制拓扑更容易迁移，也把一些隐含问题变成显式集合问题。\n\n- binlog 保留窗口：自动定位依赖 Source 保留 Replica 缺失事务的 binlog；当缺失事务进入 `gtid_purged`，Replica 需要通过备份重建或补齐日志。\n- errant transaction：Replica 上直接写入产生额外 GTID，后续提升、重接复制或链路合并会遇到集合冲突；副本写保护和变更审计是治理重点。\n- server UUID：克隆实例保留相同 `server_uuid` 会让 GTID 来源身份混乱，制作副本模板时要重新生成实例身份。\n- 一致性限制：`enforce_gtid_consistency` 会约束部分语句模式，迁移到 GTID 前要审计历史脚本、DDL、临时表和非事务表使用方式。\n- 半同步边界：半同步确认至少一个副本收到事务，读可见性仍由目标 Replica 的 applier 进度决定。\n- 性能成本：GTID 集合维护本身通常开销较小，真正影响吞吐的是 binlog fsync、事务大小、复制回放并行度、热点锁等待和副本资源竞争。\n- 版本语义：MySQL 8.0/8.4 使用 `SOURCE_*` 术语和 `SHOW REPLICA STATUS`；旧脚本里的 `MASTER_*` / `SHOW SLAVE STATUS` 需要按版本基线迁移。",
      "边界与故障模式：GTID 故障通常表现为复制启动失败、自动定位报错、候选副本缺事务、Replica 带额外事务、写后读等待超时或恢复实例接链失败。\n\n- Source 已清理缺失 binlog：`gtid_purged` 覆盖了 Replica 缺口，自动定位无法拿到需要的事务。\n- Replica 出现 errant transaction：`GTID_SUBTRACT(replica_executed, source_executed)` 返回额外集合，切换前要评估数据影响。\n- 复制线程停止：`Last_IO_Error`、`Last_SQL_Error`、Performance Schema worker 错误和 error log 能定位连接、权限、重复键或 DDL 问题。\n- GTID 模式迁移残留：匿名事务、历史脚本和不一致配置会让新副本接入失败。\n- 克隆身份冲突：多个实例共享 `server_uuid` 会让事务来源混乱，拓扑扩展和恢复演练容易暴露。\n- 读写分离等待超时：目标 Replica 追赶速度低于业务超时预算，路由层需要降级到 Source 或返回一致性等待失败。",
      "排查实践：遇到 GTID 复制或切换问题，按“身份、模式、集合、日志、线程、业务”收集证据。\n\n1. 确认实例身份：记录 Source/Replica 的 `server_id`、`server_uuid`、版本、机房和复制通道。\n2. 确认 GTID 模式：检查 `gtid_mode`、`enforce_gtid_consistency`、`SOURCE_AUTO_POSITION` 和 binlog 配置。\n3. 对比集合差异：分别读取 Source 与 Replica 的 `gtid_executed`、`gtid_purged`、`Retrieved_Gtid_Set`、`Executed_Gtid_Set`。\n4. 判断缺口方向：用 `GTID_SUBTRACT` 计算 Replica 缺失事务和额外事务。\n5. 查日志可用性：确认缺失 GTID 对应的 binlog 是否还在，必要时用 `mysqlbinlog --include-gtids` 抽取验证。\n6. 查线程和错误：读取 `SHOW REPLICA STATUS`、Performance Schema replication 表和 error log。\n7. 做业务校验：按订单号、版本号、流水表或校验和确认业务数据与 GTID 判断一致。\n8. 小步处置：写保护副本、摘除异常节点、补齐缺失事务、重建副本或调整路由，恢复后复测集合差异。\n\n```sql\nSHOW VARIABLES LIKE 'gtid_mode';\nSHOW VARIABLES LIKE 'enforce_gtid_consistency';\nSHOW BINARY LOG STATUS\\G\nSHOW REPLICA STATUS\\G\n\nSELECT @@server_id, @@server_uuid, @@version;\nSELECT @@GLOBAL.gtid_executed, @@GLOBAL.gtid_purged\\G\n\nSELECT *\nFROM performance_schema.replication_connection_status\\G\n\nSELECT *\nFROM performance_schema.replication_applier_status_by_worker\\G\n\n-- 用真实集合替换下面两个字符串\nSELECT GTID_SUBTRACT('source-uuid:1-1200', 'source-uuid:1-1180') AS missing_on_replica;\nSELECT GTID_SUBTRACT('replica-uuid:1-5', 'source-uuid:1-1200') AS errant_on_replica;\nSELECT WAIT_FOR_EXECUTED_GTID_SET('source-uuid:1-1200', 5) AS wait_result;\n```\n\n```bash\nmysqlbinlog --include-gtids='source-uuid:1181-1200' /var/lib/mysql/mysql-bin.000123\n```",
      "常见理解：GTID 是复制定位和完整性证据，数据一致性还需要复制线程状态、日志保留、业务校验和切换流程共同证明。`gtid_executed` 表示当前实例执行过的事务集合，`Retrieved_Gtid_Set` 表示已经接收但可能仍在回放的集合。自动定位能简化重接复制，前提是 Source 仍保存 Replica 缺失事务。半同步提升事务接收确认能力，强一致读取仍要等待目标副本执行到指定 GTID。生产副本应开启写保护，把额外事务、缺失事务和 binlog 清理窗口纳入告警。",
      "参考来源：本文主要参考 MySQL 8.4 Reference Manual 的 GTID 生命周期、格式与存储、自动定位、GTID 配置、GTID 限制、GTID 函数、复制线程、Replica logs/status、`SHOW REPLICA STATUS`、Performance Schema replication 表、`CHANGE REPLICATION SOURCE TO`、binary log 与 `mysqlbinlog` 文档，结合 Oracle MySQL HA 资料、HackMySQL 缺失写入分析、Percona errant GTID 资料，以及小林 coding、JavaGuide 的中文 MySQL 学习资料。"
    ],
    typicalProblems: [
      "GTID 的格式是什么，`source_uuid` 和 `transaction_id` 各自代表什么？",
      "`gtid_executed`、`gtid_purged`、`Retrieved_Gtid_Set`、`Executed_Gtid_Set` 分别说明什么？",
      "GTID auto-position 如何决定 Replica 从哪里继续复制？",
      "GTID 和 file/position 复制在运维体验上有哪些差异？",
      "主从切换前如何用 GTID 集合判断候选副本是否安全？",
      "Source 已经清理缺失事务对应的 binlog 时，Replica 接链为什么会失败，如何恢复？",
      "什么是 errant transaction，如何发现和治理？",
      "半同步复制和 GTID 对数据一致性分别提供什么证据？",
      "写后读场景如何使用 `WAIT_FOR_EXECUTED_GTID_SET`？",
      "克隆实例复用 `server_uuid` 会给 GTID 拓扑带来哪些问题？",
      "线上 GTID 复制报错时，你会按什么顺序收集证据？"
    ],
    applicationScenarios: [
      "复制自动定位",
      "主从切换候选评估",
      ...
      "跨机房灾备切换",
      "在线迁移进度校验",
      "errant transaction 审计",
      "binlog 缺失事务恢复"
    ],
    practiceAdvice: [
      "把 Source/Replica 的 `server_uuid`、`gtid_executed`、`gtid_purged`、`Retrieved_Gtid_Set`、`Executed_Gtid_Set` 放入切换前检查清单。",
      "副本开启 `read_only` 和 `super_read_only`，复制账号使用最小权限，人工写副本动作进入审批和审计。",
      "binlog 保留时间按最大副本延迟、重建耗时、灾备演练窗口和恢复 SLA 共同设置。",
      "写后读接口记录提交 GTID，并在目标副本上等待执行到该集合；超时后路由到 Source 或执行一致性降级策略。",
      "定期演练 GTID auto-position、缺失 binlog、errant transaction 和副本重建流程，保留命令输出作为运行手册证据。"
    ],
    commonIssues: [
      "Replica 缺失事务对应的 binlog 已进入 `gtid_purged`。",
      "Replica 直接写入产生 errant transaction，切换候选集合出现额外 GTID。",
      "克隆副本复用旧 `server_uuid`，事务来源身份混乱。",
      "GTID 模式迁移时历史脚本触发一致性限制。",
      "写后读等待目标副本执行 GTID 超时。",
      "复制线程停止，`Last_SQL_Error` 或 worker 错误指向重复键、DDL 或数据漂移。",
      "半同步确认接收事务后，业务读请求落到尚未应用完成的 Replica。"
    ],
    commonCommands: [
      "SHOW VARIABLES LIKE 'gtid_mode'",
      "SHOW VARIABLES LIKE 'enforce_gtid_consistency'",
      "SHOW BINARY LOG STATUS\\G",
      "SHOW REPLICA STATUS\\G",
      "SELECT @@server_id, @@server_uuid, @@GLOBAL.gtid_executed, @@GLOBAL.gtid_purged",
      "SELECT GTID_SUBTRACT('source-set', 'replica-set')",
      "SELECT GTID_SUBSET('candidate-set', 'source-set')",
      "SELECT WAIT_FOR_EXECUTED_GTID_SET('uuid:1-1200', 5)",
      "mysqlbinlog --include-gtids='uuid:1181-1200' mysql-bin.000123",
    ],
    useCases: ["复制故障恢复", "主从切换", "读写分离一致性等待", "备份恢复接链", "在线迁移校验"],
    prerequisites: ["replication", "binlog", "replication-lag"],
    related: ["read-write-splitting", "replication-lag", "failover", "backup-restore", "point-in-time-recovery"],
    order: 68,
  },
...
const mysqlKnowledgePointOverrides: Record<string, Partial<GraphKnowledgePoint>> = {
  ...
  "denormalization": {
    ...
    concept:
      "反范式是在明确权威数据源和同步契约的前提下，把可重建字段、快照或汇总结果冗余到读模型里，用有限一致性成本换取更稳定的查询性能。",
    explanation: [
      "概念定位：反范式（Denormalization）解决的是“范式化后的多表关联、聚合和排序在高频读路径上成本过高”的问题。它常出现在订单列表、商品详情、用户权限快照、消息会话列表、排行榜、运营报表、搜索索引和缓存预热链路中。\n\n在 MySQL 里，反范式通常表现为冗余字段、宽表、汇总表、快照表、派生列、生成列、读模型表或异步刷新表。新手先把它理解为“提前把查询要用的数据摆到一张更好读的表里”；老手要继续评估权威源、更新时序、陈旧窗口、写放大、索引膨胀、Binlog/复制压力、校验修复和回滚方案。",
      "准确定义：反范式是在规范化模型之外有意保存重复或派生数据，以减少查询时的 `JOIN`、`GROUP BY`、排序、回表或跨库聚合成本。\n\n几个术语要放在一起理解：\n\n- 权威表：数据事实的来源，例如 `users`、`products`、`orders`、`payments`。\n- 冗余字段：从权威表复制到读路径的字段，例如 `orders.user_name_snapshot`、`order_items.sku_name_snapshot`。\n- 派生字段：由同表或多表计算而来，例如 `order_count`、`total_amount`、`latest_message_at`。\n- 快照字段：记录某个时间点的历史事实，例如下单时商品名、价格、收货地址。\n- 汇总表：按业务维度预聚合的读模型，例如 `seller_daily_stats`。\n- 同步契约：说明字段来自哪里、何时刷新、允许多旧、失败如何补偿、如何校验修复。\n\n反范式设计的核心是把“读时计算”转成“写时维护或异步维护”，收益和风险都来自这个时间点迁移。",
      "心智模型：把反范式看成数据库里的预制菜。\n\n- 原料仓库：范式化主表保存权威事实，适合精确写入、审计和修复。\n- 预制成品：宽表或汇总表提前组合字段，适合高频读取和稳定分页。\n- 标签溯源：每个冗余字段都要知道来源列、刷新版本和更新时间。\n- 保质期：读模型允许有限陈旧窗口，窗口长度由业务风险决定。\n- 质检流程：校验 SQL、差异任务、补偿队列和告警负责发现读模型漂移。\n\n这个模型能帮助判断字段类型：订单商品名是历史快照，商品当前名称是主数据，列表页昵称是性能冗余，日统计销售额是派生汇总。",
      "主流程机制：一次反范式改造可以按从证据到契约的顺序推进。\n\n1. 固定读路径：拿到目标接口、SQL、调用量、p95/p99、扫描行数、返回列、排序分页和热点参数。\n2. 确认瓶颈：用 `EXPLAIN ANALYZE`、慢查询日志和 Performance Schema 判断成本来自 JOIN、聚合、filesort、回表、跨库查询还是网络往返。\n3. 定义读模型：决定冗余字段、快照字段、汇总维度、索引顺序和返回列，保持字段语义单一。\n4. 指定权威源：为每个冗余列写清 `source_table.source_column`、业务 owner、刷新触发条件和允许陈旧窗口。\n5. 设计写入路径：选择同事务写入、应用双写、触发器、Outbox、Binlog/CDC、定时任务或离线重算。\n6. 增加版本证据：冗余表保留 `source_version`、`source_updated_at`、`refreshed_at`、`sync_status` 或事件位点。\n7. 建立校验修复：用抽样校验、全量巡检、差异回填和告警把漂移控制在可解释范围内。\n8. 验证收益与副作用：比较改造前后的延迟、扫描行数、索引大小、写入耗时、Binlog 体积、复制延迟和失败重试量。\n\n反范式的交付物应包含 DDL、写路径、校验 SQL、监控指标和回滚策略。",
      "实践例子：订单列表常见瓶颈是每次分页都关联用户和支付表，再排序返回少量字段。先看范式化查询：\n\n```sql\nEXPLAIN ANALYZE\nSELECT o.id, o.order_no, o.status, o.amount,\n       u.nickname, p.paid_at\nFROM orders o\nJOIN users u ON u.id = o.user_id\nLEFT JOIN payments p ON p.order_id = o.id AND p.status = 'SUCCESS'\nWHERE o.user_id = 1001\nORDER BY o.created_at DESC\nLIMIT 20;\n```\n\n如果这个接口是高频读路径，可以把列表需要的字段沉淀到读模型表：\n\n```sql\nCREATE TABLE order_list_view (\n  order_id BIGINT UNSIGNED NOT NULL,\n  user_id BIGINT UNSIGNED NOT NULL,\n  order_no VARCHAR(64) NOT NULL,\n  order_status VARCHAR(32) NOT NULL,\n  amount DECIMAL(12,2) NOT NULL,\n  user_nickname VARCHAR(64) NOT NULL COMMENT '来自 users.nickname 的列表冗余',\n  paid_at DATETIME NULL,\n  source_version BIGINT UNSIGNED NOT NULL DEFAULT 0,\n  source_updated_at DATETIME NOT NULL,\n  refreshed_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,\n  PRIMARY KEY (order_id),\n  KEY idx_user_refreshed (user_id, refreshed_at, order_id),\n  KEY idx_status_refreshed (order_status, refreshed_at, order_id)\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;\n```\n\n读路径变成单表查询：\n\n```sql\nEXPLAIN ANALYZE\nSELECT order_id, order_no, order_status, amount, user_nickname, paid_at\nFROM order_list_view\nWHERE user_id = 1001\nORDER BY refreshed_at DESC, order_id DESC\nLIMIT 20;\n```\n\n这里的关键是 `order_list_view` 只是读模型，订单最终状态仍以 `orders/payments` 为准；昵称冗余要配套用户改名事件或定时校验。",
      "同步路径：反范式字段可以有多种维护方式，选择依据是数据风险、写入频率和团队控制面。\n\n- 同事务写入：业务服务在一个数据库事务里同时更新权威表和冗余表，适合单库、低延迟和强审计链路。\n- 应用双写：服务先写权威表，再写读模型或发送刷新任务，适合逻辑清晰的聚合根，要求幂等和补偿。\n- 触发器：MySQL `CREATE TRIGGER` 可以在表级 DML 后维护派生字段，适合局部规则，复杂业务逻辑会增加排查难度。\n- 生成列：MySQL generated column 适合同表内确定性表达式，例如 JSON 字段抽取索引；跨表冗余仍要靠业务或异步链路。\n- Outbox：数据库事务同时写业务表和 outbox 表，后台可靠发布刷新事件，降低提交成功后事件缺席的风险。\n- Binlog/CDC：通过 MySQL Binary Log、Debezium 等捕获行级变更，异步刷新读模型、搜索索引或缓存。\n- 定时重算：报表和低频汇总适合批处理刷新，配合水位、窗口和差异校验控制成本。\n\n同步链路越异步，读模型越需要版本号、事件 ID、重试、死信、延迟指标和人工修复入口。",
      "工程场景：反范式字段要先分清业务含义，再决定一致性目标。\n\n- 订单快照：下单时商品名、价格、收货地址表达历史事实，写入后长期稳定，适合保存在订单明细或快照表。\n- 列表冗余：用户昵称、商品主图、支付完成时间服务查询体验，允许短暂陈旧，需要刷新事件和校验任务。\n- 计数器与余额摘要：评论数、点赞数、未读数属于高并发派生数据，需要幂等事件、去重键和定期对账。\n- 报表宽表：按天、商家、渠道、类目聚合，通常接受分钟级或小时级刷新窗口，并保留重算能力。\n- 搜索/推荐读模型：MySQL 权威源通过 CDC 推送到 ES、向量库或特征表，下游延迟和 schema 演进要可观测。\n- 权限快照：登录态或网关侧权限副本必须带版本、过期和强制刷新机制，高风险操作回源确认。",
      "深层细节：反范式会把查询成本转移到写入、存储和治理链路。\n\n- 写放大：一次主数据变更可能更新多张冗余表，Redo、Undo、Binlog、二级索引和复制回放都会增加。\n- 存储膨胀：宽字段、长文本、JSON、图片 URL 和多列索引会降低页内行密度，增加 Buffer Pool 压力。\n- 索引取舍：读模型为了单表查询常创建面向场景的联合索引，索引顺序应匹配过滤、排序和分页。\n- 陈旧窗口：异步刷新链路会让用户短时间看到旧昵称、旧统计或旧状态；业务需要定义可接受窗口和强制回源条件。\n- 乱序与重复：消息重试、CDC 分区、消费者重启会带来重复事件和乱序事件，冗余表需要 `version` 或事件位点做幂等推进。\n- 回填风险：大批量重算会抢 I/O、产生长事务、放大复制延迟和锁等待，应分批、限速、可中断。\n- schema 演进：读模型字段新增、重命名和删除要与事件 schema、消费者版本、回填脚本和灰度开关一起发布。",
      "边界与故障模式：反范式问题通常表现为读得快但解释成本高。\n\n- 冗余漂移：权威表已更新，读模型仍展示旧字段，用户看到列表和详情结果不同。\n- 部分刷新：多字段来自不同表，某个事件消费失败后只更新了状态，金额或昵称停留在旧版本。\n- 事件乱序：先到的旧事件覆盖后到的新值，导致状态回退或统计变小。\n- 写入抖动：主表更新触发大量冗余更新，提交延迟、锁等待、Binlog 体积和复制延迟升高。\n- 索引过宽：宽读模型叠加多个联合索引，磁盘、备份、Buffer Pool 和 DDL 时间明显增长。\n- 口径争议：同一字段既被当成历史快照，又被当成当前事实，报表和客服解释产生冲突。\n- 修复困难：缺少 source version、刷新日志、校验 SQL 和重放位点，问题只能靠人工猜测。",
      "排查实践：线上怀疑反范式字段不一致时，把权威源、读模型、事件链路和查询计划放到同一条证据链。\n\n1. 固定对象：记录用户请求 ID、业务主键、读模型表、展示字段、客户端看到的值和时间。\n2. 对比权威源：查主表的字段、`updated_at`、业务版本和事务日志，确认最终事实。\n3. 对比读模型：查冗余表的字段、`source_version`、`source_updated_at`、`refreshed_at` 和 `sync_status`。\n4. 还原同步链路：检查应用日志、outbox 记录、消息消费状态、Debezium offset、Binlog 文件/position 和死信队列。\n5. 检查查询路径：用 `EXPLAIN ANALYZE`、`SHOW INDEX`、慢查询日志和 Performance Schema 确认读模型是否真正降低成本。\n6. 修复差异：按业务主键精确重放事件或回填读模型，再启动抽样或全量校验避免同类问题扩散。\n7. 补齐契约：增加版本推进、幂等键、失败告警、校验任务和回滚脚本，把临时修复沉淀成机制。\n\n```sql\n-- 权威源与读模型对比\nSELECT id, status, amount, updated_at, version\nFROM orders\nWHERE id = 880001;\n\nSELECT order_id, order_status, amount, source_version,\n       source_updated_at, refreshed_at, sync_status\nFROM order_list_view\nWHERE order_id = 880001;\n\n-- 找出读模型落后的订单\nSELECT v.order_id, o.version AS order_version, v.source_version,\n       o.updated_at, v.refreshed_at\nFROM order_list_view v\nJOIN orders o ON o.id = v.order_id\nWHERE v.source_version < o.version\nORDER BY o.updated_at DESC\nLIMIT 100;\n\n-- 验证读模型查询是否命中预期索引\nSHOW INDEX FROM order_list_view;\nEXPLAIN ANALYZE\nSELECT order_id, order_no, order_status\nFROM order_list_view\nWHERE user_id = 1001\nORDER BY refreshed_at DESC, order_id DESC\nLIMIT 20;\n\n-- 查看同步事件位点，命令以实际 MySQL 版本和权限为准\nSHOW BINARY LOG STATUS;\n```\n\n有效结论需要说明：权威事实是什么，读模型落后多久，事件链路停在哪个位置，是否存在重复/乱序覆盖，以及修复后如何持续校验。",
      "常见误区：反范式是受控读模型设计，核心是把性能目标和一致性契约一起写清楚。\n\n- 范式表负责权威事实，反范式表负责高频读取、快照或汇总视图，两者共同服务业务正确性。\n- 冗余字段必须有 owner、source columns、刷新时机、版本字段、校验 SQL 和修复脚本。\n- 历史快照字段保存当时事实，当前主数据字段保存最新事实，字段命名要体现时间语义。\n- 高频读接口优先用真实慢查询、`EXPLAIN ANALYZE` 和扫描行数证明改造收益。\n- 异步读模型需要定义陈旧窗口，支付、库存、余额和权限裁决类路径通常回到权威源或版本校验。\n- 写路径变重时要观察提交延迟、锁等待、Redo/Undo、Binlog 体积、复制延迟和下游消费积压。",
      "面试追问：反范式类问题适合按“目标 -> 读模型 -> 同步契约 -> 一致性窗口 -> 排查证据 -> 工程取舍”回答。\n\n- 反范式解决什么问题，它在 MySQL 表设计中常见形态有哪些？\n- 冗余字段、快照字段、派生字段和汇总表分别适合哪些业务场景？\n- 如何判断一个高频接口值得做反范式改造？要看哪些执行计划和指标？\n- 订单列表从多表 JOIN 改成读模型表，DDL、索引和刷新链路如何设计？\n- 同事务写入、应用双写、触发器、Outbox、Binlog/CDC 和定时重算各自适合什么场景？\n- 反范式字段如何避免重复事件、乱序事件和部分刷新导致的数据漂移？\n- 历史快照和当前事实如何命名、存储和解释？\n- 反范式会带来哪些写放大、存储膨胀、复制延迟和 DDL 风险？\n- 线上用户反馈列表和详情不一致时，你会查哪些 SQL、日志、位点和指标？\n- 面试中如何比较范式、反范式、缓存、搜索索引和物化汇总的边界？",
      "参考来源：本讲解主要参考 MySQL 8.4 Reference Manual 的 `CREATE TABLE`、Generated Columns、数据大小优化、索引、`EXPLAIN`、慢查询日志、Performance Schema、触发器、Binary Log 和 Binlog 选项文档，用 PlanetScale 的 Schema 与 Join 课程校准表设计和关联成本，用 Microsoft、GeeksforGeeks 与范式资料校准反范式定义与取舍，再结合 Debezium MySQL Connector、Transactional Outbox、Transaction Log Tailing、阿里巴巴 Java 开发手册、JavaGuide 与 CS-Notes 补充同步链路、中文工程表达和面试场景。官方资料用于确定命令、索引、日志和触发器边界，工程资料用于补足读模型、一致性窗口和生产排障判断。"
    ],
    typicalProblems: [
      "反范式解决什么问题，它和范式在数据源头、读模型和一致性目标上如何协同？",
      "冗余字段、快照字段、派生字段、宽表和汇总表分别适合哪些 MySQL 场景？",
      "如何用慢查询、`EXPLAIN ANALYZE`、扫描行数和 p99 延迟证明反范式改造有价值？",
      "订单列表从多表 JOIN 改成读模型表时，DDL、联合索引和查询改写如何设计？",
      "同事务写入、应用双写、触发器、Outbox、Binlog/CDC 和定时重算如何取舍？",
      "冗余字段如何用 `version`、`source_updated_at`、事件 ID 和位点处理重复、乱序和部分刷新？",
      "反范式会怎样影响写入延迟、锁、Redo/Undo、Binlog、复制延迟、索引空间和备份恢复？",
      "历史快照、当前主数据和性能冗余如何区分，字段命名和口径解释如何设计？",
      "线上发现列表页和详情页数据不一致时，如何用 SQL、日志、CDC offset 和校验任务建立证据链？",
      "面试中如何比较反范式、缓存、搜索索引、物化汇总和应用层聚合的边界？"
    ],
    commonCommands: [
      "SHOW CREATE TABLE <read_model_table>\\G",
      "SHOW INDEX FROM <read_model_table>",
      "EXPLAIN ANALYZE <read_model_query>",
      "SELECT v.id, v.source_version, s.version FROM <view_table> v JOIN <source_table> s ON s.id = v.id WHERE v.source_version < s.version LIMIT 100",
      "SHOW BINARY LOG STATUS",
      "SELECT DIGEST_TEXT, COUNT_STAR, AVG_TIMER_WAIT FROM performance_schema.events_statements_summary_by_digest ORDER BY SUM_TIMER_WAIT DESC LIMIT 10"
    ],
    applicationScenarios: [
      "高频订单列表",
      "商品详情页",
    ...
    commonIssues: [
      "冗余字段缺少 source version、刷新日志和校验任务，列表与详情出现口径漂移。",
      "应用双写失败或消息消费失败后，读模型停留在旧版本。",
      "CDC 乱序或重复事件覆盖新值，状态、计数或金额出现回退。",
      "读模型宽字段和多列索引放大写入、磁盘、Buffer Pool、备份和 DDL 成本。",
      "批量回填或重算没有限速，主库 I/O、锁等待和复制延迟升高。",
      "历史快照和当前事实命名混乱，客服、报表和审计解释成本上升。",
    ],
    useCases: ["高频列表查询", "报表宽表", "读多写少场景", "快照表", "汇总表", "搜索索引同步", "一致性排查", "性能面试"],
    prerequisites: ["normalization"],
    related: ["normalization", "schema-design", "join", "sql-optimization", "mysql-index", "binlog", "replication-lag", "cache-consistency"],
  },
  "data-type": {
    sourceRefs: [
      "mysql-data-types",
  ...
  "sql-optimization": {
    ...
    concept:
      "SQL 优化是用执行计划、索引、统计信息、SQL 改写和运行证据，减少 MySQL 为得到同一结果所做的扫描、排序、回表、锁等待和 I/O 工作。",
    explanation: [
      "概念定位：SQL 优化（SQL Optimization）解决的是“业务 SQL 在数据量、并发量和响应时间目标增长后，如何稳定地用更低成本返回正确结果”的问题。它常出现在慢查询治理、接口 p99 延迟、报表查询、列表分页、批量更新、数据库成本下降、线上面试和事故复盘里。\n\n在 MySQL 中，SQL 文本表达结果语义，优化器根据表结构、索引、统计信息、条件选择性、JOIN 关系、排序和分页要求选择物理执行计划。优化工作的核心是把同一结果集变成更少的候选行、更少的随机 I/O、更少的临时结果、更短的锁持有时间和更可预测的计划。",
      "准确定义：SQL 优化是一套以证据驱动的工程流程，包含发现、解释、改写、验证、上线和回归监控。\n\n常见抓手包括：\n\n- `EXPLAIN` / `EXPLAIN ANALYZE`：观察访问类型、索引选择、估算行数、实际耗时和执行树。\n- 索引设计：让过滤、关联、排序、分组和返回列尽量通过有序索引完成。\n- SQL 改写：调整谓词、JOIN、子查询、分页、聚合、批量写入和返回列，降低中间结果规模。\n- 统计信息：通过直方图、`ANALYZE TABLE` 和索引基数信息校准优化器估算。\n- 运行证据：慢查询日志、Performance Schema、连接状态、锁等待、I/O、CPU、复制延迟共同证明收益。\n\n一个可靠的优化结论需要同时满足语义正确、计划合理、线上指标改善、写入成本可接受和回滚路径清晰。",
      "心智模型：把 SQL 优化看成给数据库“减少搬运工作”。\n\n- 过滤越早，候选行越少，后续 JOIN、排序和回表成本越低。\n- 顺序越匹配，`ORDER BY`、`GROUP BY` 和分页越容易借助索引顺序完成。\n- 返回列越精简，网络传输、内存占用和覆盖索引机会越好。\n- 事务越短，写 SQL 的锁持有时间、Undo/Redo 压力和复制延迟越可控。\n- 统计信息越贴近真实数据分布，优化器越容易选出稳定计划。\n\n新手先记住“扫描行数决定下限，排序与回表决定额外成本”；老手继续追踪估算偏差、参数偏斜、索引维护成本、计划稳定性和线上发布风险。",
      "主流程机制：一次标准 SQL 优化可以按证据链推进。\n\n1. 发现目标：从慢查询日志、Performance Schema、APM、接口 p95/p99、CPU/I/O、连接数和错误码定位高成本 SQL。\n2. 固化上下文：记录 SQL 模板、绑定参数、表结构、索引、数据量、过滤字段基数、事务边界、隔离级别和业务调用入口。\n3. 读取计划：用 `EXPLAIN` 看 `type`、`possible_keys`、`key`、`rows`、`filtered`、`Extra`，用 `EXPLAIN ANALYZE` 对比真实耗时和行数。\n4. 判断瓶颈：确认成本来自全表扫描、低选择性索引、回表过多、filesort、临时表、JOIN 顺序、深分页、锁等待、统计信息偏差或返回列过宽。\n5. 设计改动：选择索引、SQL 改写、拆批、分页调整、统计信息刷新、表结构调整或业务缓存等方案。\n6. 验证收益：在同等参数和数据量下对比计划、扫描行数、执行时间、锁范围、写入成本和复制影响。\n7. 灰度上线：用小流量、只读副本或影子查询观察指标，保留回滚 SQL、索引变更撤销方案和告警阈值。\n\n这个流程把“感觉慢”转换成可复盘的工程证据。",
      "实践例子：订单列表慢查询的优化路径可以这样走。\n\n```sql\n-- 原始查询：按用户、状态筛选，按创建时间倒序分页\nEXPLAIN ANALYZE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC\nLIMIT 20;\n\n-- 观察表结构与索引\nSHOW CREATE TABLE orders\\G\nSHOW INDEX FROM orders;\n\n-- 常见优化：让等值过滤 + 排序列进入同一个联合索引\nCREATE INDEX idx_user_status_created_id\nON orders (user_id, status, created_at DESC, id);\n\n-- 再次验证访问路径、真实行数和耗时\nEXPLAIN FORMAT=TREE\nSELECT id, amount, created_at\nFROM orders\nWHERE user_id = 1001 AND status = 'PAID'\nORDER BY created_at DESC\nLIMIT 20;\n```\n\n理想计划会围绕 `idx_user_status_created_id` 做有序范围扫描，`rows` 和实际读取行数接近返回行数，`Extra` 中排序和临时表成本下降。`id` 作为平局列能提升分页顺序稳定性；返回列继续影响覆盖索引机会和回表成本。",
      "深层细节：SQL 优化的难点在于优化器估算、数据分布和工程代价会随时间变化。\n\n- 统计信息：优化器依赖索引基数、直方图和表统计估算行数；热点值、时间窗口和数据倾斜会让估算与真实行数出现偏差。\n- 访问类型：`const`、`eq_ref`、`ref`、`range`、`index`、`ALL` 只是入口，还要结合 `rows`、`filtered`、回表次数和实际耗时判断。\n- 组合索引：等值列、范围列、排序列和覆盖列的顺序会影响过滤、排序、分页和写入维护成本。\n- ICP 与范围访问：Index Condition Pushdown 能在存储引擎层提前过滤部分条件，减少回表和上层处理量。\n- JOIN 成本：驱动表行数、被驱动表查找方式、连接条件索引、外连接语义和优化器重排共同决定中间结果规模。\n- 写入影响：新增索引会增加插入、更新、删除、Redo、Buffer Pool 和复制开销，高频写表需要把查询收益和写入成本一起计算。\n- 计划控制：Optimizer Hint、Invisible Index 和 Optimizer Trace 适合诊断与灰度验证，生产使用需要记录触发条件和撤销路径。",
      "工程场景：不同业务形态对应不同优化重点。\n\n- 高频详情与列表：围绕主键、唯一键、用户维度、状态、时间排序和返回列建立稳定索引组合。\n- 深分页与搜索：优先使用游标分页、seek 查询或先取主键再回表读取宽列。\n- 报表与聚合：控制扫描范围，利用分区、汇总表、异步计算、只读副本和离线任务降低主库压力。\n- 批量更新与订正：按主键范围分批，限制每批行数，观察锁等待、Undo、Redo、Binlog 和复制延迟。\n- 多表 JOIN：先缩小高选择性集合，再通过索引访问被驱动表，使用 `EXPLAIN FORMAT=TREE` 验证 JOIN 顺序。\n- 上线治理：索引变更要评估建索引时间、磁盘空间、元数据锁窗口、回滚方式和业务低峰发布节奏。",
      "边界与故障模式：SQL 优化经常触发新的资源取舍。\n\n- 索引过多：读路径变快，写入维护、存储占用、Buffer Pool 压力和复制延迟随之增加。\n- 低选择性字段：单列索引收益有限，组合条件、排序需求和覆盖列会改变判断。\n- 隐式转换：字段类型、字符集和排序规则差异会改变比较方式，计划和结果口径都需要验证。\n- 参数偏斜：同一 SQL 模板在热点用户、大时间范围、空状态和普通参数下可能呈现完全不同成本。\n- 临时表与 filesort：数据量超过内存阈值后会放大磁盘 I/O，报表和排序接口需要重点监控。\n- 锁与事务：`UPDATE/DELETE` 的定位路径决定锁范围，范围扫描和批量操作会放大锁等待与死锁概率。\n- 读写分离：从库慢查询会积压复制 SQL 线程，进而影响读一致性和故障切换窗口。",
      "排查实践：线上慢 SQL 可以按下面命令建立最小证据包。\n\n```sql\n-- 1. 找高成本语句摘要\nSELECT DIGEST_TEXT, COUNT_STAR, SUM_ROWS_EXAMINED, SUM_ROWS_SENT, SUM_TIMER_WAIT\nFROM performance_schema.events_statements_summary_by_digest\nORDER BY SUM_TIMER_WAIT DESC\nLIMIT 10;\n\n-- 2. 看表、索引和字段类型\nSHOW CREATE TABLE orders\\G\nSHOW INDEX FROM orders;\n\n-- 3. 看估算计划与真实计划\nEXPLAIN FORMAT=TREE\nSELECT id, amount FROM orders WHERE user_id = 1001 ORDER BY created_at DESC LIMIT 20;\n\nEXPLAIN ANALYZE\nSELECT id, amount FROM orders WHERE user_id = 1001 ORDER BY created_at DESC LIMIT 20;\n\n-- 4. 校准统计信息\nANALYZE TABLE orders;\n\n-- 5. 必要时打开优化器跟踪诊断计划选择\nSET optimizer_trace='enabled=on';\nSELECT id, amount FROM orders WHERE user_id = 1001 ORDER BY created_at DESC LIMIT 20;\nSELECT TRACE FROM information_schema.OPTIMIZER_TRACE\\G\n```\n\n排查记录建议固定包含：SQL 模板、参数样本、优化前后计划、扫描行数、实际耗时、锁等待、索引 DDL、发布批次、回滚动作和线上指标截图。",
      "常见误区：SQL 优化的正确判断来自结果语义、执行计划和运行证据的统一。\n\n索引命中只是观察点之一，真正影响体验的是扫描行数、返回行数、回表次数、排序与临时表、锁持有时间和资源峰值。`EXPLAIN` 展示优化器估算计划，`EXPLAIN ANALYZE` 能补充真实执行数据。新增索引服务读请求，同时会增加写入维护成本。SQL 改写需要保持业务语义、分页顺序、NULL 处理、事务隔离可见性和权限边界一致。",
      "面试追问：回答 SQL 优化题时，可以按“定位、计划、改法、验证、取舍”组织。\n\n- 定义题：SQL 优化解决什么问题，优化对象包含哪些资源成本？\n- 机制题：MySQL 优化器如何根据索引、统计信息和代价模型选择执行计划？\n- 计划题：`EXPLAIN` 里的 `type`、`key`、`rows`、`filtered`、`Extra` 分别如何解读？\n- 索引题：联合索引列顺序如何结合等值、范围、排序、覆盖和写入成本选择？\n- 场景题：深分页、JOIN、GROUP BY、ORDER BY、批量 UPDATE 分别怎么优化？\n- 排障题：线上慢 SQL 如何从慢查询日志、Performance Schema、执行计划和锁等待建立证据链？\n- 边界题：统计信息偏差、参数偏斜、低选择性索引、隐式转换和从库延迟如何影响优化效果？\n- 取舍题：新增索引、SQL 改写、缓存、读写分离、汇总表和分库分表各自解决什么成本问题？",
      "参考来源：本讲解主要参考 MySQL 8.4 Reference Manual 的 Optimization、Optimizing SELECT、WHERE/Range/ORDER BY/GROUP BY/LIMIT Optimization、How MySQL Uses Indexes、Multiple-Column Indexes、EXPLAIN、Optimizer Statistics、Optimizer Trace、Optimizer Hints、Slow Query Log 和 Performance Schema 文档；结合 MySQL Workbench Visual Explain、Use The Index, Luke、JavaGuide MySQL 执行计划分析和小林 coding 的 SELECT 执行流程内容校准中文表达与实践路径。官方资料用于定义、字段、命令和边界，中文与工程资料用于补充排查步骤和面试表达。"
    ],
    typicalProblems: [
      "SQL 优化解决什么问题，为什么它要同时关注 CPU、I/O、内存、锁、网络和复制成本？",
      "MySQL 优化器如何利用索引、统计信息、直方图和代价模型选择执行计划？",
      "`EXPLAIN` 与 `EXPLAIN ANALYZE` 分别提供哪些证据，如何解读 `type`、`key`、`rows`、`filtered` 和 `Extra`？",
      "联合索引列顺序如何结合等值条件、范围条件、排序、分组、覆盖索引和写入成本设计？",
      "慢查询如何从慢查询日志、Performance Schema、表结构、索引、参数和数据分布建立完整证据链？",
      "深分页、filesort、临时表、JOIN 顺序和回表过多分别有哪些典型优化方式？",
      "为什么统计信息偏差、参数偏斜、隐式类型转换和字符集差异会让计划质量下降？",
      "新增索引、Optimizer Hint、Invisible Index、ANALYZE TABLE 和 SQL 改写各自适合什么场景？",
      "批量 UPDATE/DELETE 优化时如何控制锁范围、事务时长、Undo/Redo、Binlog 和复制延迟？",
      "生产发布 SQL 优化方案时，如何设计灰度、指标对比、回滚和复盘材料？"
    ],
    commonCommands: [
      "EXPLAIN FORMAT=TREE <sql>",
      "EXPLAIN ANALYZE <sql>",
    ...
    practiceAdvice: [
      "先收集 SQL 模板、绑定参数、数据分布和执行计划，再设计索引或改写方案。",
      "高频查询优先让过滤列、排序列和返回列形成稳定的联合索引路径。",
      "写表新增索引前同步评估写入 TPS、磁盘空间、Buffer Pool、Redo、Binlog 和复制延迟。",
      "复杂 JOIN 优化时重点看驱动表候选行规模、被驱动表访问方式和中间结果行数。",
      "深分页优先用游标分页或 seek 查询，宽列读取可以拆成先取主键再批量回表。",
      "优化发布后保留优化前后计划、指标截图、DDL、回滚动作和业务验证样本。",
    ],
    applicationScenarios: [
      "慢查询治理",
      "接口 p99 延迟优化",
    ...
    commonIssues: [
      "全表扫描导致 CPU、I/O 和响应时间升高。",
      "回表过多放大随机 I/O 和 Buffer Pool 压力。",
      "filesort 与临时表让排序、分组和去重成本升高。",
      "统计信息偏差导致优化器选择低效访问路径。",
      "参数偏斜让同一 SQL 模板在热点参数下表现异常。",
      "批量写 SQL 持锁时间过长并放大复制延迟。",
      "新增索引带来写入维护成本和磁盘空间增长。",
    ],
    useCases: ["慢查询治理", "执行计划分析", "索引设计", "SQL 改写", "批量写优化", "生产排障", "性能面试"],
    prerequisites: ["sql", "mysql-index"],
    related: ["explain", "access-type", "extra", "slow-query-log", "join-order", "composite-index", "leftmost-prefix", "range-query", "covering-index", "index-selectivity", "limit-offset"],
  },
  "explain": {
    sourceRefs: [
      "mysql-workbench-visual-explain",
    ...
    prerequisites: ["sql-optimization"],
    related: ["access-type", "extra", "mysql-index", "composite-index", "slow-query-log", "join-order"],
  },
  "access-type": {
    prerequisites: ["explain"],
