...
const operatingSystemKnowledgePointBase = [
  ...
  /* <!-- KG_REVIEWED: 用户态与内核态 | 2026-05-24 | source_count=7 --> */
  /* <!-- KG_EXPLAINED: 用户态与内核态 | 2026-05-23 | source_count=7 --> */
  { sourceRefs: ["linux-kernel-docs","linux-man-pages","posix-base-spec","gnu-bash-manual","cs-notes","javaguide","xiaolin-coding"], id: "kernel-mode", zh: "用户态与内核态", en: "User Mode and Kernel Mode", area: "foundation", difficulty: "medium", summary: "区分普通应用代码和内核特权代码的执行环境。", explanation: ["核心概念：用户态与内核态（User Mode and Kernel Mode）聚焦区分普通应用代码和内核特权代码的执行环境。。操作系统负责在硬件和应用之间管理 CPU、内存、文件、设备与权限；理解它时先抓住内核边界、系统调用、特权级和中断路径，再看输入、状态变化、输出结果和失败分支。","适用场景：用户态与内核态常用于用户态进入内核、硬件事件打断执行、内核统一管理资源。学习时把它放回操作系统链路中观察，并结合前置知识内核判断它解决的具体问题。","特殊场景：在高并发、故障恢复、扩缩容、跨组件协作或线上排障中，用户态与内核态通常会和系统调用和中断一起出现。此时重点看边界条件、顺序约束、资源消耗和异常恢复路径。","边界情况：这个知识点需要结合流程和指标判断。常见边界包括空输入、重复请求、超时、容量上限、权限限制、版本差异和依赖不可用；遇到异常时先确认内核边界、系统调用、特权级和中断路径是否仍然成立。","常见误区与注意点：实践中容易把用户态与内核态当成孤立概念处理，结果遗漏权限切换、异常处理、驱动交互和系统调用开销。落地时要同时记录配置、指标、日志、链路和回滚手段，用小规模验证确认行为符合预期。","参考来源：本讲解参考Linux Kernel 文档、Linux man-pages、POSIX/Open Group 规范、GNU Bash 手册以及 CS-Notes、JavaGuide、小林 coding，优先采用官方定义、命令语义、工程约束和主流面试资料中的稳定结论。"], typicalProblems: ["用户态与内核态底层机制是什么","用户态与内核态和相关概念如何区分","用户态与内核态线上异常如何排查"], prerequisites: ["kernel"], related: ["system-call","interrupt"], learningPathPosition: 3 },
  ...
  /* <!-- KG_REVIEWED: epoll | 2026-05-24 | source_count=7 --> */
