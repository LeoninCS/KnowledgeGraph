...
function arp(point: GraphKnowledgePoint) {
  const actors = [
    actor("host", "主机 A", "Host A", "知道目标 IP", "Knows target IP", "client"),
    actor("lan", "局域网", "LAN", "广播域", "Broadcast domain", "network"),
    actor("target", "目标/网关", "Target / gateway", "持有目标 MAC", "Owns target MAC", "server"),
  ];

  return makeSimulation(
    "network",
    point,
    tx("广播解析", "Broadcast resolution"),
    tx("模拟 ARP Request、ARP Reply 和缓存写入。", "Simulate ARP Request, ARP Reply, and cache update."),
    actors,
    {
      host: tx("ARP 缓存缺失", "ARP cache miss"),
      lan: tx("等待广播", "Waiting for broadcast"),
      target: tx("MAC 可用", "MAC available"),
    },
    [
      step("判断同网段", "Check subnet", "判断下一跳", "Check next hop", "host", "host", "IP & Mask", "IP & mask", "主机先判断目标在本地网段还是交给网关。", "The host decides whether the target is local or via gateway.", "跨网段时 ARP 解析的是网关 MAC。", "For cross-subnet traffic, ARP resolves the gateway MAC.", { host: tx("已确定查询对象", "Target chosen") }),
      step("广播请求", "Broadcast request", "发送 ARP Request", "Send ARP Request", "host", "lan", "Who has IP?", "Who has IP?", "ARP Request 以广播方式进入局域网。", "ARP Request enters the LAN as broadcast.", "广播范围由二层广播域决定。", "Broadcast scope is defined by the L2 broadcast domain.", { host: tx("等待应答", "Waiting"), lan: tx("已广播", "Broadcasted") }, "teal"),
      step("单播应答", "Unicast reply", "发送 ARP Reply", "Send ARP Reply", "target", "host", "IP is at MAC", "IP is at MAC", "目标设备返回自己的 MAC 地址。", "The target returns its MAC address.", "应答通常单播返回请求方。", "The reply usually returns as unicast.", { target: tx("已应答", "Replied"), host: tx("收到 MAC", "MAC received") }, "success"),
      step("写入缓存", "Update cache", "更新 ARP 缓存", "Update ARP cache", "host", "host", "ARP cache", "ARP cache", "主机缓存 IP 到 MAC 的映射，后续帧可直接发送。", "The host caches IP-to-MAC mapping for later frames.", "缓存错误会直接影响二层投递。", "Wrong cache entries directly affect L2 delivery.", { host: tx("缓存命中", "Cache hit"), lan: tx("待转发数据帧", "Ready for frames") }, "success"),
    ],
    [tx("广播域", "Broadcast domain"), tx("ARP 缓存", "ARP cache"), tx("下一跳 MAC", "Next-hop MAC")],
  );
}
...
function signal(point: GraphKnowledgePoint) {
  return flow(
    "network",
    point,
    ["信号、带宽、延迟与吞吐", "Signal, bandwidth, latency, and throughput"],
    [
      "把比特编码成物理信号，沿链路传播，并观察容量、延迟、丢包和协议开销如何决定实际吞吐。",
      "Encode bits into physical signals, send them across the path, and observe how capacity, latency, loss, and protocol overhead decide real throughput.",
    ],
    [
      ["bits", "比特流", "Bit stream", "应用数据转换成 0/1", "Application data becomes 0/1", "client"],
      ["signal", "物理信号", "Physical signal", "电、光或无线电波", "Electrical, optical, or radio wave", "network"],
      ["path", "传输路径", "Transmission path", "距离、介质、干扰、排队", "Distance, media, interference, queueing", "network"],
      ["bottleneck", "瓶颈链路", "Bottleneck link", "最小容量与共享竞争", "Smallest capacity and sharing contention", "data"],
      ["receiver", "接收端体验", "Receiver experience", "吞吐、RTT、抖动、丢包", "Throughput, RTT, jitter, loss", "server"],
    ],
    {
      bits: tx("待编码", "Awaiting encoding"),
      signal: tx("等待调制", "Awaiting modulation"),
      path: tx("链路空闲", "Path idle"),
      bottleneck: tx("容量待观察", "Capacity pending"),
    ...
    [
      [
        ...
        "bits -> signal",
        "bits -> signal",
        "网卡、光模块或无线设备把比特编码成介质可传播的电、光或无线信号。",
        "NIC, optical module, or wireless device encodes bits into electrical, optical, or radio signals that media can carry.",
        "物理层质量决定误码、协商速率和链路稳定性。",
        "Physical quality decides bit errors, negotiated speed, and link stability.",
        { bits: tx("已编码", "Encoded"), signal: tx("信号已生成", "Signal generated") },
      ],
      [
        "经过介质",
        "Traverse media",
        ...
        "propagation + queue",
        "propagation + queue",
        "信号沿双绞线、光纤、无线信道和中间设备传播，距离、排队和设备处理共同形成延迟。",
        "Signals traverse copper, fiber, wireless channels, and middleboxes; distance, queueing, and processing form latency.",
        "RTT、抖动和排队时延决定交互体验。",
        "RTT, jitter, and queueing delay shape interactive experience.",
        { signal: tx("传播中", "Propagating"), path: tx("产生时延", "Latency added") },
        "teal",
      ],
      [
        "遇到瓶颈",
        "Hit bottleneck",
        "观察容量上限",
        "Observe capacity limit",
        "path",
        "bottleneck",
        "bandwidth limit",
        "bandwidth limit",
        "端到端路径的有效容量由最窄链路、共享竞争、限速策略和设备处理能力共同限制。",
        "End-to-end effective capacity is limited by the narrowest link, sharing contention, rate limits, and device processing.",
        "标称带宽是容量上限，实际吞吐还要看协议和路径状态。",
        "Advertised bandwidth is a capacity ceiling; real throughput also depends on protocol and path state.",
        { path: tx("流量进入瓶颈", "Traffic enters bottleneck"), bottleneck: tx("容量受限", "Capacity constrained") },
        "warning",
      ],
      [
        "形成吞吐",
        ...
        "throughput + loss",
        "throughput + loss",
        "接收端看到的是吞吐、RTT、抖动和丢包的组合结果，协议头、重传、窗口和拥塞控制都会降低有效速率。",
        "The receiver sees combined throughput, RTT, jitter, and loss; headers, retransmission, windows, and congestion control reduce effective rate.",
        "iperf 看吞吐，ping/mtr 看延迟与丢包，接口错误计数看链路质量。",
        "iperf checks throughput, ping/mtr checks latency and loss, and interface counters reveal link quality.",
        { bottleneck: tx("输出有效速率", "Effective rate output"), receiver: tx("体验可观测", "Experience observable") },
        "success",
      ],
    ],
    [
      ["带宽容量", "Bandwidth capacity"],
      ["RTT/抖动", "RTT / jitter"],
      ["丢包重传", "Loss and retransmission"],
      ["实际吞吐", "Real throughput"],
    ],
  );
}
...
function tcpIpModel(point: GraphKnowledgePoint) {
  return flow(
    "network",
    point,
    ["TCP/IP 四层路径", "TCP/IP four-layer path"],
    [
      "按应用层、传输层、Internet 层和链路层推进一次请求，观察字段新增、路由选择、下一跳帧和反向解封装。",
      "Step through one request across application, transport, Internet, and link layers, watching headers, routing, next-hop framing, and reverse decapsulation.",
    ],
    [
      ["application", "应用层", "Application", "HTTP、DNS、SSH、SMTP 与业务语义", "HTTP, DNS, SSH, SMTP, and business semantics", "client"],
      ["transport", "传输层", "Transport", "TCP/UDP、端口、连接、可靠性", "TCP / UDP, ports, connection, reliability", "network"],
      ["internet", "Internet 层", "Internet", "IP、ICMP、路由表、TTL、默认网关", "IP, ICMP, route table, TTL, default gateway", "network"],
      ["link", "链路层", "Link", "Ethernet/Wi-Fi、ARP、MAC、VLAN、MTU、帧校验", "Ethernet / Wi-Fi, ARP, MAC, VLAN, MTU, frame check", "data"],
      ["receiver", "接收端协议栈", "Receiver stack", "按链路、IP、传输、应用逐层解封装", "Decapsulates link, IP, transport, and application layers", "server"],
    ],
    {
      application: tx("应用数据待发送", "App data ready"),
      transport: tx("等待端口", "Awaiting ports"),
      internet: tx("等待 IP/路由", "Awaiting IP/route"),
    ...
    [
      [
        ...
        "HTTP/DNS payload",
        "HTTP / DNS payload",
        "应用层把用户动作表达成 HTTP、DNS、SMTP、SSH 或业务 RPC 载荷，并决定域名、路径、Header 与 Body。",
        "The application layer expresses user intent as HTTP, DNS, SMTP, SSH, or RPC payload and decides host name, path, headers, and body.",
        "应用层排查看域名、URL、Header、状态码、序列化格式和业务日志。",
        "Application-layer debugging checks name, URL, headers, status, serialization, and business logs.",
        { application: tx("载荷与目标已确定", "Payload and target fixed"), transport: tx("准备选择 TCP/UDP", "Choosing TCP/UDP") },
      ],
      [
        "传输层分段",
        "Transport segment",
        ...
        "src/dst port + seq / ack",
        "传输层写入源端口、目标端口和 TCP/UDP 控制字段，把应用载荷变成 segment 或 datagram。",
        "The transport layer adds source port, destination port, and TCP/UDP control fields, turning payload into a segment or datagram.",
        "端口定位进程；握手、重传、窗口、RST、FIN 和连接状态都在这一层分析。",
        "Ports identify processes; handshake, retransmission, windows, RST, FIN, and connection state are inspected here.",
        { transport: tx("Segment/Datagram 已形成", "Segment/datagram ready"), internet: tx("等待 IP 头", "Awaiting IP header") },
        "teal",
      ],
      [
        "Internet 层选路",
        "Internet routing",
        "写入 IP/TTL 并查路由",
        "Add IP/TTL and route",
        "internet",
        "link",
        "src/dst IP + TTL + next hop",
        "src/dst IP + TTL + next hop",
        "Internet 层写入源 IP、目的 IP 和 TTL，再根据子网掩码、路由表和默认网关选择下一跳。",
        "The Internet layer adds source IP, destination IP, and TTL, then chooses next hop from subnet mask, route table, and default gateway.",
        "IP、子网掩码、路由表、默认网关、NAT、TTL 和 ICMP 是这一层的核心信号。",
        "IP, subnet mask, route table, default gateway, NAT, TTL, and ICMP are key signals here.",
        { internet: tx("IP Packet 与下一跳就绪", "IP packet and next hop ready"), link: tx("准备解析下一跳 MAC", "Resolving next-hop MAC") },
        "warning",
      ],
      [
        "链路层成帧",
        "Link framing",
        "写入 MAC 并发送帧",
        "Add MAC and send frame",
        "link",
        "receiver",
        "next-hop MAC + frame + FCS",
        "next-hop MAC + frame + FCS",
        "链路层用 ARP 或邻居表拿到下一跳 MAC，写入帧头、可选 VLAN、Payload 和 FCS，并把帧编码成链路信号。",
        "The link layer resolves next-hop MAC through ARP or neighbor table, writes frame header, optional VLAN, payload, and FCS, then encodes the frame onto the link.",
        "MAC、ARP、VLAN、MTU、FCS/CRC、网卡速率和物理链路都在这一层分析。",
        "MAC, ARP, VLAN, MTU, FCS/CRC, NIC speed, and physical link are inspected here.",
        { link: tx("下一跳帧已发出", "Next-hop frame sent"), receiver: tx("收到链路帧", "Link frame received") },
        "warning",
      ],
      [
        "反向解封装",
        ...
        "Ethernet -> IP -> TCP / UDP -> app",
        "接收端先校验帧，再按 Link -> Internet -> Transport -> Application 拆头，最终把载荷交给目标进程或应用协议。",
        "The receiver validates the frame, then removes headers in Link -> Internet -> Transport -> Application order and delivers payload to the target process or application protocol.",
        "抓包读法通常从 Ethernet 到 IP，再到 TCP/UDP，最后查看 HTTP/DNS 等应用数据。",
        "Packet reading usually goes from Ethernet to IP, then TCP/UDP, then HTTP/DNS or other application data.",
        { receiver: tx("解封装并处理完成", "Decapsulation and handling complete"), application: tx("响应返回应用", "Response delivered to app") },
        "success",
      ],
    ],
    [
      ["应用语义", "Application semantics"],
      ["端口与连接", "Ports and connections"],
      ["IP/TTL/网关", "IP / TTL / gateway"],
      ["MAC/帧/FCS", "MAC / frame / FCS"],
    ],
  );
}
