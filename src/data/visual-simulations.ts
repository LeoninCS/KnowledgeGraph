  ...
  return flow(
    ...
    [
      [
        ...
        "应用层排查看 URL、域名、Header、状态码和业务日志。",
        "Application debugging checks URL, name, headers, status code, and business logs.",
        { application: tx("载荷就绪", "Payload ready"), transport: tx("准备添加端口", "Preparing ports") },
      ],
      [
        "传输封装",
        "Transport wrap",
        ...
        "segment/datagram",
        "segment / datagram",
        "传输层写入源端口和目标端口，并提供连接、可靠传输或轻量数据报能力。",
        "Transport adds source and destination ports and provides connection, reliability, or lightweight datagram behavior.",
        "端口、握手、重传、窗口和连接状态都在这一层分析。",
        "Ports, handshake, retransmission, windows, and connection states are analyzed here.",
        { transport: tx("段/数据报已形成", "Segment/datagram ready"), internet: tx("准备添加 IP", "Preparing IP") },
        "teal",
      ],
      [
        "Internet 寻址",
        ...
        "IP packet",
        "IP packet",
        "Internet 层写入源 IP、目标 IP、TTL 等字段，并通过路由表选择下一跳。",
        "The Internet layer adds source IP, destination IP, TTL, and chooses next hop by route table.",
        "IP、路由、网关、NAT 和 ICMP 是这一层的核心信号。",
        "IP, routes, gateways, NAT, and ICMP are key signals at this layer.",
        { internet: tx("IP 包已形成", "IP packet ready"), link: tx("准备链路投递", "Preparing link delivery") },
        "warning",
      ],
      [
        "链路投递",
        ...
        "Ethernet frame",
        "Ethernet frame",
        "链路层处理 ARP、MAC、VLAN、MTU 和帧校验，把帧变成电、光或无线信号交给下一跳。",
        "The link layer handles ARP, MAC, VLAN, MTU, and frame checks, then sends frames as electrical, optical, or wireless signals to the next hop.",
        "MAC、ARP、VLAN、MTU 和物理链路都在这一层分析。",
        "MAC, ARP, VLAN, MTU, and physical link are analyzed here.",
        { link: tx("帧已发送", "Frame sent"), receiver: tx("收到帧", "Frame received") },
        "warning",
      ],
      [
        "接收端解封装",
        ...
        "Ethernet -> IP -> TCP / UDP -> app",
        "接收端按链路、Internet、传输、应用的顺序拆头，最终把载荷交给目标进程或应用协议。",
        "The receiver removes headers in link, Internet, transport, and application order, then delivers payload to the target process or application protocol.",
        "抓包读法通常就是从 Ethernet 到 IP，再到 TCP/UDP，最后看应用协议。",
        "Packet reading usually goes from Ethernet to IP, then TCP/UDP, then application protocol.",
        { receiver: tx("解封装完成", "Decapsulation complete"), application: tx("应用收到响应", "Application received response") },
        "success",
      ],
    ],
    [
