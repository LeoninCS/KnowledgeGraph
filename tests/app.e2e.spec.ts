...
test("home graph supports search and opens detail", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Go Module" })).toBeVisible();
  ...
  await page.getByRole("button", { name: /^TCP 当前聚焦$/ }).click();
...
test("detail page can open a simulation and step through it", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Go Module" }).click();
